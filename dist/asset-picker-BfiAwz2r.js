import { css as _, LitElement as y, html as n, nothing as v, svg as ze } from "lit";
import { property as h, query as Ze, customElement as $, state as b } from "lit/decorators.js";
class fs {
  constructor(e) {
...
class ms {
  ...
  handleSelect(e, s, i) {
    ...
    if (!(((c = a.config) == null ? void 0 : c.multiSelect) ?? !0)) {
      r.clear(), r.set(e.uuid, e), this.store.setState({ selectedAssets: r }), this.lastClickedIndex = s;
      return;
    }
    if (i.shiftKey && this.lastClickedIndex >= 0) {
      const u = Math.min(this.lastClickedIndex, s), f = Math.max(this.lastClickedIndex, s), p = a.assets;
      for (let g = u; g <= f; g++)
        p[g] && r.set(p[g].uuid, p[g]);
    } else i.metaKey || i.ctrlKey ? r.has(e.uuid) ? r.delete(e.uuid) : r.set(e.uuid, e) : (r.clear(), r.set(e.uuid, e));
    const l = (d = a.config) == null ? void 0 : d.maxSelections;
    l && r.size > l || (this.lastClickedIndex = s, this.store.setState({ selectedAssets: r }));
  }
  isSelected(e) {
...
class _s {
  ...
  updateMarqueeRect(e) {
    ...
    this.rect = {
      x: Math.min(this.startX, i),
      y: Math.min(this.startY, a),
      width: Math.abs(i - this.startX),
      height: Math.abs(a - this.startY)
    }, this.host.requestUpdate(), this.selectIntersecting();
  }
  startAutoScroll(e) {
    if (!this.container) return;
    const s = this.container.getBoundingClientRect(), i = e.clientY - s.top, a = s.bottom - e.clientY, r = i < fe || a < fe;
  ...
  autoScrollTick() {
    ...
    let r = 0;
    i < fe ? r = -Mt * (1 - i / fe) : a < fe && (r = Mt * (1 - a / fe)), r !== 0 && (this.container.scrollTop += r, this.updateMarqueeRect(s), this._scrollRAF = requestAnimationFrame(() => this.autoScrollTick()));
  }
  stopAutoScroll() {
  ...
  queryCards() {
    for (const e of Array.from(this.container.children))
      if (e.shadowRoot) {
        const s = e.shadowRoot.querySelectorAll("[data-asset-uuid]");
  ...
  selectIntersecting() {
    var a;
    if (!this.container) return;
    const e = this.queryCards(), s = new Map(this.preMarqueeSelection);
    e.forEach((r) => {
      const o = r.getBoundingClientRect(), l = this.container.getBoundingClientRect(), c = {
        x: o.left - l.left + this.container.scrollLeft,
        y: o.top - l.top + this.container.scrollTop,
        width: o.width,
        height: o.height
      }, d = this.rect.x < c.x + c.width && this.rect.x + this.rect.width > c.x && this.rect.y < c.y + c.height && this.rect.y + this.rect.height > c.y, u = r.dataset.assetUuid;
      if (d) {
        const f = this.store.getState().assets.find((p) => p.uuid === u);
        f && s.set(u, f);
      }
    });
    const i = (a = this.store.getState().config) == null ? void 0 : a.maxSelections;
    if (i && s.size > i) {
      const r = Array.from(s.entries()).slice(0, i);
      this.store.setState({ selectedAssets: new Map(r) });
    } else
    ...
    try {
      ...
      if (clearTimeout(o), !l.ok) {
        ...
        try {
          ...
          u.msg && (d = `API error: ${l.status} - ${u.msg}`);
        } catch {
        }
        throw new Error(d);
      }
      ...
      throw clearTimeout(o), l instanceof DOMException && l.name === "AbortError" ? new Error("API request timed out") : l;
    }
  }
  async post(e, s) {
...
Object.fromEntries(
  Object.entries(Je).map(([t, e]) => [e, t])
);
const Kt = Object.values(Je), Ce = {
  PRESET: "preset",
  AFTER: "after",
  BEFORE: "before",
  BETWEEN: "between",
  ...
  EMPTY: "empty",
  NOT_EMPTY: "non-empty"
}, U = "empty", q = "non-empty", oe = "specific", ct = 10, dt = 10, Ee = 10, Gt = "filters:pinned:", Ht = "metadata:pinned:", It = [
  m.DATE,
  m.TYPE,
  m.METADATA
], Yt = x.IS, Lt = "30", Rt = "1", ks = 0, Fs = [
  { key: m.DATE, label: "Date", icon: "calendar-days", type: "date" },
  { key: m.TYPE, label: "Format", icon: "file-type", type: "string" },
  { key: m.SIZE, label: "Size", icon: "scaling", type: "string" },
  { key: m.LICENSE_EXPIRY, label: "License expiry", icon: "history", type: "date" },
  { key: m.TAGS, label: "Tags", icon: "hash", type: "string" },
  { key: m.LABELS, label: "Labels", icon: "tag", type: "string" },
  ...
  [m.DATE]: "Date",
  [m.SIZE]: "Size",
  [m.LICENSE_EXPIRY]: "License expiry",
  [m.TAGS]: "Tags",
  [m.LABELS]: "Labels",
  [m.PRODUCT_REF]: "Products",
  [m.IMAGE]: "Image",
  [m.COLOR]: "Color",
  [m.METADATA]: "Metadata",
  [m.APPROVAL]: "Approval"
...
async function Vs(t) {
  var C, O, I, N;
  const e = await t.request("/settings"), s = e.settings ?? e, i = (s == null ? void 0 : s.metadata) ?? e.metadata, r = ((i == null ? void 0 : i.model) ?? [])[ks], o = i == null ? void 0 : i.store, c = ((o == null ? void 0 : o.regional_variants_groups) ?? []).map((E) => ({
    uuid: E.uuid,
    label: E.label ?? E.name ?? "",
    variants: (E.variants ?? []).map((k) => ({
      api_value: k.api_value ?? k.value ?? "",
      internal_unique_value: k.internal_unique_value ?? k.api_value ?? "",
      label: k.label ?? k.name ?? ""
    }))
  })).filter((E) => E.variants.length > 1), d = {};
  for (const E of c)
    d[E.uuid] = ((C = E.variants[0]) == null ? void 0 : C.api_value) ?? "";
  const u = (s == null ? void 0 : s.project_branding) ?? ((O = e.settings) == null ? void 0 : O.project_branding), f = u == null ? void 0 : u.brandColor;
  if (!r) return { fields: [], regionalVariantGroups: c, regionalFilters: d, brandColor: f };
  const p = r.groups ?? [], g = [];
  for (const E of p) {
    const k = (E.fields ?? []).find(
      (F) => F.type === "select-one" || F.type === "multi-select"
    );
    if (k) {
  ...
  for (const E of p) {
    const k = ((I = E.name) == null ? void 0 : I.toLowerCase()) === "products" || ((N = E.name) == null ? void 0 : N.toLowerCase()) === "product fields";
    for (const F of E.fields ?? []) {
      const R = F.possible_values, Re = R == null ? void 0 : R.map((K) => ({
        api_value: K.api_value ?? K.value ?? "",
        internal_unique_value: K.internal_unique_value ?? K.api_value ?? "",
        label: K.label ?? K.name ?? K.api_value ?? ""
      }));
      g.push({
  ...
  return { fields: g, regionalVariantGroups: c, regionalFilters: d, brandColor: f };
}
function ne(t) {
...
function Zt(t) {
  ...
  switch (t) {
    ...
    case "last_week": {
      const s = new Date(e);
      return s.setDate(s.getDate() - 7), { from: ne(s), to: null };
    }
    case "within_week": {
      const s = new Date(e);
      return s.setDate(s.getDate() + 7), { from: je(), to: ne(s) };
    }
    case "last_month": {
      const s = new Date(e);
      return s.setDate(s.getDate() - 30), { from: ne(s), to: null };
    }
    case "within_month": {
      const s = new Date(e);
      return s.setDate(s.getDate() + 30), { from: je(), to: ne(s) };
    }
    case "last_year": {
      const s = new Date(e);
      return s.setDate(s.getDate() - 365), { from: ne(s), to: null };
    }
    case "within_year": {
      const s = new Date(e);
      return s.setDate(s.getDate() + 365), { from: je(), to: ne(s) };
    }
    default:
      return null;
  }
...
function jt(t, e) {
  return e ? e.type === "date" ? Bs(e) : e.type === "string" ? Us(e) : !0 : !1;
}
function Bs(t) {
...
function Gs(t, e) {
  if (e.type === "date")
    return Jt(t, e);
  const s = e, { operator: i = Yt, values: a = [], logic: r } = s;
...
function Jt(t, e) {
  ...
  if (e.kind === Ce.PRESET && e.preset) {
    if (e.preset === Dt.EMPTY)
      return [`${s}:"empty"`];
    if (e.preset === Dt.NOT_EMPTY)
      return [`${s}:"non-empty"`];
    const i = Zt(e.preset);
    return i ? i.to === null ? [`${s}>"${i.from}"`] : [`${s}:"${i.from}..${i.to}"`] : [];
  }
  if (e.kind === Ce.AFTER && e.from)
    return [`${s}:>"${e.from}"`];
  if (e.kind === Ce.BEFORE && e.to)
    return [`${s}:<"${e.to}"`];
  if (e.kind === Ce.BETWEEN) {
    if (e.from && e.to)
  ...
  return e.kind === Ce.SPECIFIC && e.from ? [`${s}:"${e.from}"`] : [];
}
function Hs(t, e, s) {
...
function Xs(t, e, s) {
  const i = [], a = le("color_search", t, e, ",", s);
...
function Ws(t) {
  ...
  if (typeof t == "object" && t !== null && !Array.isArray(t)) {
    const o = t, l = o.resolution, c = o.orientation, d = o.faces;
    return l != null && l.length && e.push(`resolution:"${l.join(",")}"`), c != null && c.length && e.push(`orientation:"${c.join(",")}"`), d != null && d.length && e.push(`faces:"${d.join(",")}"`), e;
  }
  const s = t, i = s[pt.RESOLUTION], a = s[pt.ORIENTATION], r = s[pt.FACES];
...
function Qs(t, e) {
  ...
  if (e.type === "date")
    return Zs(s, e, i);
  const a = e, { operator: r = Yt, values: o = [], logic: l, metadataType: c } = a;
  return o.length === 0 ? [] : (c === w.NUMERIC || c === w.DECIMAL2) && r === x.RANGE ? [`${s}${x.IS}"${o.join('","')}"`] : c === w.GEO_POINT ? le(s, r + "~", o, ",", l) : le(s, r, o, ",", l);
}
function Zs(t, e, s) {
  const i = Jt(t, e);
  return !s || i.length === 0 ? i : i.map((a) => {
    ...
    if (r === -1) return `"${a}"`;
    const o = a.slice(0, r), c = a.slice(r).replace(/"/g, "");
    return `"${o}${c}"`;
  });
}
function Js(t) {
...
function ta(t) {
  const e = localStorage.getItem(Gt + t), s = localStorage.getItem(Ht + t);
  let i;
  try {
    i = e ? JSON.parse(e) : [...It];
  } catch {
...
function sa(t, e) {
  localStorage.setItem(Gt + t, JSON.stringify(e));
}
function aa(t, e) {
  localStorage.setItem(Ht + t, JSON.stringify(e));
}
const es = "sort-by", ts = "sort-order";
function Ut(t, e) {
  try {
    localStorage.setItem(es, t), localStorage.setItem(ts, e);
  } catch {
  }
}
function ia() {
  try {
    const t = localStorage.getItem(es), e = localStorage.getItem(ts);
...
var na = Object.defineProperty, la = Object.getOwnPropertyDescriptor, $t = (t, e, s, i) => {
...
var da = Object.defineProperty, pa = Object.getOwnPropertyDescriptor, et = (t, e, s, i) => {
...
et([
  h({ type: Object })
], ge.prototype, "selectedFilters", 2);
et([
  b()
], ge.prototype, "_open", 2);
ge = et([
  $("ap-regional-settings")
], ge);
var ha = Object.defineProperty, ua = Object.getOwnPropertyDescriptor, se = (t, e, s, i) => {
...
let G = class extends y {
  constructor() {
    super(...arguments), this.activeTab = "assets", this.hiddenTabs = [], this.viewMode = "grid", this.searchQuery = "", this.regionalGroups = [], this.regionalFilters = {}, this._localSearch = "", this._debouncedSearch = ca((t) => {
  ...
  render() {
    const t = fa.filter((e) => !this.hiddenTabs.includes(e.value));
    return n`
...
se([
  h({ type: Array })
], G.prototype, "hiddenTabs", 2);
se([
  h()
], G.prototype, "viewMode", 2);
se([
  h()
], G.prototype, "searchQuery", 2);
se([
  h({ type: Array })
], G.prototype, "regionalGroups", 2);
se([
  h({ type: Object })
], G.prototype, "regionalFilters", 2);
se([
  b()
], G.prototype, "_localSearch", 2);
G = se([
  $("ap-header")
], G);
var va = Object.defineProperty, ga = Object.getOwnPropertyDescriptor, ss = (t, e, s, i) => {
...
var ma = Object.defineProperty, ba = Object.getOwnPropertyDescriptor, wt = (t, e, s, i) => {
...
var _a = Object.defineProperty, ya = Object.getOwnPropertyDescriptor, as = (t, e, s, i) => {
...
`;
var xa = Object.defineProperty, $a = Object.getOwnPropertyDescriptor, he = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? $a(e, s) : e, r = t.length - 1, o; r >= 0; r--)
    (o = t[r]) && (a = (i ? o(e, s, a) : o(a)) || a);
...
let X = class extends y {
  ...
  get _rangeOptions() {
    return this._isLicenseExpiry ? Qt : yt;
  }
  get _todayStr() {
    return vt(/* @__PURE__ */ new Date());
  }
  get _hasFilter() {
  ...
  _renderDateInputs() {
    ...
    const e = this.from ? vt(new Date(this.from)) : "", s = this.to ? vt(new Date(this.to)) : "", i = this._todayStr;
    return t === "specific" ? n`
...
var wa = Object.defineProperty, Sa = Object.getOwnPropertyDescriptor, xe = (t, e, s, i) => {
...
let Z = class extends y {
  constructor() {
    super(...arguments), this.available = [], this.selected = [], this.suggested = [], this.filterKey = m.TAGS, this._search = "";
  }
  get _atLimit() {
    return this.selected.length >= ct;
  }
  get _isSearching() {
  ...
  _renderTagItem(t) {
    const e = this.selected.includes(t), s = !e && this._atLimit;
    return n`
      <div
        class="tag-item ${s ? "disabled" : ""}"
        title=${s ? `You can select up to ${ct} tags` : v}
  ...
  _renderSearchResults(t) {
    const e = this.available.filter(
      (s) => s.toLowerCase().includes(t)
    );
    return e.length === 0 ? n`<div class="no-results">No tags found</div>` : n`
  ...
  _renderSections() {
    const t = this.suggested.filter(
      (s) => this.available.includes(s)
    ), e = this.available;
    return n`
      ${t.length > 0 ? n`
            <div class="filter-section">
      ...
      <div class="filter-section">
        ...
        <div class="options-list">
          ${e.length === 0 ? n`<div class="no-results">No tags available</div>` : e.map((s) => this._renderTagItem(s))}
        </div>
      </div>
...
xe([
  h({ type: Array })
], Z.prototype, "available", 2);
xe([
  h({ type: Array })
], Z.prototype, "selected", 2);
xe([
  h({ type: Array })
], Z.prototype, "suggested", 2);
xe([
  h()
], Z.prototype, "filterKey", 2);
xe([
  b()
...
var Ca = Object.defineProperty, Ea = Object.getOwnPropertyDescriptor, tt = (t, e, s, i) => {
...
let me = class extends y {
  ...
  get _atLimit() {
    return this.selected.length >= dt;
  }
  get _isSearching() {
    return this._search.trim().length > 0;
  }
  ...
  _getLabelByUuid(t) {
    return this.labels.find((e) => e.uuid === t);
  }
  _renderLabelItem(t) {
    const e = this.selected.includes(t.uuid), s = !e && this._atLimit;
    return n`
      <div
        class="label-item ${s ? "disabled" : ""}"
        title=${s ? `You can select up to ${dt} labels` : v}
      ...
      <div class="filter-content">
        ...
        ${!this._isSearching && this.selected.length > 0 ? n`
              <div class="filter-section">
                <div class="chips-wrap">
                  ${this.selected.map((s) => {
      const i = this._getLabelByUuid(s);
      return i ? n`
                      <div class="chip">
                        <div
        ...
        ${this._atLimit ? n`
              <div class="info-alert">
                <ap-icon name="info" .size=${14}></ap-icon>
                <span>You can select up to ${dt} labels.</span>
              </div>
            ` : v}
      </div>
    `;
...
var ka = Object.defineProperty, Fa = Object.getOwnPropertyDescriptor, st = (t, e, s, i) => {
...
let J = class extends y {
  constructor() {
    super(...arguments), this.colors = [], this._advanced = !1, this._showPalette = !1;
  }
  get _hasSelection() {
    return this.colors.length > 0;
  }
  get _selectedHexes() {
    return new Set(this.colors.map((t) => t.hex));
  }
  _selectColor(t) {
    if (!J._HEX_RE.test(t)) return;
    const e = this.colors.findIndex((s) => s.hex === t);
    if (e >= 0) {
      this._removeColor(e);
      return;
    }
    this.colors.length >= gt || (this.colors = [
      ...this.colors,
      {
        hex: t,
        tolerance: Rt,
  ...
  _removeColor(t) {
    this.colors = this.colors.filter((e, s) => s !== t), this._dispatchChange();
  }
  _clearAll() {
    this.colors = [], this._showPalette = !1, this._dispatchChange();
  }
  _toggleAdvanced() {
    this._advanced = !this._advanced, this.colors.length > 0 && this._dispatchChange();
  }
  _handleToleranceChange(t, e) {
    const s = [...this.colors];
    s[t] = { ...s[t], tolerance: e.detail.value }, this.colors = s, this._dispatchChange();
  }
  _handleCoverageInput(t, e) {
    const s = e.target.value, i = parseInt(s, 10);
    if (!isNaN(i) && i >= 1 && i <= 100) {
      const a = [...this.colors];
      a[t] = { ...a[t], coverage: String(i) }, this.colors = a, this._dispatchChange();
    }
  }
  _showAddPalette() {
    this._showPalette = !0;
  ...
  _dispatchChange() {
    ...
    const t = this.colors.map((e) => {
      const s = this._advanced ? e.tolerance : Rt, i = this._advanced ? e.coverage : Lt;
      return `${e.hex} ${s} ${s} ${i}`;
    });
    this.dispatchEvent(
      new CustomEvent("filter-change", {
  ...
  _renderPalette() {
    const t = this._selectedHexes, e = this.colors.length >= gt;
    return n`
      <div class="palette">
        ${Is.map(
      (s) => {
  ...
  render() {
    const t = this.colors.length === 0 || this._showPalette, e = this.colors.length < gt && !this._showPalette;
    return n`
      <div class="filter-content">
        <button
          class="clear-btn"
        ...
        ${this.colors.length > 0 ? n`
          <div class="filter-section">
            ${this._advanced ? n`
              <div class="selected-colors">
                ${this.colors.map(
      (s, i) => this._renderAdvancedColorEntry(s, i)
    )}
              </div>
            ` : n`
              <div class="selected-colors">
                ${this.colors.map(
      (s, i) => this._renderNormalColorEntry(s, i)
    )}
              </div>
            `}
          </div>
...
J.styles = [ae, _`
  ...
  `];
J._HEX_RE = /^#[0-9A-Fa-f]{6}$/;
st([
  b()
], J.prototype, "colors", 2);
st([
  b()
], J.prototype, "_advanced", 2);
st([
  b()
], J.prototype, "_showPalette", 2);
J = st([
  $("ap-filter-color")
], J);
var Ta = Object.defineProperty, Aa = Object.getOwnPropertyDescriptor, ue = (t, e, s, i) => {
...
let W = class extends y {
  ...
  _emitChange() {
    const t = Oa[this._unit], e = this._minVal !== "" ? parseFloat((parseFloat(this._minVal) * t).toFixed(2)).toString() : null, s = this._maxVal !== "" ? parseFloat((parseFloat(this._maxVal) * t).toFixed(2)).toString() : null;
    let i;
    if (e !== null && s !== null)
      i = `${e}..${s}`;
    else if (e !== null)
...
ue([
  b()
], W.prototype, "_minVal", 2);
ue([
  b()
], W.prototype, "_maxVal", 2);
ue([
  b()
], W.prototype, "_unit", 2);
ue([
  b()
], W.prototype, "_initialized", 2);
W = ue([
  $("ap-filter-size")
], W);
var Ma = Object.defineProperty, Da = Object.getOwnPropertyDescriptor, at = (t, e, s, i) => {
...
let T = class extends y {
  ...
  _isSpecialValue(t) {
    return !t || t.length !== 1 ? !1 : t[0] === U || t[0] === q;
  }
  /** Get the "content" mode for a field: 'empty', 'not-empty', or 'specific' */
  _getContentMode(t) {
    var s, i;
    const e = this._getApplied(t);
    return ((s = e.values) == null ? void 0 : s[0]) === U ? U : ((i = e.values) == null ? void 0 : i[0]) === q ? q : this._specificModeFields.has(t) || e.values && e.values.length > 0 ? oe : "";
  }
  // ── Event dispatchers ────────────────────────────────────────────
  ...
  _clearFieldFilter(t) {
    ...
    this._dateTos = l;
    const { [e]: c, ...d } = this._selectSearches;
    this._selectSearches = d;
    const { [e]: u, ...f } = this._tagInputs;
    this._tagInputs = f, this._emitFilterChange(t, void 0, []);
  }
  // ── Text filter ──────────────────────────────────────────────────
  _onTextInput(t, e) {
    const s = e.target.value, a = this._getApplied(S(t)).operator || ht[0].value;
    this._emitFilterChange(t, a, s ? [s] : []);
  }
  // ── Number filter ────────────────────────────────────────────────
  _onNumberInput(t, e, s) {
    const i = e.target.value, a = S(t), r = this._getApplied(a), o = r.operator || ut[0].value, l = [...r.values || []];
    s === 0 ? l[0] = i : l[1] = i;
    const c = l.filter((d) => d !== "" && d !== void 0);
    if (c.length > 0) {
      const d = t.type === w.DECIMAL2;
      if (!c.every(
        (f) => d ? !isNaN(parseFloat(f)) : /^-?\d+$/.test(f)
      )) return;
    }
  ...
  _onSelectOneToggle(t, e) {
    const s = S(t), i = this._getApplied(s), a = i.operator || Ne[0].value, r = i.values || [];
    r.length === 1 && r[0] === e ? this._emitFilterChange(t, a, []) : this._emitFilterChange(t, a, [e]);
  }
  // ── Multi-select filter ──────────────────────────────────────────
  _onMultiSelectToggle(t, e) {
    const s = S(t), i = this._getApplied(s), a = i.operator || ke[0].value, r = i.values || [], o = r.includes(e) ? r.filter((l) => l !== e) : [...r, e];
  ...
  _onTagInput(t, e) {
    ...
    if (s.includes(",")) {
      const i = s.replace(/,/g, "").trim();
      if (i) {
        this._tagInputs = { ...this._tagInputs, [t]: i };
        const a = this._getField(t);
        a && this._commitTagInput(a, t);
      }
      return;
    }
    this._tagInputs = { ...this._tagInputs, [t]: s };
  }
  _commitTagInput(t, e) {
    const s = (this._tagInputs[e] || "").trim();
    if (!s) return;
    const i = this._getApplied(e), a = i.operator || ke[0].value, r = i.values || [];
    r.includes(s) || this._emitFilterChange(t, a, [...r, s]), this._tagInputs = { ...this._tagInputs, [e]: "" };
  }
  _removeTag(t, e) {
    const s = S(t), i = this._getApplied(s), a = i.operator || ke[0].value, r = i.values || [];
  ...
  _onDatePreset(t, e) {
    ...
    switch (e) {
      ...
      case "last_week": {
        const o = new Date(i), l = o.getDay();
        o.setDate(o.getDate() - (l === 0 ? 6 : l - 1)), o.setHours(0, 0, 0, 0);
        const c = new Date(o);
        c.setDate(c.getDate() - 7), a = c.toISOString(), r = o.toISOString();
        break;
      }
      case "last_month": {
        const o = new Date(i.getFullYear(), i.getMonth(), 1), l = new Date(o);
        l.setMonth(l.getMonth() - 1), a = l.toISOString(), r = o.toISOString();
        break;
      }
      case "last_year": {
        const o = new Date(i.getFullYear(), 0, 1), l = new Date(o);
        l.setFullYear(l.getFullYear() - 1), a = l.toISOString(), r = o.toISOString();
        break;
      }
    }
  ...
  _onGeoLocationInput(t, e) {
    var u;
    const s = e.target.value.trim(), i = S(t), a = (u = this.shadowRoot) == null ? void 0 : u.querySelector(
      `[data-geo-radius="${i}"]`
    ...
    const o = s.split(",").map((f) => f.trim());
    if (o.length !== 2 || o.some((f) => isNaN(Number(f)) || f === ""))
      return;
    const [l, c] = o;
    let d = `${l},${c}`;
    r && (d += `..${r}`), this._emitFilterChange(t, x.IS, [d]);
  }
  ...
  _renderFieldSelection() {
    ...
    ), i = e.filter((r) => r.group === "product"), a = (r, o) => o.length === 0 ? v : n`
        <div class="field-group-label">${r}</div>
        ${o.map((l) => {
      const c = S(l), d = this.visibleFields.includes(c), u = !d && this._atFieldLimit;
      return n`
            <div
              class="field-item ${d ? "selected" : ""} ${u ? "disabled" : ""}"
              @click=${() => !u && !d && this._addField(l)}
            >
              ${d ? n`<ap-icon name="check" .size=${14}></ap-icon>` : v}
              <span>${l.label}</span>
            </div>
          `;
    })}
    ...
    return n`
      ...
      this._fieldSearch = r.target.value;
        ...
        ${this.visibleFields.length > 0 ? n`
              ...
              >
                ${this.visibleFields.length} / ${Ee} fields selected
              </div>
            ` : v}
      </div>
    `;
  ...
  _renderTextFilter(t) {
    var d, u;
    const e = S(t), s = this._getApplied(e), a = this._isSpecialValue(s.values) ? "" : ((d = s.values) == null ? void 0 : d[0]) || "", r = this._getContentMode(e), o = r === U || r === q, l = s.operator || ht[0].value, c = t.type === w.TEXT_AREA;
    return n`
  ...
  _renderNumberFilter(t) {
    var E, k, F;
    const e = S(t), s = this._getApplied(e), i = s.operator || ut[0].value, a = this._isSpecialValue(s.values), r = this._getContentMode(e), o = r === U || r === q, l = i === x.RANGE, c = i === x.GREATER_THAN_OR_EQUAL, d = i === x.LESS_THAN_OR_EQUAL, u = t.type === w.DECIMAL2, f = u ? "0.01" : "1", p = u ? "0.00" : "0", g = a ? "" : ((E = s.values) == null ? void 0 : E[0]) || "", C = a ? "" : ((k = s.values) == null ? void 0 : k[1]) || "", O = l || c || d, I = l || c || d ? "Min" : "Value";
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
        ...
        <div class="filter-section">
          ...
          <ap-radio-group
            ...
            .value=${i}
            ?disabled=${o}
            @ap-change=${(R) => this._onOperatorChange(t, R.detail.value)}
          ></ap-radio-group>
        </div>
        ...
        <div class="filter-section">
          ...
          <ap-radio-group
            .options=${ft}
            .value=${r}
            @ap-change=${(R) => this._onContentModeChange(t, R.detail.value)}
          ></ap-radio-group>
        </div>
        ...
        ${r === oe ? n`
              <div class="filter-section">
                <div class="grid-2">
                  <div>
                    <span class="input-label" aria-disabled=${d ? "true" : v}>${I}</span>
                    <input
                      class="filter-input"
                      type="number"
                      step=${f}
                      ...
                      .value=${g}
                      ?disabled=${d}
                      @change=${(R) => this._onNumberInput(t, R, 0)}
                    />
                  </div>
                  ${O ? n`
                        <div>
                          ...
                          <input
                            ...
                            .value=${C}
                            ?disabled=${c}
                            @change=${(R) => this._onNumberInput(t, R, 1)}
                          />
                        </div>
                      ` : v}
                </div>
  ...
  _renderSelectOneFilter(t) {
    const e = S(t), s = this._getApplied(e), i = this._isSpecialValue(s.values), a = i ? [] : s.values || [], r = t.possible_values || [], o = s.operator || Ne[0].value, l = (this._selectSearches[e] || "").toLowerCase(), c = l.length > 0, d = a.length > 0 || i, u = l ? r.filter((p) => p.label.toLowerCase().includes(l)) : r, f = a.length >= Ee;
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
        ...
        ${c ? v : n`
              ...
              ${a.length > 0 ? n`
                    <div class="filter-section">
                      <div class="chips-wrap">
                        ${a.map((p) => {
      const g = r.find((C) => C.api_value === p);
      return n`
                            <div class="chip">
      ...
      const g = a.includes(p.api_value), C = !g && f;
      return n`
                    <div
                      class="option-item ${C ? "disabled" : ""}"
                      @click=${() => !C && this._onSelectOneToggle(t, p.api_value)}
                    >
  ...
  _renderMultiSelectFilter(t) {
    const e = S(t), s = this._getApplied(e), i = this._isSpecialValue(s.values), a = i ? [] : s.values || [], r = t.possible_values || [], o = s.operator || ke[0].value, l = (this._selectSearches[e] || "").toLowerCase(), c = l.length > 0, d = a.length > 0 || i, u = l ? r.filter((p) => p.label.toLowerCase().includes(l)) : r, f = a.length >= Ee;
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
      ...
      return n`
                    <div
                      class="option-item ${C ? "disabled" : ""}"
                      @click=${() => !C && this._onMultiSelectToggle(t, p.api_value)}
                    >
                      <ap-checkbox ?checked=${g}></ap-checkbox>
                      <span>${p.label}</span>
                    </div>
  ...
  // ── Render: Tags filter ──────────────────────────────────────────
  _renderTagsFilter(t) {
    const e = S(t), s = this._getApplied(e), i = this._isSpecialValue(s.values), a = i ? [] : s.values || [], r = this._tagInputs[e] || "";
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
  ...
  _renderDateFilter(t) {
    ...
    ], l = this._dateFroms[e] ? new Date(this._dateFroms[e]).toISOString().split("T")[0] : "", c = this._dateTos[e] ? new Date(this._dateTos[e]).toISOString().split("T")[0] : "", d = ["before", "after", "between"].includes(r);
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
  ...
  _renderGeoFilter(t) {
    ...
    if (!i && ((u = s.values) != null && u[0])) {
      const p = s.values[0], [g, C] = p.split("..");
      r = g || "", o = C || "";
    }
    const l = r.length > 0, c = r.split(",").map((p) => p.trim()), d = !l || c.length === 2 && c.every((p) => !isNaN(Number(p)) && p !== "");
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
              ...
              <div class="filter-section">
                <div class="geo-row">
                  <div class="geo-location">
                    ...
                    />
                    ${l && !d ? n`<div class="error-text">Enter coordinates as "latitude, longitude"</div>` : v}
                  </div>
  ...
  _renderSelectorMode() {
    const t = this._fieldSearch.toLowerCase(), e = this.fields.filter(
      ...
      (r) => !r.group || r.group === "root"
        ...
        <div class="field-group-label">${r}</div>
        ${o.map((l) => {
      const c = S(l), d = this.visibleFields.includes(c), u = !!this.appliedMetadata[c] || !!this.appliedFilters[c], f = this.pinnedFields.includes(c), p = !d && this._atFieldLimit;
      return n`
            <div
            ...
            >
              ...
              ${u ? n`<span class="field-item-dot"></span>` : v}
              <button
                class="field-item-pin ${f ? "pinned" : ""}"
                @click=${(g) => {
        g.stopPropagation(), this._emitPin(c, !f);
      }}
...
P([
  h({ type: Array })
], T.prototype, "fields", 2);
P([
  h({ type: Object })
], T.prototype, "appliedFilters", 2);
P([
  h({ type: Array })
], T.prototype, "visibleFields", 2);
P([
  h()
], T.prototype, "mode", 2);
P([
  h()
], T.prototype, "activeFieldKey", 2);
P([
  h({ type: Array })
], T.prototype, "pinnedFields", 2);
P([
  h({ type: Object })
], T.prototype, "appliedMetadata", 2);
P([
  b()
], T.prototype, "_showFieldSelection", 2);
P([
  b()
], T.prototype, "_fieldSearch", 2);
P([
  b()
], T.prototype, "_collapsedFields", 2);
P([
  b()
], T.prototype, "_tagInputs", 2);
P([
  b()
], T.prototype, "_datePresets", 2);
P([
  b()
], T.prototype, "_dateFroms", 2);
P([
  b()
], T.prototype, "_dateTos", 2);
P([
  b()
], T.prototype, "_specificModeFields", 2);
P([
  b()
], T.prototype, "_selectSearches", 2);
T = P([
  $("ap-filter-metadata")
], T);
var Ra = Object.defineProperty, za = Object.getOwnPropertyDescriptor, Y = (t, e, s, i) => {
...
let B = class extends y {
  ...
  get _statusHasEmptyValue() {
    return this.selectedStatus.includes(U) || this.selectedStatus.includes(q);
  }
  ...
  _renderDueDateSection() {
    ...
    return n`
      <div class="filter-section">
        ...
        <ap-radio-group
          .options=${ve}
          .value=${this.dueDatePreset === U || this.dueDatePreset === q ? this.dueDatePreset : ""}
...
Y([
  h({ type: Array })
], B.prototype, "selectedStatus", 2);
Y([
  h()
], B.prototype, "statusOperator", 2);
Y([
  h()
], B.prototype, "approverValue", 2);
Y([
  h()
], B.prototype, "approverOperator", 2);
Y([
  h()
], B.prototype, "requesterValue", 2);
Y([
  h()
], B.prototype, "requesterOperator", 2);
Y([
  h()
], B.prototype, "dueDatePreset", 2);
Y([
  h()
], B.prototype, "dueDateFrom", 2);
Y([
  h()
], B.prototype, "dueDateTo", 2);
B = Y([
  $("ap-filter-approval")
], B);
var Na = Object.defineProperty, ja = Object.getOwnPropertyDescriptor, M = (t, e, s, i) => {
...
let A = class extends y {
  constructor() {
    super(...arguments), this.totalCount = 0, this.totalFolderCount = 0, this.isLoading = !1, this.sortBy = "created_at", this.sortDirection = "desc", this.sortOptions = xt, this.filters = {
      metadata: { pinned: [], visible: [], applied: {} },
      pinned: [],
      visible: [],
      applied: {}
    }, this.labels = [], this.metadataFields = [], this.pinnedFilters = [], this._showDropdown = !1, this._openFilter = null, this._openMetadataField = null, this._externalTrigger = !1, this._externalLeft = null, this._outsideClickHandler = (t) => {
      const e = t.composedPath();
      if (!(this._externalTrigger && e.find(
        (i) => i instanceof HTMLElement && i.tagName === "AP-FILTERS-BAR"
      ))) {
        if (!e.includes(this)) {
        ...
        if (this._openFilter) {
          const s = this.renderRoot.querySelector(".popover-anchor");
          s && !e.includes(s) && (this._openFilter = null, this._openMetadataField = null, this._externalTrigger = !1, this._externalLeft = null);
        }
      }
    };
  }
  ...
  updated(t) {
    if ((t.has("_openFilter") || t.has("_externalTrigger") || t.has("_openMetadataField")) && this.dispatchEvent(new CustomEvent("filter-panel-change", {
      ...
      if (e) {
        const s = e.getBoundingClientRect(), i = window.innerWidth;
        if (s.right > i - 8) {
          const a = s.right - i + 8;
          e.style.left = `${Math.max(8, this._externalLeft - a)}px`;
        }
      }
    }
  }
  _closeAllDropdowns() {
    var t;
    this._showDropdown = !1, this._openFilter = null, this._openMetadataField = null, this._externalTrigger = !1, this._externalLeft = null, (t = this._sortDropdown) == null || t.close();
  }
  _toggleDropdown() {
  ...
  _getMetadataFieldLabel(t) {
    for (const e of this.metadataFields)
      if (`${Je[e.type] || ""}${e.key}` === t) return e.label;
    return t;
  }
  _togglePin(t, e) {
    e.stopPropagation();
  ...
  _renderFilterContent(t) {
    ...
    switch (t) {
      ...
      case "color": {
        const o = e.color, c = (((s = o == null ? void 0 : o.values) == null ? void 0 : s[0]) || "").split(" ");
      ...
      case "size": {
        const o = e.size, l = ((i = o == null ? void 0 : o.values) == null ? void 0 : i[0]) || "", [c, d] = l.split(".."), u = c ? parseFloat(c) / 1e6 : 0, f = d ? parseFloat(d) / 1e6 : 0;
      ...
      case "image": {
        const o = e.image, l = o && "resolution" in o ? o : null;
      ...
      case "approval": {
        const o = e.approval_status, l = e.task_approver, c = e.task_requester, d = e.task_duedate;
    ...
    return n`
          ...
          <div class="filter-dropdown">
            ...
            ${this._showDropdown ? n`
              <div class="dropdown-menu">
                ${zt.map((e) => this._renderFilterButton(e))}
              </div>
            ` : v}
          </div>
          <div class="divider"></div>
      ...
      e.stopPropagation(), this._openMetadataField = null;
                  ...
                  ` : n`
                    ...
                    ${bt[this._openFilter] || this._openFilter}
                  `}
...
M([
  h({ type: Number })
], A.prototype, "totalCount", 2);
M([
  h({ type: Number })
], A.prototype, "totalFolderCount", 2);
M([
  h({ type: Boolean })
], A.prototype, "isLoading", 2);
M([
  h()
], A.prototype, "sortBy", 2);
M([
  h()
], A.prototype, "sortDirection", 2);
M([
  h({ type: Array })
], A.prototype, "sortOptions", 2);
M([
  h({ type: Object })
], A.prototype, "filters", 2);
M([
  h({ type: Array })
], A.prototype, "labels", 2);
M([
  h({ type: Array })
], A.prototype, "metadataFields", 2);
M([
  h({ type: Array })
], A.prototype, "pinnedFilters", 2);
M([
  Ze("ap-dropdown")
], A.prototype, "_sortDropdown", 2);
M([
  b()
], A.prototype, "_showDropdown", 2);
M([
  b()
], A.prototype, "_openFilter", 2);
M([
  b()
], A.prototype, "_openMetadataField", 2);
M([
  b()
], A.prototype, "_externalTrigger", 2);
M([
  b()
], A.prototype, "_externalLeft", 2);
A = M([
  $("ap-content-toolbar")
], A);
var Va = Object.defineProperty, Ba = Object.getOwnPropertyDescriptor, ie = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? Ba(e, s) : e, r = t.length - 1, o; r >= 0; r--)
    (o = t[r]) && (a = (i ? o(e, s, a) : o(a)) || a);
  return i && a && Va(e, s, a), a;
};
let H = class extends y {
  constructor() {
    super(...arguments), this.appliedFilters = {}, this.appliedMetadata = {}, this.metadataFields = [], this.pinnedFilters = [], this.pinnedMetadataFields = [], this.activeFilter = null, this.activeMetadataField = null;
  }
  _mapTypeLabel(t) {
    var e;
    return ((e = Xt.find((s) => s.value === t)) == null ? void 0 : e.label) || t;
  }
  _getSizeSummary(t) {
    if (t.values.length === 0) return "";
    const e = t.values[0], [s, i] = e.split(".."), a = s ? parseFloat(s) : null, r = i ? parseFloat(i) : null, o = (l) => l >= 1e3 ? `${(l / 1e3).toFixed(l % 1e3 === 0 ? 0 : 1)} GB` : `${l} MB`;
  ...
  _getFilterSummary(t, e) {
    if (t.type === "string") {
      ...
      if (e === m.SIZE) return this._getSizeSummary(s);
      const i = e === m.TYPE ? (a) => this._mapTypeLabel(a) : (a) => a;
  ...
  _getDateSummary(t) {
    var o, l;
    const e = ((o = Wt.find((c) => c.value === t.field)) == null ? void 0 : o.label) || t.field, s = [...yt, ...Qt], i = t.preset ? ((l = s.find((c) => c.value === t.preset)) == null ? void 0 : l.label) || t.preset : "", a = (c) => {
      try {
        return c.split("T")[0];
      } catch {
        return c;
    ...
    switch (t.kind) {
      case "preset":
        ...
        break;
      case "before":
        r = t.to ? `Before ${a(t.to)}` : i;
        break;
      case "after":
        r = t.from ? `After ${a(t.from)}` : i;
        break;
      case "between":
        t.from && t.to ? r = `${a(t.from)} – ${a(t.to)}` : t.from ? r = `After ${a(t.from)}` : t.to && (r = `Before ${a(t.to)}`);
        break;
      case "specific":
        r = t.from ? a(t.from) : "";
        break;
  ...
  render() {
    const t = Object.entries(this.appliedFilters), e = Object.entries(this.appliedMetadata), s = this.pinnedFilters.filter(
      (r) => !(r in this.appliedFilters)
    ), i = this.pinnedMetadataFields.filter(
      (r) => !(r in this.appliedMetadata)
    );
    return t.length + e.length + s.length + i.length === 0 ? v : n`
      <div class="chips-row">
        <div class="chips">
          ${s.map((r) => n`
            <span class="chip pinned-empty ${r === this.activeFilter ? "active" : ""}" @click=${(o) => this._openFilter(r, o)}>
              <span class="chip-label">${bt[r] || r}</span>
            </span>
          `)}
          ${i.map((r) => {
      const o = this._getMetadataLabel(r);
      return n`
          ...
          ${t.map(([r, o]) => {
      const l = this._getFilterSummary(o, r), c = o.type === "date";
      return n`
              <span class="chip ${r === this.activeFilter ? "active" : ""}" @click=${(d) => this._openFilter(r, d)}>
                ${(c || r === m.TYPE || r === m.SIZE) && l ? n`<span class="chip-label">${l}</span>` : n`
                      <span class="chip-label">${bt[r] || r}</span>
                      ${l ? n`<span class="chip-summary">${l}</span>` : v}
                    `}
                <button class="chip-remove" @click=${(d) => {
        d.stopPropagation(), this._removeFilter(r);
      }} title="Remove filter">
                  <ap-icon name="close" .size=${12}></ap-icon>
                </button>
              </span>
          ...
          ${e.map(([r, o]) => {
      const l = this._getFilterSummary(o), c = this._getMetadataLabel(r);
      return n`
              <span class="chip ${r === this.activeMetadataField ? "active" : ""}" @click=${(d) => this._openMetadataFilter(r, d)}>
                <span class="chip-label">${c}</span>
                ${l ? n`<span class="chip-summary">${l}</span>` : v}
                <button class="chip-remove" @click=${(d) => {
        d.stopPropagation(), this._removeMetadataFilter(r);
      }} title="Remove filter">
                  <ap-icon name="close" .size=${12}></ap-icon>
                </button>
              </span>
        ...
        ${t.length + e.length > 1 ? n`<button class="clear-all" @click=${this._clearAll}>Clear all</button>` : v}
      </div>
    `;
  }
};
...
ie([
  h({ type: Object })
], H.prototype, "appliedFilters", 2);
ie([
  h({ type: Object })
], H.prototype, "appliedMetadata", 2);
ie([
  h({ type: Array })
], H.prototype, "metadataFields", 2);
ie([
  h({ type: Array })
], H.prototype, "pinnedFilters", 2);
ie([
  h({ type: Array })
], H.prototype, "pinnedMetadataFields", 2);
ie([
  h()
], H.prototype, "activeFilter", 2);
ie([
  h()
], H.prototype, "activeMetadataField", 2);
H = ie([
  $("ap-filters-bar")
], H);
var Ua = Object.defineProperty, qa = Object.getOwnPropertyDescriptor, $e = (t, e, s, i) => {
  ...
  render() {
    return n`
      ...
      (t) => n`
        ...
        ${this.assets.map(
      (t, e) => {
        const s = e >= this._prevCount ? Math.min(e - this._prevCount, 20) : Math.min(e, 20);
        return n`
              <ap-asset-card
                .asset=${t}
                .index=${e}
        ...
        ${this.isLoading ? Array.from({ length: Ka }, () => n`<div class="ghost-card"></div>`) : v}
      </div>
    `;
  }
};
...
var Ga = Object.defineProperty, Ha = Object.getOwnPropertyDescriptor, Ie = (t, e, s, i) => {
  ...
  render() {
    return n`
      ...
      (t, e) => n`
        ...
        ${this.isLoading ? Array.from({ length: Ya }, () => n`<div class="ghost-row"></div>`) : v}
      </div>
    `;
  }
};
...
Ie([
  h({ type: Array })
], de.prototype, "assets", 2);
Ie([
  h({ type: Array })
], de.prototype, "folders", 2);
Ie([
  h({ type: Array })
], de.prototype, "selectedIds", 2);
Ie([
  h({ type: Boolean })
], de.prototype, "isLoading", 2);
de = Ie([
  $("ap-list-view")
], de);
function Ae(t) {
...
function it(t) {
  var s, i;
  let e = Oe(((s = t.url) == null ? void 0 : s.cdn) || ((i = t.url) == null ? void 0 : i.public) || "");
  return e ? (e = e.replace(/([?&])func=proxy&?/, "$1").replace(/[?&]$/, ""), is(e, {
    w: "200",
...
const qt = /^(https:\/\/)([a-z0-9_-]+)\.filerobot\.com\//i;
function Oe(t, e) {
  if (!t) return "";
  ...
  if (s) {
    const i = s[2];
    return t.replace(qt, `$1assets.filerobot.com/${i}/`);
  }
  if (e)
    try {
      const i = new URL(t);
...
const Wa = /* @__PURE__ */ new Set(["jpg", "jpeg", "png", "gif", "webp", "svg", "bmp", "ico", "tiff", "tif", "avif", "heic", "heif"]), Qa = /* @__PURE__ */ new Set([
  ...Wa,
  "pdf",
  "psd",
  "eps",
...
const rs = "https://assets.filerobot.com/scaleflex/assets/file-types/v2/", Za = {
  png: "PNG",
  jpg: "JPG",
  jpeg: "JPG",
  gif: "GIF",
...
function Le(t) {
  const e = (t == null ? void 0 : t.toLowerCase().replace(".", "")) || "", s = Za[e];
  return `${rs}${s || "DEFAULT"}.png`;
}
function pe() {
  return `${rs}DEFAULT.png`;
}
function mt(t) {
  if (!t) return "";
  const e = t.split("/");
...
const Ja = /* @__PURE__ */ new Set(["png", "svg", "webp", "gif", "avif"]);
function rt(t) {
  const e = (t == null ? void 0 : t.toLowerCase().replace(".", "")) || "";
  return Ja.has(e);
}
var ei = Object.defineProperty, ti = Object.getOwnPropertyDescriptor, ot = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? ti(e, s) : e, r = t.length - 1, o; r >= 0; r--)
    (o = t[r]) && (a = (i ? o(e, s, a) : o(a)) || a);
...
let _e = class extends y {
  ...
  render() {
    ...
    const t = this.asset;
    if (!t) return n``;
    const s = ce(t.type) === "video", i = ((c = t.extension) == null ? void 0 : c.toLowerCase()) === "pdf", a = Et(t.extension || "");
    let r = "";
    s ? r = St(t) : i ? r = it(t) : a && (r = Ae(t));
    const o = Le(t.extension || ""), l = rt(t.extension || "");
    return n`
      <div class="card" @click=${this._handleSelect}>
        <div class="thumbnail ${l && r ? "chess" : ""}">
          ${r ? n`<img
                ...
                @error=${(f) => {
      const p = f.target, g = pe();
      p.src !== o && p.src !== g ? (p.src = o, p.classList.add("icon-fallback")) : p.src !== g && (p.src = g);
    }}
              />` : n`<img
                src=${o}
                alt=${t.name}
      ...
      p.src !== g && (p.src = g);
        ...
        <div class="info">
          ...
          <div class="meta">${(d = t.extension) == null ? void 0 : d.toUpperCase()} · ${Ct(((u = t.size) == null ? void 0 : u.bytes) || 0)}</div>
        </div>
      </div>
    `;
  }
...
var si = Object.defineProperty, ai = Object.getOwnPropertyDescriptor, nt = (t, e, s, i) => {
  ...
  render() {
    ...
    return n`
      ...
      p.src !== g && (p.src = g);
        ...
        <div class="desc">${Ct(((u = t.size) == null ? void 0 : u.bytes) || 0)}</div>
        <div class="date">${Ue(t.created_at || "")}</div>
        <div class="actions">
...
var ii = Object.defineProperty, ri = Object.getOwnPropertyDescriptor, kt = (t, e, s, i) => {
...
let Pe = class extends y {
   ...
   */
  _getPreviewUrl(t) {
    const e = ce(t.file_type), s = t.file_uri_cdn;
    if (!s) return "";
    if (e === "video")
      try {
        const i = new URL(s);
        return i.searchParams.set("w", "200"), i.searchParams.set("force_format", "webp,jpeg"), i.toString();
      } catch {
        return s;
      }
    if (t.file_type === "application/pdf" || mt(t.file_type).toLowerCase() === "pdf")
      try {
        let i = s.replace(/([?&])func=proxy&?/, "$1").replace(/[?&]$/, "");
        const a = new URL(i);
        return a.searchParams.set("w", "200"), a.searchParams.set("force_format", "webp,jpeg"), a.searchParams.set("doc_page", "1"), a.searchParams.set("bypass_process_proxy", "1"), a.toString();
      } catch {
        return s;
      }
    return s;
  ...
  _renderPreviewImg(t) {
    const e = this._isAlternativeDisplay(t.file_type), s = this._getPreviewUrl(t), i = Le(mt(t.file_type)), a = pe();
    return n`<div class="preview-slot ${e ? "icon-bg" : ""}">
      <img
        ...
        loading="lazy"
        @error=${(r) => {
      const o = r.target, l = o.parentElement;
      o.src !== i && o.src !== a ? (o.src = i, o.classList.add("icon-mode"), l == null || l.classList.add("icon-bg")) : o.src !== a && (o.src = a, o.classList.add("icon-mode"), l == null || l.classList.add("icon-bg"));
    }}
        @load=${(r) => {
      const o = r.target, l = o.parentElement;
      o.src.includes("scaleflex/assets/file-types") && (o.classList.add("icon-mode"), l == null || l.classList.add("icon-bg"));
    }}
      />
    </div>`;
  }
...
var oi = Object.defineProperty, ni = Object.getOwnPropertyDescriptor, os = (t, e, s, i) => {
  ...
  render() {
    ...
    return t ? n`
      <div class="row" @click=${this._handleOpen}>
        ...
        <div class="meta">${Ue(t.modified_at || t.updated_at || "")}</div>
        <div></div>
      </div>
    ` : n``;
  }
...
var li = Object.defineProperty, ci = Object.getOwnPropertyDescriptor, ns = (t, e, s, i) => {
...
var pi = Object.defineProperty, hi = Object.getOwnPropertyDescriptor, z = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? hi(e, s) : e, r = t.length - 1, o; r >= 0; r--)
    (o = t[r]) && (a = (i ? o(e, s, a) : o(a)) || a);
  return i && a && pi(e, s, a), a;
};
let D = class extends y {
  constructor() {
    super(...arguments), this.asset = null, this.assets = [], this.selectedIds = [], this.containerToken = "", this.showMetadata = !0, this.metadataFields = [], this.regionalFilters = {}, this._isFullscreen = !1, this._fsImageLoaded = !1, this._previewLoading = !1, this._openSections = /* @__PURE__ */ new Set(["file-info"]), this._hls = null;
  }
  connectedCallback() {
    ...
    if (s) {
      ...
      try {
        ...
        this._hls = o, o.loadSource(e), o.attachMedia(s), o.on(r.Events.ERROR, (l, c) => {
          var d;
          c.fatal && (this._destroyHls(), s.src = ((d = t.url) == null ? void 0 : d.cdn) || "");
        });
      } catch {
      }
    }
  ...
  _getFullscreenImageUrl(t) {
    ...
    if (!e) return Ae(t) || "";
    const s = Oe(e, this.containerToken);
    return is(s, {
  ...
  _getBlurDimensions(t) {
    var o, l;
    const e = (o = t.info) == null ? void 0 : o.img_w, s = (l = t.info) == null ? void 0 : l.img_h;
  ...
  _getFileInfoRows(t) {
    var s, i, a, r, o;
    const e = [];
    return e.push({ label: "File name", value: t.name }), t.extension && e.push({ label: "Type", value: t.extension.toUpperCase() }), (s = t.size) != null && s.bytes && e.push({ label: "Size", value: Ct(t.size.bytes) }), (i = t.info) != null && i.img_w && e.push({ label: "Dimensions", value: Xa(t.info.img_w, t.info.img_h) }), (a = t.info) != null && a.video_w && e.push({ label: "Video resolution", value: `${t.info.video_w} x ${t.info.video_h}` }), (r = t.info) != null && r.duration && e.push({ label: "Duration", value: `${Math.round(t.info.duration)}s` }), (o = t.info) != null && o.color_space && e.push({ label: "Color space", value: t.info.color_space }), t.created_at && e.push({ label: "Created", value: Ue(t.created_at) }), t.modified_at && e.push({ label: "Modified", value: Ue(t.modified_at) }), e;
  }
  /**
  ...
  _getAllMetadataRows(t) {
    ...
    if (this.metadataFields.length > 0) {
      ...
      for (const u of this.metadataFields) {
        const f = d[u.key] ?? ((o = (r = t.info) == null ? void 0 : r.metadata) == null ? void 0 : o[u.key]) ?? ((l = t.meta) == null ? void 0 : l[u.key]), p = this._resolveFieldValue(f, u);
        if (!p) continue;
        i.add(u.key), u.slug && i.add(u.slug);
        const g = p.length > 50;
        e.push({ label: u.label, value: p, wrap: g });
      }
    }
    if (t.meta)
      for (const [d, u] of Object.entries(t.meta)) {
        if (i.has(d) || u == null || u === "") continue;
        const f = s.get(d);
        if (f) {
          ...
          i.add(d);
          const g = p.length > 50;
          e.push({ label: f.label, value: p, wrap: g });
        } else {
          const p = typeof u == "object" ? JSON.stringify(u) : String(u);
          if (!p || p === "{}" || p === "[]") continue;
          i.add(d);
          const g = d.charAt(0).toUpperCase() + d.slice(1).replace(/_/g, " "), C = p.length > 50;
          e.push({ label: g, value: p, wrap: C });
        }
      }
  ...
  _renderTags(t) {
    ...
    const s = this._openSections.has("tags");
    return n`
      <div class="accordion">
        <button
          class="accordion-trigger"
  ...
  render() {
    ...
    const t = this.asset;
    if (!t) return v;
    const e = ce(t.type), s = e === "image", i = e === "video", a = e === "audio", r = ((k = t.extension) == null ? void 0 : k.toLowerCase()) === "pdf", o = Le(t.extension || ""), l = rt(t.extension || ""), c = Ae(t), d = s ? c || Oe(((F = t.url) == null ? void 0 : F.cdn) || "", this.containerToken) : "", u = s ? this._getFullscreenImageUrl(t) : "", f = s ? this._getBlurDimensions(t) : null, p = i && ((R = t.url) == null ? void 0 : R.cdn) || "", g = i && (((Re = t.info) == null ? void 0 : Re.video_thumbnail) || ((K = t.info) == null ? void 0 : K.preview)) || "", C = r && !s ? it(t) : "", O = this._getCurrentIndex(), I = O > 0 && this._findNavTarget(O, -1) >= 0, N = O >= 0 && this._findNavTarget(O, 1) >= 0, E = this.assets.length > 1;
    return n`
      <div class="panel">
        ...
        <div class="preview-area ${s && l ? "chess" : ""}">
          ${E ? n`
            ...
            <button class="nav-btn next" @click=${this._next} ?disabled=${!N} aria-label="Next">
          ...
          ${s ? n`
            <img
              ...
              alt=${t.name}
              @load=${(Se) => {
      Se.target.classList.remove("icon-fallback"), this._previewLoading = !1;
    }}
              @error=${(Se) => {
      const re = Se.target;
      this._previewLoading = !1, re.src !== o && (re.src = o, re.classList.add("icon-fallback"));
    }}
            />
          ...
          ` : v}
          ${i ? n`<video src=${p} poster=${g || v} controls @loadeddata=${() => {
      this._previewLoading = !1;
    }} @error=${() => {
      this._previewLoading = !1;
    }}></video>` : v}
          ${a ? n`<audio src=${((Ot = t.url) == null ? void 0 : Ot.cdn) || ""} controls></audio>` : v}
          ${r && !s ? n`
            <img
              src=${C || o}
              alt=${t.name}
              class=${C ? "" : "icon-fallback"}
              @error=${(Se) => {
      const re = Se.target;
      re.src !== o && (re.src = o, re.classList.add("icon-fallback"));
    }}
            />
          ` : v}
          ${!s && !i && !a && !r ? n`<img src=${o} alt=${t.name} class="icon-fallback" />` : v}
        </div>
...
var ui = Object.defineProperty, fi = Object.getOwnPropertyDescriptor, ls = (t, e, s, i) => {
...
let Ge = class extends y {
  ...
  render() {
    return this.selectedAssets.length === 0 ? v : n`
      <div class="bar">
      ...
      return n`
              <div class="thumb ${l && r ? "chess" : ""}" @click=${() => this._deselect(t.uuid)} title="Click to deselect ${t.name}">
                ${r ? n`<img
                      ...
                      alt=${t.name}
                      @error=${(d) => {
        const u = d.target, f = pe();
        u.src !== o && u.src !== f ? u.src = o : u.src !== f && (u.src = f);
      }}
                    />` : n`<img
                      src=${o}
                      alt=${t.name}
...
ls([
  h({ type: Array })
], Ge.prototype, "selectedAssets", 2);
Ge = ls([
  $("ap-selection-bar")
], Ge);
var vi = Object.defineProperty, gi = Object.getOwnPropertyDescriptor, Ft = (t, e, s, i) => {
...
var mi = Object.defineProperty, bi = Object.getOwnPropertyDescriptor, Tt = (t, e, s, i) => {
...
let De = class extends y {
  ...
  render() {
    const t = _i[this.name];
    if (!t) return v;
    const e = typeof t == "string" ? ze`<path d=${t}></path>` : t.map((s) => {
      const { tag: i, attrs: a } = s;
      return i === "circle" ? ze`<circle cx=${a.cx} cy=${a.cy} r=${a.r} fill=${a.fill ?? "none"}></circle>` : i === "rect" ? ze`<rect width=${a.width} height=${a.height} x=${a.x} y=${a.y} rx=${a.rx ?? "0"} ry=${a.ry ?? "0"}></rect>` : ze`<path d=${a.d}></path>`;
    });
    return n`
      <svg
...
var yi = Object.defineProperty, xi = Object.getOwnPropertyDescriptor, cs = (t, e, s, i) => {
...
var $i = Object.defineProperty, wi = Object.getOwnPropertyDescriptor, ds = (t, e, s, i) => {
...
var Si = Object.defineProperty, Ci = Object.getOwnPropertyDescriptor, ps = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? Ci(e, s) : e, r = t.length - 1, o; r >= 0; r--)
    (o = t[r]) && (a = (i ? o(e, s, a) : o(a)) || a);
  return i && a && Si(e, s, a), a;
};
...
var Ei = Object.defineProperty, ki = Object.getOwnPropertyDescriptor, V = (t, e, s, i) => {
...
let L = class extends y {
  ...
  _updateMenuPosition() {
    const t = this.getBoundingClientRect(), e = window.innerHeight - t.bottom, s = t.top, i = Math.min(this.options.length * 36 + 8, 300);
...
V([
  b()
], L.prototype, "_focusedIndex", 2);
V([
  b()
], L.prototype, "_menuPosition", 2);
V([
  b()
], L.prototype, "_menuAlign", 2);
V([
  b()
], L.prototype, "_menuTop", 2);
V([
  b()
], L.prototype, "_menuBottom", 2);
V([
  b()
], L.prototype, "_menuLeft", 2);
V([
  b()
], L.prototype, "_menuRight", 2);
L = V([
  $("ap-dropdown")
], L);
var Fi = Object.defineProperty, Ti = Object.getOwnPropertyDescriptor, hs = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? Ti(e, s) : e, r = t.length - 1, o; r >= 0; r--)
    (o = t[r]) && (a = (i ? o(e, s, a) : o(a)) || a);
...
var Ai = Object.defineProperty, Oi = Object.getOwnPropertyDescriptor, us = (t, e, s, i) => {
...
var Pi = Object.defineProperty, Mi = Object.getOwnPropertyDescriptor, we = (t, e, s, i) => {
...
let te = class extends y {
  ...
  _select(t) {
    this.disabled || t.disabled || this.dispatchEvent(new CustomEvent("ap-change", {
...
we([
  h({ reflect: !0 })
], te.prototype, "direction", 2);
we([
  h({ reflect: !0 })
], te.prototype, "columns", 2);
te = we([
  $("ap-radio-group")
], te);
var Di = Object.defineProperty, Ii = (t, e, s, i) => {
...
const At = class At extends y {
  constructor() {
    super(), this._initFailed = !1, this._loadId = 0, this._loadMoreId = 0, this._loadDataTimer = null, this.store = vs(), this.storeCtrl = new gs(this, this.store), this.selectionCtrl = new ms(this, this.store), this.infiniteScrollCtrl = new bs(this, () => this._loadMore()), this.marqueeCtrl = new _s(this, this.store);
  }
  connectedCallback() {
  ...
  async _doInit(e) {
    const s = ia(), i = s.sortBy ?? e.defaultSortBy ?? "created_at", a = s.sortDirection ?? e.defaultSortDirection ?? "desc";
    this.store.setState({
      config: e,
    ...
    }), this.apiClient = new ys(e.auth, e.apiBase);
    try {
      if (e.auth.mode === "securityTemplate") {
        const u = await Cs(this.apiClient);
        this.apiClient.setSassKey(u), this.store.setState({ sassKey: u });
      }
      ...
      if (r.status === "fulfilled") {
        const { fields: u, regionalVariantGroups: f, regionalFilters: p, brandColor: g } = r.value;
        this.store.setState({
          metadataFields: u,
          regionalVariantGroups: f,
          regionalFilters: p,
          ...g ? { brandColor: g } : {}
        });
      }
      o.status === "fulfilled" && this.store.setState({ labels: o.value.labels || [] });
      const { pinnedFilters: l, pinnedMetadata: c } = ta(e.auth.projectToken ?? null), d = this.store.getState().filters;
      this.store.setState({
        filters: {
          ...d,
          pinned: l,
  ...
  async open() {
    ...
    this.store.setState({
      ...
      assets: [],
      folders: [],
      currentFolder: ((s = this.config) == null ? void 0 : s.rootFolderUuid) ?? null,
      currentFolderPath: "/",
      breadcrumb: [],
      selectedAssets: /* @__PURE__ */ new Map(),
      folderPreviews: {},
  ...
  async _loadData() {
    ...
    try {
      const g = s.activeTab;
      if (g === "assets") {
        const C = this._buildSearchNotation(), O = await lt(this.apiClient, {
          folder: s.currentFolderPath || "/",
          offset: 0,
          limit: s.limit,
          sort_by: s.sortBy,
        ...
        const I = ((i = O.files) == null ? void 0 : i.length) ?? 0, N = I >= s.limit, E = ((r = (a = O.base) == null ? void 0 : a.count) == null ? void 0 : r.files_recursive) ?? ((l = (o = O.base) == null ? void 0 : o.count) == null ? void 0 : l.files_direct) ?? 0, k = N ? E : I;
        this.store.setState({
          assets: O.files || [],
          folders: [],
          totalCount: k,
        ...
        const C = this._buildSearchNotation(), [O, I] = await Promise.all([
          $s(this.apiClient, {
        ...
        if (N.length > 0)
          try {
            E = await ws(this.apiClient, N.map((k) => k.uuid));
          } catch {
          }
        if (e !== this._loadId) return;
        this.store.setState({
          ...
          folders: N,
          folderPreviews: E,
          totalCount: (((c = I.files) == null ? void 0 : c.length) ?? 0) >= s.limit ? ((u = (d = I.base) == null ? void 0 : d.count) == null ? void 0 : u.files_direct) ?? 0 : ((f = I.files) == null ? void 0 : f.length) ?? 0,
          totalFolderCount: O.total ?? N.length,
          offset: 0,
          hasMore: (((p = I.files) == null ? void 0 : p.length) ?? 0) >= s.limit,
          isLoading: !1
        });
      }
    } catch (g) {
      ...
      this.store.setState({ isLoading: !1 }), this.dispatchEvent(new CustomEvent("ap-error", {
        detail: { error: g, context: "loadData" },
        bubbles: !0,
        composed: !0
      }));
    }
  ...
  async _loadMore() {
    ...
    try {
      const a = this._buildSearchNotation(), r = await lt(this.apiClient, {
      ...
      const o = this.store.getState().assets, l = r.files || [], c = l.length >= e.limit, d = [...o, ...l];
      this.store.setState({
        assets: d,
        offset: i,
        hasMore: c,
  ...
  _handleCancel(e) {
    ...
    this.close(), (a = (i = this.config) == null ? void 0 : i.onCancel) == null || a.call(i), this.dispatchEvent(new CustomEvent("ap-cancel", {
      detail: { reason: e },
      bubbles: !0,
      composed: !0
    }));
  ...
  _handleSortChange(e) {
    ...
    this.store.setState({
      ...
      assets: [],
      folders: []
    }), Ut(s, i.sortDirection), this.selectionCtrl.resetRange(), this._loadData();
  }
  _handleSortDirectionChange(e) {
    ...
    this.store.setState({
      ...
      assets: [],
      folders: []
    }), Ut(i.sortBy, s), this.selectionCtrl.resetRange(), this._loadData();
  }
  _handleTabChange(e) {
    this.store.setState({
  ...
  _handleQuickSelect(e) {
    ...
    (a = (i = this.config) == null ? void 0 : i.onSelect) == null || a.call(i, [s]), this.dispatchEvent(new CustomEvent("ap-select", {
      detail: { assets: [s] },
      bubbles: !0,
      composed: !0
    })), this.close();
  ...
  _handleBreadcrumbNavigate(e) {
    const s = e.detail.uuid, i = this.store.getState(), a = s ? i.breadcrumb.findIndex((l) => l.uuid === s) : -1, r = s ? i.breadcrumb.slice(0, a + 1) : [], o = r.length > 0 ? r[r.length - 1].path : "/";
    this.store.setState({
  ...
  // ── Filter Handlers ──────────────────────────────────────────────────
  _isDateFilterKey(e) {
    return e === m.DATE || e === m.LICENSE_EXPIRY || e === j.DUE_DATE;
  }
  _handleFilterUpdate(e) {
    ...
    if (!l && this._isDateFilterKey(s) && typeof i == "object" && !Array.isArray(i)) {
      const { kind: c, preset: d, from: u, to: f } = i;
      l = !c && !d && !u && !f;
    }
    if (l) {
      const c = { ...o.applied };
      delete c[s], o.applied = c, o.pinned.includes(s) || (o.visible = o.visible.filter((d) => d !== s));
    } else {
      const c = { ...o.applied };
      this._isDateFilterKey(s) && typeof i == "object" && !Array.isArray(i) ? c[s] = {
        ...
        from: i.from || null,
        to: i.to || null
      } : s === m.IMAGE && typeof i == "object" && !Array.isArray(i) ? c[s] = i : c[s] = {
        type: "string",
        values: Array.isArray(i) ? i : [i],
        operator: a || ":"
      }, o.applied = c, o.visible.includes(s) || (o.visible = [...o.visible, s]);
    }
    this.store.setState({ filters: o, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._debouncedLoadData();
  }
  _handleFilterOpen(e) {
  ...
  _handleFilterPanelChange(e) {
    ...
    s && (s.activeFilter = e.detail.key, s.activeMetadataField = e.detail.metadataFieldKey || null);
  }
  _handleFilterRemove(e) {
    const s = e.detail.key, a = { ...this.store.getState().filters }, r = { ...a.applied };
    delete r[s], a.applied = r, a.pinned.includes(s) || (a.visible = a.visible.filter((o) => o !== s)), this.store.setState({ filters: a, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._debouncedLoadData();
  }
  _handleMetadataFilterChange(e) {
    const { fieldKey: s, operator: i, values: a, metadataType: r } = e.detail, l = { ...this.store.getState().filters }, c = { ...l.metadata }, d = { ...c.applied };
    !a || Array.isArray(a) && a.length === 0 ? (delete d[s], c.applied = d, c.pinned.includes(s) || (c.visible = c.visible.filter((f) => f !== s))) : (d[s] = {
      type: "string",
      values: Array.isArray(a) ? a : [a],
      operator: i || ":",
      metadataType: r
    }, c.applied = d, c.visible.includes(s) || (c.visible = [...c.visible, s])), l.metadata = c, this.store.setState({ filters: l, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._debouncedLoadData();
  }
  _handleMetadataFieldToggle(e) {
    const { fieldKey: s, visible: i } = e.detail, r = { ...this.store.getState().filters }, o = { ...r.metadata };
    i ? o.visible.includes(s) || (o.visible = [...o.visible, s]) : o.visible = o.visible.filter((l) => l !== s), r.metadata = o, this.store.setState({ filters: r });
  }
  _handleMetadataFilterRemove(e) {
    const { fieldKey: s } = e.detail, a = { ...this.store.getState().filters }, r = { ...a.metadata }, o = { ...r.applied };
    delete o[s], r.applied = o, r.pinned.includes(s) || (r.visible = r.visible.filter((l) => l !== s)), a.metadata = r, this.store.setState({ filters: a, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._debouncedLoadData();
  }
  _handleFilterPin(e) {
    var l, c;
    const { key: s, pinned: i } = e.detail, r = { ...this.store.getState().filters };
    i ? r.pinned.includes(s) || (r.pinned = [...r.pinned, s]) : (r.pinned = r.pinned.filter((d) => d !== s), s in r.applied || (r.visible = r.visible.filter((d) => d !== s))), this.store.setState({ filters: r });
  ...
  _handleFiltersSet(e) {
    const { applied: s, metadata: i } = e.detail, r = { ...this.store.getState().filters };
    r.applied = s;
    const o = Object.keys(s);
    if (r.visible = [.../* @__PURE__ */ new Set([...r.pinned, ...o])], i) {
      const l = { ...r.metadata };
      l.applied = i.applied;
      const c = Object.keys(i.applied);
      l.visible = [.../* @__PURE__ */ new Set([...l.pinned, ...c])], r.metadata = l;
    }
    this.store.setState({ filters: r, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._debouncedLoadData();
  }
  // ── Sort Options ────────────────────────────────────────────────────
  _getSortOptions() {
    const e = this.storeCtrl.state;
    return e.searchQuery ? ra : e.activeTab === "folders" ? oa : xt;
  }
  // ── Search Notation Builder ──────────────────────────────────────────
  _buildSearchNotation() {
    const e = this.store.getState();
    return qs(e.filters.applied, e.filters.metadata.applied).join(" ");
  }
  render() {
    var a, r;
    const e = this.storeCtrl.state, s = Array.from(e.selectedAssets.keys()), i = this.selectionCtrl.getSelectedAssets();
    return n`
      <ap-modal
        ?open=${e.isOpen}
      ...
      >
        ...
        <div class="content-area">
          <div class="main-content">
            ...
            ${e.isLoading && e.assets.length === 0 && e.folders.length === 0 ? n`<ap-skeleton .variant=${e.viewMode}></ap-skeleton>` : this._renderContent(e, s)}
