import { css as b, LitElement as _, html as n, nothing as m, svg as Ae } from "lit";
import { property as u, query as $e, customElement as y, state as E } from "lit/decorators.js";
class Qt {
  constructor(e) {
  ...
  setState(e) {
    ...
    const a = this.state;
    this.state = { ...a, ...e }, this._notifying = !0;
    try {
      this.listeners.forEach((r) => r(this.state, a));
    } finally {
      this._notifying = !1;
    }
...
class Zt {
  constructor(e, a) {
    this.host = e, this.store = a, e.addController(this);
  }
  get state() {
...
class Jt {
  constructor(e, a) {
    this.lastClickedIndex = -1, this.host = e, this.store = a, e.addController(this);
  }
  hostConnected() {
  }
  hostDisconnected() {
  ...
  handleSelect(e, a, r) {
    var c, d;
    const s = this.store.getState(), o = new Map(s.selectedAssets);
    if (!(((c = s.config) == null ? void 0 : c.multiSelect) ?? !0)) {
      o.clear(), o.set(e.uuid, e), this.store.setState({ selectedAssets: o }), this.lastClickedIndex = a;
      return;
    }
    if (r.shiftKey && this.lastClickedIndex >= 0) {
      const p = Math.min(this.lastClickedIndex, a), h = Math.max(this.lastClickedIndex, a), f = s.assets;
      for (let v = p; v <= h; v++)
        f[v] && o.set(f[v].uuid, f[v]);
    } else r.metaKey || r.ctrlKey ? o.has(e.uuid) ? o.delete(e.uuid) : o.set(e.uuid, e) : (o.clear(), o.set(e.uuid, e));
    const l = (d = s.config) == null ? void 0 : d.maxSelections;
    l && o.size > l || (this.lastClickedIndex = a, this.store.setState({ selectedAssets: o }));
  }
  isSelected(e) {
  ...
  clearSelection() {
    this.store.setState({ selectedAssets: /* @__PURE__ */ new Map() }), this.lastClickedIndex = -1;
  }
  getSelectedAssets() {
...
class ea {
  constructor(e, a) {
    this.host = e, this.onLoadMore = a, e.addController(this);
  }
  hostConnected() {
  }
  hostDisconnected() {
  ...
  observe(e, a) {
    this.disconnect(), this.sentinel = e, this.observer = new IntersectionObserver(
      (r) => {
        var s;
        (s = r[0]) != null && s.isIntersecting && this.onLoadMore();
      },
      { root: a ?? null, rootMargin: "200px" }
...
class ta {
  ...
  isInteractiveTarget(e) {
    return e.composedPath().some((r) => {
      if (!(r instanceof HTMLElement)) return !1;
      if (r instanceof HTMLButtonElement || r instanceof HTMLInputElement) return !0;
      const s = r.tagName.toLowerCase();
      return !!(s.startsWith("ap-asset-") || s.startsWith("ap-folder-") || r.classList.contains("check") || r.classList.contains("check-box"));
    });
  }
  onMouseDown(e) {
  ...
  onMouseMove(e) {
    if (this.container) {
      if (!this._dragging) {
        const a = Math.abs(e.clientX - this.startClientX), r = Math.abs(e.clientY - this.startClientY);
        if (a < bt && r < bt) return;
  ...
  updateMarqueeRect(e) {
    ...
    const a = this.container.getBoundingClientRect(), r = e.clientX - a.left + this.container.scrollLeft, s = e.clientY - a.top + this.container.scrollTop;
    this.rect = {
      x: Math.min(this.startX, r),
      y: Math.min(this.startY, s),
      width: Math.abs(r - this.startX),
      height: Math.abs(s - this.startY)
    }, this.host.requestUpdate(), this.selectIntersecting();
  }
  startAutoScroll(e) {
    if (!this.container) return;
    const a = this.container.getBoundingClientRect(), r = e.clientY - a.top, s = a.bottom - e.clientY, o = r < re || s < re;
  ...
  autoScrollTick() {
    ...
    const e = this.container.getBoundingClientRect(), a = this._lastMouseEvent, r = a.clientY - e.top, s = e.bottom - a.clientY;
    let o = 0;
    r < re ? o = -_t * (1 - r / re) : s < re && (o = _t * (1 - s / re)), o !== 0 && (this.container.scrollTop += o, this.updateMarqueeRect(a), this._scrollRAF = requestAnimationFrame(() => this.autoScrollTick()));
  }
  stopAutoScroll() {
  ...
  queryCards() {
    for (const e of Array.from(this.container.children))
      if (e.shadowRoot) {
        const a = e.shadowRoot.querySelectorAll("[data-asset-uuid]");
  ...
  selectIntersecting() {
    var s;
    if (!this.container) return;
    const e = this.queryCards(), a = new Map(this.preMarqueeSelection);
    e.forEach((o) => {
      const i = o.getBoundingClientRect(), l = this.container.getBoundingClientRect(), c = {
        x: i.left - l.left + this.container.scrollLeft,
        y: i.top - l.top + this.container.scrollTop,
        width: i.width,
        height: i.height
      }, d = this.rect.x < c.x + c.width && this.rect.x + this.rect.width > c.x && this.rect.y < c.y + c.height && this.rect.y + this.rect.height > c.y, p = o.dataset.assetUuid;
      if (d) {
        const h = this.store.getState().assets.find((f) => f.uuid === p);
        h && a.set(p, h);
      }
    });
    const r = (s = this.store.getState().config) == null ? void 0 : s.maxSelections;
    if (r && a.size > r) {
      const o = Array.from(a.entries()).slice(0, r);
      this.store.setState({ selectedAssets: new Map(o) });
    } else
      this.store.setState({ selectedAssets: a });
  }
}
class aa {
  constructor(e, a) {
    ...
    this.baseUrl = a || `https://api.filerobot.com/${r}/v5`;
  }
  setSassKey(e) {
    this.sassKey = e;
  }
  ...
  async request(e, a) {
    const r = new URL(`${this.baseUrl}${e}`);
    a && Object.entries(a).forEach(([l, c]) => {
      c != null && (Array.isArray(c) ? r.searchParams.set(l, c.join(",")) : r.searchParams.set(l, String(c)));
    });
    const s = {};
    if (this.auth.mode === "session") {
      const l = this.auth;
      s["X-Session-Token"] = l.sessionToken, s["X-Company-Token"] = l.companyToken, s["X-Project-Token"] = l.projectToken;
    } else {
      const l = this.auth;
      this.sassKey ? s["X-Filerobot-Key"] = this.sassKey : s["X-Filerobot-Key"] = l.securityTemplateKey;
    }
    const o = new AbortController(), i = setTimeout(() => o.abort(), 3e4);
    try {
      const l = await fetch(r.toString(), { headers: s, signal: o.signal });
      if (clearTimeout(i), !l.ok) {
        let d = `API error: ${l.status} ${l.statusText}`;
        try {
          const p = await l.json();
          p.msg && (d = `API error: ${l.status} - ${p.msg}`);
        } catch {
        }
        throw new Error(d);
      }
      ...
      throw clearTimeout(i), l instanceof DOMException && l.name === "AbortError" ? new Error("API request timed out") : l;
    }
  }
  async post(e, a) {
    const r = new URL(`${this.baseUrl}${e}`), s = {
...
function sa(t) {
  ...
  }, a = t.sort_by ?? "created_at", r = t.sort_direction ?? "desc", s = {
  ...
  e.sort = `${s[a] || a}:${r}`, t.folder && (e.folder = t.folder);
  const o = [];
  return t.search && o.push(t.search), t.q && o.push(t.q), o.length > 0 && (e.q = o.join(" ")), e;
}
async function Je(t, e) {
  const a = sa(e);
  return t.request("/files", a);
}
async function ra(t, e) {
  ...
  e != null && e.q && (a.q = e.q);
  const r = (e == null ? void 0 : e.sort_by) ?? "created_at", s = (e == null ? void 0 : e.sort_direction) ?? "asc", o = {
    created_at: "created_at",
    modified_at: "modified_at",
    name: "name"
  };
  return a.sort = `${o[r] || r}:${s}`, await t.request("/folders", a);
}
...
Object.fromEntries(
  Object.entries(it).map(([t, e]) => [e, t])
);
const Ot = Object.values(it), ve = {
  PRESET: "preset",
  AFTER: "after",
  BEFORE: "before",
  BETWEEN: "between",
  SPECIFIC: "specific"
}, ot = "empty", nt = "non-empty", Oe = "specific", yt = 10, xt = 10, wt = 10, Ft = "filters:pinned:", Pt = "metadata:pinned:", $t = [
  g.DATE,
  g.TYPE,
  g.METADATA
], Mt = $.IS, ca = "20", da = "1", pa = 0, ua = [
  { key: g.DATE, label: "Date", icon: "calendar-days", type: "date" },
  { key: g.TYPE, label: "Format", icon: "file-type", type: "string" },
  { key: g.SIZE, label: "Size", icon: "scaling", type: "string" },
  { key: g.LICENSE_EXPIRY, label: "License expiry", icon: "history", type: "date" },
  { key: g.TAGS, label: "Tags", icon: "hash", type: "string" },
  { key: g.LABELS, label: "Labels", icon: "tag", type: "string" },
  { key: g.PRODUCT_REF, label: "Products", icon: "box", type: "string" },
  { key: g.IMAGE, label: "Image", icon: "image", type: "string" },
  { key: g.COLOR, label: "Color", icon: "swatch-book", type: "string" }
], ha = {
  key: g.METADATA,
  label: "Metadata",
  icon: "file-text",
  ...
  [g.DATE]: "Date",
  [g.SIZE]: "Size",
  [g.LICENSE_EXPIRY]: "License expiry",
  [g.TAGS]: "Tags",
  [g.LABELS]: "Labels",
  [g.PRODUCT_REF]: "Products",
  [g.IMAGE]: "Image",
  [g.COLOR]: "Color",
  [g.METADATA]: "Metadata",
  [g.APPROVAL]: "Approval"
  ...
  { value: q.IMAGE, label: "Image", icon: "image" },
  { value: q.VIDEO, label: "Video", icon: "clapperboard" },
  { value: q.AUDIO, label: "Audio", icon: "music" },
  { value: q.DOCUMENT, label: "Document", icon: "document" },
  { value: q.ARCHIVE, label: "Archive", icon: "archive" },
  { value: q.DESIGN_TEMPLATE, label: "Design template", icon: "layout-template" },
  { value: q.FONTS, label: "Fonts", icon: "text" },
  { value: q.OTHER, label: "Other", icon: "layout-grid" }
], et = {
  RESOLUTION: 0,
  ORIENTATION: 1,
  FACES: 2
  ...
  { label: "Empty", value: ot },
  { label: "Not empty", value: nt }
], Ea = [
  ...Fe,
  { label: "Specific", value: Oe }
], ka = [
  { label: "Empty", value: ot },
  { label: "Not empty", value: nt },
...
async function Aa(t) {
  var A, L, P, z;
  const e = await t.request("/settings"), a = e.settings ?? e, r = (a == null ? void 0 : a.metadata) ?? e.metadata, o = ((r == null ? void 0 : r.model) ?? [])[pa], i = r == null ? void 0 : r.store, c = ((i == null ? void 0 : i.regional_variants_groups) ?? []).map((S) => ({
    uuid: S.uuid,
  ...
  })).filter((S) => S.variants.length > 1), d = {};
  for (const S of c)
    d[S.uuid] = ((A = S.variants[0]) == null ? void 0 : A.api_value) ?? "";
  const p = (a == null ? void 0 : a.project_branding) ?? ((L = e.settings) == null ? void 0 : L.project_branding), h = p == null ? void 0 : p.brandColor;
  if (!o) return { fields: [], regionalVariantGroups: c, regionalFilters: d, brandColor: h };
  const f = o.groups ?? [], v = [];
  for (const S of f) {
    const k = (S.fields ?? []).find(
      (C) => C.type === "select-one" || C.type === "multi-select"
    );
    if (k) {
      console.log("[ap-debug] raw select field:", JSON.stringify(k));
      break;
    }
  }
  for (const S of f) {
    const k = ((P = S.name) == null ? void 0 : P.toLowerCase()) === "products" || ((z = S.name) == null ? void 0 : z.toLowerCase()) === "product fields";
    for (const C of S.fields ?? []) {
      const se = C.possible_values, Ce = se == null ? void 0 : se.map((N) => ({
        api_value: N.api_value ?? N.value ?? "",
        internal_unique_value: N.internal_unique_value ?? N.api_value ?? "",
        label: N.label ?? N.name ?? N.api_value ?? ""
      }));
      v.push({
        ...
        ckey: C.ckey,
        possible_values: Ce,
        group: k ? "product" : "root",
        regionalVariantsGroupUuid: C.regional_variants_group_uuid
      });
    }
  }
  return { fields: v, regionalVariantGroups: c, regionalFilters: d, brandColor: h };
}
function Dt(t) {
  const e = new Date(t);
...
function Ta(t) {
  const e = Dt(t), a = e.getDay(), r = a === 0 ? 6 : a - 1;
...
function Lt(t) {
  ...
  switch (t) {
    case "today":
      return {
        from: Dt(e).toISOString(),
        to: e.toISOString()
      };
    case "last_week": {
      const a = Ta(e), r = new Date(a);
    ...
    case "last_month": {
      const a = Oa(e), r = new Date(a);
      return r.setMonth(r.getMonth() - 1), {
        from: r.toISOString(),
        to: a.toISOString()
      };
    }
    case "within_month": {
      const a = new Date(e);
      return a.setDate(a.getDate() - 30), {
        from: a.toISOString(),
        to: e.toISOString()
      };
    }
    case "last_year": {
      const a = Fa(e), r = new Date(a);
      return r.setFullYear(r.getFullYear() - 1), {
        from: r.toISOString(),
        to: a.toISOString()
      };
    }
    case "within_year": {
      const a = new Date(e);
      return a.setDate(a.getDate() - 365), {
        from: a.toISOString(),
        to: e.toISOString()
      };
    }
...
function Ma(t, e) {
  if (e.type === "date")
    return zt(t, e);
  const a = e, { operator: r = Mt, values: s = [], logic: o } = a;
...
function zt(t, e) {
  ...
  if (e.kind === ve.PRESET && e.preset) {
    const r = Lt(e.preset);
    return r ? [`${a}:"${r.from}..${r.to}"`] : [];
  }
  return e.kind === ve.AFTER && e.from ? [`${a}:>"${e.from}"`] : e.kind === ve.BEFORE && e.to ? [`${a}:<"${e.to}"`] : e.kind === ve.BETWEEN && e.from && e.to ? [`${a}:"${e.from}..${e.to}"`] : e.kind === ve.SPECIFIC && e.from ? [`${a}:"${e.from}"`] : [];
}
function Ia(t, e, a) {
  return e.length === 0 ? [] : [`${t}:"${e.join(a)}"`];
}
function G(t, e, a, r, s) {
  if (a.length === 0) return [];
  if (s === la.AND)
    return a.map((i) => `${t}${e}"${i}"`);
  const o = a.map((i) => `"${i}"`).join(r);
  return [`${t}${e}${o}`];
}
function Da(t, e, a) {
  const r = [], s = G("color_search", t, e, ",", a);
  return r.push(...s), r.push('color_operator:"AND"'), r;
}
function Et(t, e, a, r) {
  if (e.length === 0) return [];
  const s = e.map((o) => `"${o}"`).join(",");
  return [`${t}${a}${s}`];
}
function La(t) {
  const e = [];
  if (typeof t == "object" && t !== null && !Array.isArray(t)) {
    const i = t, l = i.resolution, c = i.orientation, d = i.faces;
    return l != null && l.length && e.push(...l.map((p) => `resolution:"${p}"`)), c != null && c.length && e.push(...c.map((p) => `orientation:"${p}"`)), d != null && d.length && e.push(...d.map((p) => `faces:"${p}"`)), e;
  }
  const a = t, r = a[et.RESOLUTION], s = a[et.ORIENTATION], o = a[et.FACES];
  return r && e.push(`resolution:"${r}"`), s && e.push(`orientation:"${s}"`), o && e.push(`faces:"${o}"`), e;
}
function za(t, e) {
  const a = Ra(t);
  if (e.type === "date")
    return zt(a, e);
  const r = e, { operator: s = Mt, values: o = [], logic: i, metadataType: l } = r;
  return o.length === 0 ? [] : (l === x.NUMERIC || l === x.DECIMAL2) && s === $.RANGE ? [`${a}${$.IS}"${o.join("..")}"`] : l === x.GEO_POINT ? G(a, s + "~", o, ",", i) : G(a, s, o, ",", i);
}
function Ra(t) {
  for (const e of Ot)
    if (t.startsWith(e))
...
function ja(t) {
  const e = localStorage.getItem(Ft + t), a = localStorage.getItem(Pt + t);
  let r;
  try {
    r = e ? JSON.parse(e) : [...$t];
  } catch {
  ...
  return { pinnedFilters: r, pinnedMetadata: s };
}
function Na(t, e) {
  localStorage.setItem(Ft + t, JSON.stringify(e));
}
function Ua(t, e) {
  localStorage.setItem(Pt + t, JSON.stringify(e));
}
var Ba = Object.defineProperty, Va = Object.getOwnPropertyDescriptor, ct = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Va(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Ba(e, a, s), s;
};
...
function qa(t, e) {
  ...
  return r.cancel = () => clearTimeout(a), r;
}
var Ga = Object.defineProperty, Ha = Object.getOwnPropertyDescriptor, qe = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Ha(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Ga(e, a, s), s;
};
      ...
      <div class="popover ${this._open ? "open" : ""}">
        ...
        ${this.groups.map((e) => n`
          <div class="group">
            ...
            ${e.variants.map((a) => {
      const r = this.selectedFilters[e.uuid] === a.api_value;
      return n`
                <button
                  class="variant-option"
                  ?data-selected=${r}
                  @click=${() => this._selectVariant(e.uuid, a.api_value)}
                >
                  <span class="check-mark">${r ? n`<ap-icon name="check" .size=${14}></ap-icon>` : ""}</span>
                  ${a.label}
                </button>
              `;
    })}
...
qe([
  u({ type: Array })
], ie.prototype, "groups", 2);
qe([
  u({ type: Object })
], ie.prototype, "selectedFilters", 2);
qe([
  E()
], ie.prototype, "_open", 2);
ie = qe([
  y("ap-regional-settings")
], ie);
var Ka = Object.defineProperty, Ya = Object.getOwnPropertyDescriptor, Q = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Ya(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Ka(e, a, s), s;
};
const Xa = [
...
let U = class extends _ {
  constructor() {
    super(...arguments), this.activeTab = "assets", this.hiddenTabs = [], this.viewMode = "grid", this.searchQuery = "", this.regionalGroups = [], this.regionalFilters = {}, this._localSearch = "", this._debouncedSearch = qa((t) => {
      this.dispatchEvent(new CustomEvent("search-change", { detail: { value: t }, bubbles: !0, composed: !0 }));
    }, 300);
  }
  disconnectedCallback() {
  ...
  render() {
    const t = Xa.filter((e) => !this.hiddenTabs.includes(e.value));
    return n`
...
Q([
  u({ type: Array })
], U.prototype, "hiddenTabs", 2);
Q([
  u()
], U.prototype, "viewMode", 2);
Q([
  u()
], U.prototype, "searchQuery", 2);
Q([
  u({ type: Array })
], U.prototype, "regionalGroups", 2);
Q([
  u({ type: Object })
], U.prototype, "regionalFilters", 2);
Q([
  E()
], U.prototype, "_localSearch", 2);
U = Q([
  y("ap-header")
], U);
var Qa = Object.defineProperty, Wa = Object.getOwnPropertyDescriptor, Rt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Wa(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Qa(e, a, s), s;
};
...
var Za = Object.defineProperty, Ja = Object.getOwnPropertyDescriptor, dt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Ja(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Za(e, a, s), s;
};
...
var es = Object.defineProperty, ts = Object.getOwnPropertyDescriptor, jt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? ts(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && es(e, a, s), s;
};
let Me = class extends _ {
  constructor() {
  ...
  _toggle(t) {
    const e = this.selected.includes(t) ? this.selected.filter((a) => a !== t) : [...this.selected, t];
    this.dispatchEvent(
...
var as = Object.defineProperty, ss = Object.getOwnPropertyDescriptor, ae = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? ss(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && as(e, a, s), s;
};
function Ct(t) {
...
let V = class extends _ {
  constructor() {
    super(...arguments), this.filterKey = g.DATE, this.field = "created", this.kind = null, this.preset = "", this.from = "", this.to = "";
  }
  get _isLicenseExpiry() {
    return this.filterKey === g.LICENSE_EXPIRY;
  }
  get _rangeOptions() {
    return this._isLicenseExpiry ? xa : lt;
  }
  ...
  _selectPreset(t) {
    ...
    const a = Lt(t);
    a && (this.from = a.from, this.to = a.to), this._dispatchChange();
  }
  _dispatchChange() {
    this.dispatchEvent(
      new CustomEvent("filter-change", {
  ...
  _handleDateInput(t, e) {
    const a = e.target.value, r = a ? (/* @__PURE__ */ new Date(a + "T00:00:00")).toISOString() : "";
    if (t === "from" ? this.from = r : this.to = r, this.preset === "specific" && t === "from" && r) {
      const s = /* @__PURE__ */ new Date(a + "T23:59:59");
      this.to = s.toISOString();
    }
    this.preset === "after" && t === "from" && r && (this.to = (/* @__PURE__ */ new Date()).toISOString()), this.preset === "before" && t === "to" && r && (this.from = (/* @__PURE__ */ new Date(0)).toISOString()), this._dispatchChange();
  }
  _renderFieldTabs() {
    return this._isLicenseExpiry ? m : n`
  ...
  _renderDateInputs() {
    ...
    const e = this.from ? Ct(new Date(this.from)) : "", a = this.to ? Ct(new Date(this.to)) : "";
    return t === "specific" ? n`
...
var rs = Object.defineProperty, is = Object.getOwnPropertyDescriptor, Se = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? is(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && rs(e, a, s), s;
};
let J = class extends _ {
  constructor() {
    super(...arguments), this.available = [], this.selected = [], this.filterKey = g.TAGS, this._search = "";
  }
  get _atLimit() {
  ...
  _toggle(t) {
    ...
    const a = e ? this.selected.filter((r) => r !== t) : [...this.selected, t];
    this.dispatchEvent(
      new CustomEvent("filter-change", {
        detail: {
          key: this.filterKey,
  ...
  render() {
    const t = this.available.filter(
      (e) => e.toLowerCase().includes(this._search.toLowerCase())
    );
    return n`
      <input
        class="search-input"
      ...
      ${this.selected.length > 0 ? n`
            <div class="limit-indicator ${this._atLimit ? "at-limit" : ""}">
              ${this.selected.length} / ${yt} selected
            </div>
          ` : ""}

      <div class="tags">
        ${t.length === 0 ? n`<span class="empty-message">No tags found</span>` : t.map((e) => {
      const a = this.selected.includes(e), r = !a && this._atLimit;
      return n`
                <button
                  class="tag ${a ? "selected" : ""} ${r ? "disabled" : ""}"
                  ?disabled=${r}
...
Se([
  u({ type: Array })
], J.prototype, "available", 2);
Se([
  u({ type: Array })
], J.prototype, "selected", 2);
Se([
  u()
], J.prototype, "filterKey", 2);
Se([
  E()
...
var os = Object.defineProperty, ns = Object.getOwnPropertyDescriptor, Ge = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? ns(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && os(e, a, s), s;
};
let oe = class extends _ {
  ...
  get _atLimit() {
    return this.selected.length >= xt;
  }
  _toggle(t) {
    const e = this.selected.includes(t);
    if (!e && this._atLimit) return;
  ...
  render() {
    const t = this.labels.filter(
      (e) => e.name.toLowerCase().includes(this._search.toLowerCase())
    );
    return n`
      <input
        class="search-input"
      ...
      ${this.selected.length > 0 ? n`
            <div class="limit-indicator ${this._atLimit ? "at-limit" : ""}">
              ${this.selected.length} / ${xt} selected
            </div>
          ` : ""}

      <div class="labels">
        ${t.length === 0 ? n`<span class="empty-message">No labels found</span>` : t.map((e) => {
      const a = this.selected.includes(e.uuid), r = !a && this._atLimit;
      return n`
                <div
                  class="label-item ${r ? "disabled" : ""}"
                  @click=${() => !r && this._toggle(e.uuid)}
                >
                  <ap-checkbox ?checked=${a}></ap-checkbox>
                  <div
                    class="color-dot"
...
var ls = Object.defineProperty, cs = Object.getOwnPropertyDescriptor, He = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? cs(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && ls(e, a, s), s;
};
let ne = class extends _ {
  constructor() {
    super(...arguments), this.selected = "", this.tolerance = da, this.coverage = ca;
  }
  _selectColor(t) {
    this.selected = this.selected === t ? "" : t, this._dispatchChange();
  }
  _selectTolerance(t) {
    this.tolerance = t, this._dispatchChange();
  }
  _handleCoverageInput(t) {
    const e = t.target.value, a = parseInt(e, 10);
    !isNaN(a) && a >= 0 && a <= 100 && (this.coverage = String(a), this._dispatchChange());
  }
  _dispatchChange() {
    if (!this.selected) {
      this.dispatchEvent(
    ...
    const t = `${this.selected} ${this.tolerance} ${this.tolerance} ${this.coverage}`;
    this.dispatchEvent(
      new CustomEvent("filter-change", {
...
He([
  u()
], ne.prototype, "tolerance", 2);
He([
  u()
], ne.prototype, "coverage", 2);
ne = He([
  y("ap-filter-color")
], ne);
var ds = Object.defineProperty, ps = Object.getOwnPropertyDescriptor, pe = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? ps(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && ds(e, a, s), s;
};
const At = 1e6;
let H = class extends _ {
  constructor() {
    super(...arguments), this.min = 0, this.max = 0, this._minMB = "", this._maxMB = "", this._initialized = !1;
  }
  willUpdate(t) {
    !this._initialized && (t.has("min") || t.has("max")) && (this._initialized = !0, this.min > 0 && (this._minMB = String(this.min)), this.max > 0 && (this._maxMB = String(this.max)));
  }
  _emitChange() {
    const t = this._minMB !== "" ? Math.round(parseFloat(this._minMB) * At) : null, e = this._maxMB !== "" ? Math.round(parseFloat(this._maxMB) * At) : null;
    let a;
    if (t !== null && e !== null)
      a = `${t}..${e}`;
    else if (t !== null)
  ...
  _onMinInput(t) {
    this._minMB = t.target.value, this._emitChange();
  }
  _onMaxInput(t) {
    this._maxMB = t.target.value, this._emitChange();
  }
  render() {
    return n`
...
pe([
  E()
], H.prototype, "_minMB", 2);
pe([
  E()
], H.prototype, "_maxMB", 2);
pe([
  E()
], H.prototype, "_initialized", 2);
H = pe([
  y("ap-filter-size")
], H);
var us = Object.defineProperty, hs = Object.getOwnPropertyDescriptor, Ke = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? hs(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && us(e, a, s), s;
};
let le = class extends _ {
  ...
  _renderSection(t, e, a, r) {
    return n`
      ...
      (s) => n`
              <button
                class="option ${a.includes(s.value) ? "selected" : ""}"
                @click=${() => r(s.value)}
              >
                ${s.label}
              </button>
  ...
  render() {
    return n`
      ...
      (t) => this._toggleResolution(t)
    )}
      ${this._renderSection(
      "Orientation",
      ba,
      this.selectedOrientation,
      (t) => this._toggleOrientation(t)
    )}
      ${this._renderSection(
      "Faces",
      _a,
      this.selectedFaces,
      (t) => this._toggleFaces(t)
    )}
    `;
  }
};
...
Ke([
  u({ type: Array })
], le.prototype, "selectedResolution", 2);
Ke([
  u({ type: Array })
], le.prototype, "selectedOrientation", 2);
Ke([
  u({ type: Array })
...
var fs = Object.defineProperty, vs = Object.getOwnPropertyDescriptor, R = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? vs(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && fs(e, a, s), s;
};
function w(t) {
  return `${it[t.type] || ""}${t.key}`;
}
let F = class extends _ {
  constructor() {
    super(...arguments), this.fields = [], this.appliedFilters = {}, this.visibleFields = [], this._showFieldSelection = !1, this._fieldSearch = "", this._collapsedFields = /* @__PURE__ */ new Set(), this._tagInputs = {}, this._datePresets = {}, this._dateFroms = {}, this._dateTos = {}, this._specificModeFields = /* @__PURE__ */ new Set();
  }
  // ── Computed helpers ─────────────────────────────────────────────
  get _atFieldLimit() {
    return this.visibleFields.length >= wt;
  }
  _getField(t) {
    return this.fields.find((e) => w(e) === t);
  }
  _getUIType(t) {
    return Ca[t.type];
  }
  _getApplied(t) {
    return this.appliedFilters[t] || {};
  }
  _isSpecialValue(t) {
    return !t || t.length !== 1 ? !1 : t[0] === ot || t[0] === nt;
  }
  // ── Event dispatchers ────────────────────────────────────────────
  ...
  _onOperatorChange(t, e) {
    const a = e.target.value, s = this._getApplied(w(t)).values || [];
    this._emitFilterChange(t, a, s);
  }
  // ── Text filter ──────────────────────────────────────────────────
  _onTextInput(t, e) {
    const a = e.target.value, s = this._getApplied(w(t)).operator || St[0].value;
    this._emitFilterChange(t, s, a ? [a] : []);
  }
  // ── Number filter ────────────────────────────────────────────────
  _onNumberInput(t, e, a) {
    const r = e.target.value, s = w(t), o = this._getApplied(s), i = o.operator || tt[0].value, l = [...o.values || []];
    a === 0 ? l[0] = r : l[1] = r;
    const c = l.filter((d) => d !== "" && d !== void 0);
    if (c.length > 0) {
      const d = t.type === x.DECIMAL2;
      if (!c.every(
        (h) => d ? !isNaN(parseFloat(h)) : /^-?\d+$/.test(h)
      )) return;
    }
    this._emitFilterChange(t, i, l);
  }
  // ── Boolean filter ───────────────────────────────────────────────
  _onBooleanSelect(t, e) {
    var s;
    const a = this._getApplied(w(t));
    ((s = a.values) == null ? void 0 : s[0]) === e ? this._emitFilterChange(t, void 0, []) : this._emitFilterChange(t, a.operator, [e]);
  }
  // ── Select-one filter ────────────────────────────────────────────
  _onSelectOneChange(t, e) {
    const a = e.target.value, s = this._getApplied(w(t)).operator || Te[0].value;
    this._emitFilterChange(t, s, a ? [a] : []);
  }
  // ── Multi-select filter ──────────────────────────────────────────
  _onMultiSelectToggle(t, e) {
    const a = w(t), r = this._getApplied(a), s = r.operator || ge[0].value, o = r.values || [], i = o.includes(e) ? o.filter((l) => l !== e) : [...o, e];
    this._emitFilterChange(t, s, i);
  }
  // ── Tags filter ──────────────────────────────────────────────────
  _onTagKeydown(t, e) {
    const a = w(t);
  ...
  _onTagInput(t, e) {
    ...
    if (a.includes(",")) {
      ...
      if (r) {
        this._tagInputs = { ...this._tagInputs, [t]: r };
        const s = this._getField(t);
        s && this._commitTagInput(s, t);
      }
      return;
    }
    this._tagInputs = { ...this._tagInputs, [t]: a };
  }
  _commitTagInput(t, e) {
    const a = (this._tagInputs[e] || "").trim();
    if (!a) return;
    const r = this._getApplied(e), s = r.operator || ge[0].value, o = r.values || [];
    o.includes(a) || this._emitFilterChange(t, s, [...o, a]), this._tagInputs = { ...this._tagInputs, [e]: "" };
  }
  _removeTag(t, e) {
    const a = w(t), r = this._getApplied(a), s = r.operator || ge[0].value, o = r.values || [];
  ...
  _onDatePreset(t, e) {
    ...
    switch (e) {
      case "today": {
        const i = new Date(r);
        i.setHours(0, 0, 0, 0), s = i.toISOString(), o = r.toISOString();
        break;
      }
      case "last_week": {
        const i = new Date(r), l = i.getDay();
        i.setDate(i.getDate() - (l === 0 ? 6 : l - 1)), i.setHours(0, 0, 0, 0);
        const c = new Date(i);
        c.setDate(c.getDate() - 7), s = c.toISOString(), o = i.toISOString();
        break;
      }
      case "last_month": {
        const i = new Date(r.getFullYear(), r.getMonth(), 1), l = new Date(i);
        l.setMonth(l.getMonth() - 1), s = l.toISOString(), o = i.toISOString();
        break;
      }
      case "last_year": {
        const i = new Date(r.getFullYear(), 0, 1), l = new Date(i);
        l.setFullYear(l.getFullYear() - 1), s = l.toISOString(), o = i.toISOString();
        break;
      }
    }
    s && o && (this._dateFroms = { ...this._dateFroms, [a]: s }, this._dateTos = { ...this._dateTos, [a]: o }, this._emitFilterChange(t, "..", [s, o]));
  }
  _onDateInput(t, e, a) {
    const r = a.target.value, s = w(t), o = this._datePresets[s] || "", i = r ? (/* @__PURE__ */ new Date(r + "T00:00:00")).toISOString() : "";
    e === "from" ? this._dateFroms = { ...this._dateFroms, [s]: i } : this._dateTos = { ...this._dateTos, [s]: i };
    let l = this._dateFroms[s] || "", c = this._dateTos[s] || "";
    o === "after" && l && (c = (/* @__PURE__ */ new Date()).toISOString(), this._dateTos = { ...this._dateTos, [s]: c }), o === "before" && c && (l = (/* @__PURE__ */ new Date(0)).toISOString(), this._dateFroms = { ...this._dateFroms, [s]: l }), l && c && this._emitFilterChange(t, "..", [l, c]);
  }
  // ── Geo filter ───────────────────────────────────────────────────
  _onGeoInput(t) {
    var d, p, h;
    const e = w(t), a = (d = this.shadowRoot) == null ? void 0 : d.querySelector(
      `[data-geo-lat="${e}"]`
    ), r = (p = this.shadowRoot) == null ? void 0 : p.querySelector(
      `[data-geo-lng="${e}"]`
    ), s = (h = this.shadowRoot) == null ? void 0 : h.querySelector(
      `[data-geo-radius="${e}"]`
    ), o = (a == null ? void 0 : a.value) || "", i = (r == null ? void 0 : r.value) || "", l = (s == null ? void 0 : s.value) || "";
    if (!o && !i) {
      this._emitFilterChange(t, void 0, []);
      return;
    }
    let c = `${o},${i}`;
    l && (c += `..${l}`), this._emitFilterChange(t, $.IS, [c]);
  }
  // ── Empty/non-empty option click ─────────────────────────────────
  _onEmptyOption(t, e) {
    var s;
    const a = this._getApplied(w(t));
    ((s = a.values) == null ? void 0 : s[0]) === e ? this._emitFilterChange(t, a.operator, []) : this._emitFilterChange(t, a.operator, [e]);
  }
  // ── Render: Field selection panel ────────────────────────────────
  _renderFieldSelection() {
    const t = this._fieldSearch.toLowerCase(), e = this.fields.filter(
      (o) => o.label.toLowerCase().includes(t)
    ), a = e.filter(
      (o) => !o.group || o.group === "root"
    ), r = e.filter((o) => o.group === "product"), s = (o, i) => i.length === 0 ? m : n`
        <div class="field-group-label">${o}</div>
        ${i.map((l) => {
      const c = w(l), d = this.visibleFields.includes(c), p = !d && this._atFieldLimit;
      return n`
            <div
              class="field-item ${d ? "selected" : ""} ${p ? "disabled" : ""}"
              @click=${() => !p && !d && this._addField(l)}
            >
              ${d ? n`<ap-icon name="check" .size=${14}></ap-icon>` : m}
              <span>${l.label}</span>
            </div>
          `;
    })}
    ...
    return n`
      <div class="field-selection">
        ...
        <input
          ...
          @input=${(o) => {
      this._fieldSearch = o.target.value;
    }}
        />
        ...
        ${this.visibleFields.length > 0 ? n`
              <div
                class="limit-note ${this._atFieldLimit ? "at-limit" : ""}"
              >
                ${this.visibleFields.length} / ${wt} fields selected
              </div>
            ` : m}
      </div>
    `;
  ...
  _renderOperator(t, e) {
    const r = this._getApplied(w(t)).operator || e[0].value;
    return n`
      <select
        class="operator-select"
        .value=${r}
        @change=${(s) => this._onOperatorChange(t, s)}
      >
        ${e.map(
      (s) => n`<option value=${s.value} ?selected=${r === s.value}>
              ${s.label}
            </option>`
    )}
      </select>
  ...
  _renderEmptyOptions(t) {
    var r;
    const a = (r = this._getApplied(w(t)).values) == null ? void 0 : r[0];
    return n`
      <div class="divider"></div>
      ...
      (s) => n`
            <div
              class="option-item"
              @click=${() => this._onEmptyOption(t, s.value)}
            >
              <div class="radio-dot ${a === s.value ? "active" : ""}">
  ...
  _renderTextFilter(t) {
    var s;
    const e = this._getApplied(w(t)), r = this._isSpecialValue(e.values) ? "" : ((s = e.values) == null ? void 0 : s[0]) || "";
    return n`
      ${this._renderOperator(t, St)}
      <input
        class="text-input"
        type="text"
  ...
  _renderNumberFilter(t) {
    var h, f;
    const e = w(t), a = this._getApplied(e), r = a.operator || tt[0].value, s = this._isSpecialValue(a.values), o = r === "..", i = t.type === x.DECIMAL2, l = i ? "0.01" : "1", c = i ? "0.00" : "0", d = s ? "" : ((h = a.values) == null ? void 0 : h[0]) || "", p = s ? "" : ((f = a.values) == null ? void 0 : f[1]) || "";
    return n`
      ${this._renderOperator(t, tt)}
      ${o ? n`
            <div class="range-row">
              <input
                class="number-input"
  ...
  _renderBooleanFilter(t) {
    ...
    return n`
      ...
      (s) => n`
            <div
              class="option-item"
              @click=${() => this._onBooleanSelect(t, s.value)}
            >
              <div class="radio-dot ${a === s.value ? "active" : ""}">
                <div class="radio-dot-inner"></div>
              </div>
  ...
  _renderSelectOneFilter(t) {
    var l;
    const e = w(t), a = this._getApplied(e), r = this._isSpecialValue(a.values), s = this._specificModeFields.has(e), o = r ? "" : ((l = a.values) == null ? void 0 : l[0]) || "", i = t.possible_values || [];
    return n`
      ${this._renderOperator(t, Te)}
      ${s ? n`
          <input
            class="text-input"
            type="text"
            ...
            .value=${o}
            @change=${(c) => {
      const d = c.target.value, p = a.operator || Te[0].value;
      this._emitFilterChange(t, p, d ? [d] : []);
    }}
          />
        ` : n`
          <select
            class="value-select"
            .value=${o}
            @change=${(c) => this._onSelectOneChange(t, c)}
          >
            <option value="">Select...</option>
            ${i.map(
      (c) => n`<option value=${c.api_value} ?selected=${o === c.api_value}>
                  ${c.label}
                </option>`
    )}
          </select>
      ...
      (c) => {
        ...
        return n`
          ...
          if (c.value === Oe) {
            ...
            p.add(e), this._specificModeFields = p;
          } else {
            if (this._specificModeFields.has(e)) {
              const p = new Set(this._specificModeFields);
              p.delete(e), this._specificModeFields = p;
            }
            this._onEmptyOption(t, c.value);
          }
        }}
            >
              <div class="radio-dot ${c.value === Oe && s || c.value !== Oe && ((d = a.values) == null ? void 0 : d[0]) === c.value ? "active" : ""}">
                <div class="radio-dot-inner"></div>
              </div>
              <span>${c.label}</span>
            </div>
  ...
  _renderMultiSelectFilter(t) {
    const e = this._getApplied(w(t)), r = this._isSpecialValue(e.values) ? [] : e.values || [], s = t.possible_values || [];
    return n`
      ${this._renderOperator(t, ge)}
  ...
  // ── Render: Tags filter ──────────────────────────────────────────
  _renderTagsFilter(t) {
    const e = w(t), a = this._getApplied(e), s = this._isSpecialValue(a.values) ? [] : a.values || [], o = this._tagInputs[e] || "";
    return n`
      ${this._renderOperator(t, ge)}
      ...
      (i) => n`
        ...
        <input
          class="tag-input"
          type="text"
          placeholder=${s.length > 0 ? "Add tag..." : "Type and press Enter..."}
          .value=${o}
  ...
  _renderDateFilter(t) {
    var p;
    const e = w(t), a = this._datePresets[e] || "", r = this._getApplied(e), o = this._isSpecialValue(r.values) ? ((p = r.values) == null ? void 0 : p[0]) || "" : a, i = [
      ...lt,
      { value: "empty", label: "Empty" },
      { value: "non-empty", label: "Not empty" }
    ], l = this._dateFroms[e] ? new Date(this._dateFroms[e]).toISOString().split("T")[0] : "", c = this._dateTos[e] ? new Date(this._dateTos[e]).toISOString().split("T")[0] : "", d = ["before", "after", "between"].includes(o);
    return n`
      <div class="presets">
        ${i.map(
      (h) => n`
            <button
              class="preset ${o === h.value ? "active" : ""}"
              @click=${() => this._onDatePreset(t, h.value)}
            >
              ${h.label}
            </button>
          `
  ...
  _renderGeoFilter(t) {
    var l;
    const e = w(t), a = this._getApplied(e), r = this._isSpecialValue(a.values);
    let s = "", o = "", i = "";
    if (!r && ((l = a.values) != null && l[0])) {
      const c = a.values[0], [d, p] = c.split(".."), [h, f] = d.split(",");
      s = h || "", o = f || "", i = p || "";
    }
    return n`
      <div class="geo-row">
  ...
  _renderFaceMatcherFilter(t) {
    const e = w(t), a = this._getApplied(e), r = a.values || [], s = a.operator || $.IS;
    return n`
  ...
  _renderFieldSection(t) {
    ...
    if (!e) return m;
    const a = this._collapsedFields.has(t);
    return n`
        ...
        >
          ...
          <div class="field-header-actions">
            <button
              ...
              @click=${(r) => {
      r.stopPropagation(), this._removeField(e);
    }}
              title="Remove field"
            >
              <ap-icon name="close" .size=${14}></ap-icon>
        ...
        <div class="field-body ${a ? "collapsed" : ""}">
...
R([
  u({ type: Object })
], F.prototype, "appliedFilters", 2);
R([
  u({ type: Array })
], F.prototype, "visibleFields", 2);
R([
  E()
], F.prototype, "_showFieldSelection", 2);
R([
  E()
], F.prototype, "_fieldSearch", 2);
R([
  E()
], F.prototype, "_collapsedFields", 2);
R([
  E()
], F.prototype, "_tagInputs", 2);
R([
  E()
], F.prototype, "_datePresets", 2);
R([
  E()
], F.prototype, "_dateFroms", 2);
R([
  E()
], F.prototype, "_dateTos", 2);
R([
  E()
], F.prototype, "_specificModeFields", 2);
F = R([
  y("ap-filter-metadata")
], F);
var gs = Object.defineProperty, ms = Object.getOwnPropertyDescriptor, j = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? ms(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && gs(e, a, s), s;
};
let M = class extends _ {
  constructor() {
    super(...arguments), this.selectedStatus = [], this.statusOperator = ":", this.approverValue = "", this.approverOperator = ":", this.requesterValue = "", this.requesterOperator = ":", this.dueDatePreset = "", this.dueDateFrom = "", this.dueDateTo = "", this._expandedSection = null;
  }
  _toggleSection(t) {
    this._expandedSection = this._expandedSection === t ? null : t;
  }
  _toggleStatus(t) {
  ...
  _handleDueDatePreset(t) {
    const e = this.dueDatePreset === t ? "" : t;
    this._emitChange(B.DUE_DATE, e ? [e] : [], ":");
  }
  ...
  _emitChange(t, e, a) {
    this.dispatchEvent(new CustomEvent("filter-change", {
      detail: { key: t, values: e, operator: a },
      bubbles: !0,
      composed: !0
    }));
  }
  render() {
    return n`
      ...
      (t) => n`
        ...
        ${this.dueDatePreset === "before" || this.dueDatePreset === "after" || this.dueDatePreset === "between" ? n`
            <div class="date-input-row">
              ${this.dueDatePreset !== "before" ? n`<input class="date-input" type="date" .value=${this.dueDateFrom} @change=${this._handleDueDateFrom} />` : m}
              ${this.dueDatePreset !== "after" ? n`<input class="date-input" type="date" .value=${this.dueDateTo} @change=${this._handleDueDateTo} />` : m}
            </div>
          ` : m}
      </div>
    `;
...
j([
  u({ type: Array })
], M.prototype, "selectedStatus", 2);
j([
  u()
], M.prototype, "statusOperator", 2);
j([
  u()
], M.prototype, "approverValue", 2);
j([
  u()
], M.prototype, "approverOperator", 2);
j([
  u()
], M.prototype, "requesterValue", 2);
j([
  u()
], M.prototype, "requesterOperator", 2);
j([
  u()
], M.prototype, "dueDatePreset", 2);
j([
  u()
], M.prototype, "dueDateFrom", 2);
j([
  u()
], M.prototype, "dueDateTo", 2);
j([
  E()
], M.prototype, "_expandedSection", 2);
M = j([
  y("ap-filter-approval")
], M);
var bs = Object.defineProperty, _s = Object.getOwnPropertyDescriptor, I = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? _s(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && bs(e, a, s), s;
};
let T = class extends _ {
  constructor() {
    super(...arguments), this.totalCount = 0, this.totalFolderCount = 0, this.isLoading = !1, this.sortBy = "created_at", this.sortDirection = "desc", this.filters = {
      metadata: { pinned: [], visible: [], applied: {} },
      pinned: [],
      visible: [],
      applied: {}
    }, this.labels = [], this.metadataFields = [], this._showDropdown = !1, this._openFilter = null, this._handleOutsideClick = (t) => {
      const e = t.composedPath();
      this._showDropdown && this._filterDropdownEl && !e.includes(this._filterDropdownEl) && (this._showDropdown = !1), this._openFilter && !e.includes(this) && (this._openFilter = null);
    };
  }
  connectedCallback() {
    super.connectedCallback(), document.addEventListener("click", this._handleOutsideClick);
  ...
  _closeAllDropdowns() {
    var t;
    this._showDropdown = !1, this._openFilter = null, (t = this._sortDropdown) == null || t.close();
  }
  _toggleDropdown() {
  ...
  _selectFilterKey(t) {
    this._closeAllDropdowns(), this._openFilter = t;
  }
  _handleSortOpen() {
    this._showDropdown = !1, this._openFilter = null;
  }
  _handleSort(t) {
  ...
  _handleFilterChange(t) {
    t.stopPropagation();
    const { key: e, values: a, operator: r } = t.detail;
    this.dispatchEvent(new CustomEvent("filter-update", {
      detail: { key: e, values: a, operator: r },
      bubbles: !0,
      composed: !0
    }));
  }
  ...
  _renderFilterContent(t) {
    ...
    switch (t) {
      ...
      case "color": {
        const i = e.color, c = (((a = i == null ? void 0 : i.values) == null ? void 0 : a[0]) || "").split(" ");
      ...
      case "size": {
        const i = e.size, l = ((r = i == null ? void 0 : i.values) == null ? void 0 : r[0]) || "", [c, d] = l.split(".."), p = c ? parseFloat(c) / 1e6 : 0, h = d ? parseFloat(d) / 1e6 : 0;
      ...
      case "image": {
        const i = e.image, l = i && "resolution" in i ? i : null;
      ...
      case "approval": {
        const i = e.approval_status, l = e.task_approver, c = e.task_requester, d = e.task_duedate;
        return n`<ap-filter-approval
          .selectedStatus=${(i == null ? void 0 : i.values) || []}
          .statusOperator=${(i == null ? void 0 : i.operator) || ":"}
          .approverValue=${((s = l == null ? void 0 : l.values) == null ? void 0 : s[0]) || ""}
          .approverOperator=${(l == null ? void 0 : l.operator) || ":"}
          .requesterValue=${((o = c == null ? void 0 : c.values) == null ? void 0 : o[0]) || ""}
  ...
  _renderFilterButton(t) {
    const e = this._isFilterActive(t.key);
    return n`
      <button
  ...
  render() {
    ...
    return n`
      <div class="toolbar-row">
        ${this.isLoading ? n`<span class="count-skeleton"></span>` : this.totalCount > 0 || this.totalFolderCount > 0 ? n`<span class="count">${this.totalFolderCount > 0 ? `${this.totalFolderCount.toLocaleString()} folder${this.totalFolderCount !== 1 ? "s" : ""}${this.totalCount > 0 ? ", " : ""}` : ""}${this.totalCount > 0 ? `${this.totalCount.toLocaleString()} asset${this.totalCount !== 1 ? "s" : ""}` : ""}</span>` : n`<span class="count"></span>`}
          ...
          <div class="filter-dropdown">
            ...
            ${this._showDropdown ? n`
              <div class="dropdown-menu">
                ${va.map((e) => this._renderFilterButton(e))}
              </div>
            ` : m}
          </div>
          <div class="divider"></div>
      ...
      ${this._openFilter ? n`
        <div class="popover-wrapper">
          <ap-filter-popover .open=${!0} .title=${It[this._openFilter] || this._openFilter}>
...
I([
  $e(".filter-dropdown")
], T.prototype, "_filterDropdownEl", 2);
I([
  E()
...
T = I([
  y("ap-content-toolbar")
], T);
var ys = Object.defineProperty, xs = Object.getOwnPropertyDescriptor, ue = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? xs(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && ys(e, a, s), s;
};
let K = class extends _ {
  constructor() {
    super(...arguments), this.appliedFilters = {}, this.appliedMetadata = {}, this.pinnedFilters = [], this.pinnedMetadata = [], this.metadataFields = [];
  }
  _getFilterSummary(t) {
    if (t.type === "string") {
      const e = t;
      return e.values.length === 0 ? "" : e.values.length === 1 ? e.values[0] : `${e.values[0]} +${e.values.length - 1}`;
    }
    if (t.type === "date") {
      const e = t;
      return e.preset ? e.preset : e.from && e.to ? `${e.from} - ${e.to}` : e.from ? `after ${e.from}` : e.to ? `before ${e.to}` : "";
    }
  ...
  _getMetadataLabel(t) {
    const e = this._stripMetadataPrefix(t), a = this.metadataFields.find((r) => r.key === e);
  ...
  _togglePin(t) {
    const e = this.pinnedFilters.includes(t);
    this.dispatchEvent(new CustomEvent("filter-pin", {
      detail: { key: t, pinned: !e },
      bubbles: !0,
      composed: !0
    }));
  }
  _toggleMetadataPin(t) {
    const e = this.pinnedMetadata.includes(t);
  ...
  render() {
    const t = Object.entries(this.appliedFilters), e = Object.entries(this.appliedMetadata), a = t.length + e.length;
    return a === 0 ? m : n`
      <div class="chips-row">
        <div class="chips">
          ${t.map(([r, s]) => {
      const o = this.pinnedFilters.includes(r), i = this._getFilterSummary(s);
      return n`
              <span class="chip">
                <span class="chip-label">${It[r] || r}</span>
                ${i ? n`<span class="chip-summary">${i}</span>` : m}
                <span class="chip-actions">
                  <button
                    class="chip-btn ${o ? "pinned" : ""}"
                    @click=${(l) => {
        l.stopPropagation(), this._togglePin(r);
      }}
          ...
          ${e.map(([r, s]) => {
      const o = this.pinnedMetadata.includes(r), i = this._getFilterSummary(s), l = this._getMetadataLabel(r);
      return n`
              <span class="chip">
                ...
                <span class="chip-actions">
                  <button
                    class="chip-btn ${o ? "pinned" : ""}"
                    @click=${(c) => {
        c.stopPropagation(), this._toggleMetadataPin(r);
      }}
                    title=${o ? "Unpin filter" : "Pin filter"}
                  >
                    <ap-icon name=${o ? "pin-off" : "pin"} .size=${12}></ap-icon>
        ...
        ${a > 1 ? n`<button class="clear-all" @click=${this._clearAll}>Clear all</button>` : m}
      </div>
    `;
  }
};
...
ue([
  u({ type: Object })
], K.prototype, "appliedFilters", 2);
ue([
  u({ type: Object })
], K.prototype, "appliedMetadata", 2);
ue([
  u({ type: Array })
], K.prototype, "pinnedFilters", 2);
ue([
  u({ type: Array })
], K.prototype, "pinnedMetadata", 2);
ue([
  u({ type: Array })
], K.prototype, "metadataFields", 2);
K = ue([
  y("ap-filters-bar")
], K);
var ws = Object.defineProperty, $s = Object.getOwnPropertyDescriptor, he = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? $s(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && ws(e, a, s), s;
};
const Ss = 6;
let Y = class extends _ {
  constructor() {
    super(...arguments), this.assets = [], this.folders = [], this.folderPreviews = {}, this.selectedIds = [], this.isLoading = !1, this._prevCount = 0;
  }
  willUpdate(t) {
    if (t.has("assets")) {
    ...
    return n`
      ...
      (t) => n`
            <ap-folder-card
              ...
              @folder-open=${(e) => {
        e.stopPropagation(), this.dispatchEvent(new CustomEvent("folder-open", { detail: e.detail, bubbles: !0, composed: !0 }));
      }}
            ></ap-folder-card>
          `
    )}
        ${this.assets.map(
      (t, e) => {
        const a = e >= this._prevCount ? Math.min(e - this._prevCount, 20) : Math.min(e, 20);
        return n`
              <ap-asset-card
                .asset=${t}
                .index=${e}
                ?selected=${this.selectedIds.includes(t.uuid)}
                style="--ap-stagger-index: ${a}"
        ...
        ${this.isLoading ? Array.from({ length: Ss }, () => n`<div class="ghost-card"></div>`) : m}
      </div>
    `;
  }
};
...
var Es = Object.defineProperty, ks = Object.getOwnPropertyDescriptor, Ee = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? ks(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Es(e, a, s), s;
};
const Cs = 5;
let ee = class extends _ {
  constructor() {
    super(...arguments), this.assets = [], this.folders = [], this.selectedIds = [], this.isLoading = !1;
  }
  render() {
    return n`
      ...
      (t, e) => n`
        ...
        ${this.isLoading ? Array.from({ length: Cs }, () => n`<div class="ghost-row"></div>`) : m}
      </div>
    `;
  }
};
...
function _e(t) {
  var a;
  const e = t.info;
  return e != null && e.video_thumbnail ? e.video_thumbnail : e != null && e.image_thumbnail ? e.image_thumbnail : e != null && e.thumbnail ? e.thumbnail : e != null && e.preview ? e.preview : Ie(((a = t.url) == null ? void 0 : a.cdn) || "");
}
function pt(t) {
  const e = t.info;
  return e != null && e.video_thumbnail ? e.video_thumbnail : e != null && e.image_thumbnail ? e.image_thumbnail : e != null && e.thumbnail ? e.thumbnail : "";
}
function Ye(t) {
  var a, r;
  let e = Ie(((a = t.url) == null ? void 0 : a.cdn) || ((r = t.url) == null ? void 0 : r.public) || "");
  return e ? (e = e.replace(/([?&])func=proxy&?/, "$1").replace(/[?&]$/, ""), Nt(e, {
    w: "200",
...
function Nt(t, e) {
  try {
    const a = new URL(t);
    for (const [r, s] of Object.entries(e))
      a.searchParams.set(r, s);
    return a.toString();
  } catch {
    return t;
  }
}
const Tt = /^(https:\/\/)([a-z0-9_-]+)\.filerobot\.com\//i;
function Ie(t, e) {
  if (!t) return "";
  ...
  if (a) {
    const r = a[2];
    return t.replace(Tt, `$1assets.filerobot.com/${r}/`);
  }
  if (e)
    try {
      const r = new URL(t);
      return `https://assets.filerobot.com/${e}${r.pathname}${r.search}`;
    } catch {
    }
  return t;
}
function ut(t) {
  if (t === 0) return "0 B";
  const e = ["B", "KB", "MB", "GB", "TB"], a = 1024, r = Math.floor(Math.log(t) / Math.log(a));
  return `${(t / Math.pow(a, r)).toFixed(r === 0 ? 0 : 1)} ${e[r]}`;
}
function De(t) {
  if (!t) return "—";
  const e = new Date(t);
...
const Ts = /* @__PURE__ */ new Set(["jpg", "jpeg", "png", "gif", "webp", "svg", "bmp", "ico", "tiff", "tif", "avif", "heic", "heif"]), Os = /* @__PURE__ */ new Set([
  ...Ts,
  "pdf",
  "psd",
  "eps",
...
const Ut = "https://assets.filerobot.com/scaleflex/assets/file-types/v2/", Fs = {
  png: "PNG",
  jpg: "JPG",
  jpeg: "JPG",
  gif: "GIF",
...
function ke(t) {
  const e = (t == null ? void 0 : t.toLowerCase().replace(".", "")) || "", a = Fs[e];
  return `${Ut}${a || "DEFAULT"}.png`;
}
function te() {
  return `${Ut}DEFAULT.png`;
}
function st(t) {
  if (!t) return "";
  const e = t.split("/");
...
const Ps = /* @__PURE__ */ new Set(["png", "svg", "webp", "gif", "avif"]);
function Xe(t) {
  const e = (t == null ? void 0 : t.toLowerCase().replace(".", "")) || "";
  return Ps.has(e);
}
var Ms = Object.defineProperty, Is = Object.getOwnPropertyDescriptor, Qe = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Is(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Ms(e, a, s), s;
};
let ce = class extends _ {
  constructor() {
    super(...arguments), this.index = 0, this.selected = !1;
  }
  _handleSelect(t) {
    if (t.stopPropagation(), t.composedPath().some(
      (r) => r instanceof HTMLElement && (r.classList.contains("check") || r.classList.contains("check-box"))
    )) {
  ...
  render() {
    ...
    const t = this.asset;
    if (!t) return n``;
    const a = Z(t.type) === "video", r = ((c = t.extension) == null ? void 0 : c.toLowerCase()) === "pdf", s = ht(t.extension || "");
    let o = "";
    a ? o = pt(t) : r ? o = Ye(t) : s && (o = _e(t));
    const i = ke(t.extension || ""), l = Xe(t.extension || "");
    return n`
      <div class="card" @click=${this._handleSelect}>
        <div class="thumbnail ${l && o ? "chess" : ""}">
          ${o ? n`<img
                ...
                @error=${(h) => {
      const f = h.target, v = te();
      f.src !== i && f.src !== v ? (f.src = i, f.classList.add("icon-fallback")) : f.src !== v && (f.src = v);
    }}
              />` : n`<img
                src=${i}
                alt=${t.name}
      ...
      f.src !== v && (f.src = v);
        ...
        <div class="info">
          ...
          <div class="meta">${(d = t.extension) == null ? void 0 : d.toUpperCase()} · ${ut(((p = t.size) == null ? void 0 : p.bytes) || 0)}</div>
        </div>
      </div>
    `;
  }
...
var Ds = Object.defineProperty, Ls = Object.getOwnPropertyDescriptor, We = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Ls(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Ds(e, a, s), s;
};
  ...
  render() {
    ...
    return n`
      ...
      f.src !== v && (f.src = v);
        ...
        <div class="desc">${ut(((p = t.size) == null ? void 0 : p.bytes) || 0)}</div>
        <div class="date">${De(t.created_at || "")}</div>
        <div class="actions">
...
var zs = Object.defineProperty, Rs = Object.getOwnPropertyDescriptor, ft = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Rs(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && zs(e, a, s), s;
};
let ye = class extends _ {
  ...
  _isAlternativeDisplay(t) {
    const e = st(t).toLowerCase();
    return ["svg", "svg+xml", "png"].includes(e);
  }
  /**
   ...
   */
  _getPreviewUrl(t) {
    const e = Z(t.file_type), a = t.file_uri_cdn;
    if (!a) return "";
    if (e === "video")
      try {
        const r = new URL(a);
        return r.searchParams.set("w", "200"), r.searchParams.set("force_format", "webp,jpeg"), r.toString();
      } catch {
        return a;
      }
    if (t.file_type === "application/pdf" || st(t.file_type).toLowerCase() === "pdf")
      try {
        let r = a.replace(/([?&])func=proxy&?/, "$1").replace(/[?&]$/, "");
        const s = new URL(r);
        return s.searchParams.set("w", "200"), s.searchParams.set("force_format", "webp,jpeg"), s.searchParams.set("doc_page", "1"), s.searchParams.set("bypass_process_proxy", "1"), s.toString();
      } catch {
        return a;
      }
    return a;
  ...
  _renderPreviewImg(t) {
    const e = this._isAlternativeDisplay(t.file_type), a = this._getPreviewUrl(t), r = ke(st(t.file_type)), s = te();
    return n`<div class="preview-slot ${e ? "icon-bg" : ""}">
      <img
        ...
        loading="lazy"
        @error=${(o) => {
      const i = o.target, l = i.parentElement;
      i.src !== r && i.src !== s ? (i.src = r, i.classList.add("icon-mode"), l == null || l.classList.add("icon-bg")) : i.src !== s && (i.src = s, i.classList.add("icon-mode"), l == null || l.classList.add("icon-bg"));
    }}
        @load=${(o) => {
      const i = o.target, l = i.parentElement;
      i.src.includes("scaleflex/assets/file-types") && (i.classList.add("icon-mode"), l == null || l.classList.add("icon-bg"));
    }}
      />
    </div>`;
  }
    ...
    return t.length === 1 ? n`<div class="preview-left">${this._renderPreviewImg(t[0])}</div>` : n`
      <div class="preview-grid">
        ...
        <div class="preview-right">
          ...
          ${t[2] ? n`<div class="preview-right-item">${this._renderPreviewImg(t[2])}</div>` : m}
        </div>
      </div>
    `;
  }
  render() {
    ...
    const t = this.folder;
    if (!t) return n``;
    const e = ((s = t.count) == null ? void 0 : s.files_direct) ?? 0, r = (((o = t.count) == null ? void 0 : o.files_recursive) ?? 0) - e;
    return n`
      ...
      <div class="info">
        ...
        <div class="counts">
          ...
          ${r > 0 ? n`<span>${r} sub-asset${r !== 1 ? "s" : ""}</span>` : m}
        </div>
      </div>
    `;
  }
...
ft([
  u({ type: Object })
], ye.prototype, "folder", 2);
ft([
  u({ type: Array })
], ye.prototype, "previews", 2);
ye = ft([
  y("ap-folder-card")
], ye);
var js = Object.defineProperty, Ns = Object.getOwnPropertyDescriptor, Bt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Ns(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && js(e, a, s), s;
};
let Le = class extends _ {
  _handleOpen() {
    this.dispatchEvent(new CustomEvent("folder-open", {
  ...
  render() {
    ...
    return t ? n`
      <div class="row" @click=${this._handleOpen}>
        ...
        <div class="meta">${((e = t.count) == null ? void 0 : e.files_recursive) ?? ((a = t.count) == null ? void 0 : a.files_direct) ?? 0} items</div>
        <div class="meta">${De(t.modified_at || t.updated_at || "")}</div>
        <div></div>
      </div>
    ` : n``;
  }
...
var Us = Object.defineProperty, Bs = Object.getOwnPropertyDescriptor, Vt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Bs(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Us(e, a, s), s;
};
let ze = class extends _ {
  ...
  render() {
    return this.variant === "grid" ? n`
        <div class="grid-skeleton">
          ${Array.from({ length: 18 }, () => n`<div class="skeleton card-skel"></div>`)}
        </div>
      ` : n`
      ${Array.from({ length: 7 }, () => n`<div class="skeleton row-skel"></div>`)}
    `;
  }
};
ze.styles = b`
...
function Vs(t) {
  var s, o;
  const e = (s = t.info) == null ? void 0 : s.playlists;
  if (!(e != null && e.length)) return null;
  const a = (o = e[0]) == null ? void 0 : o.playlists;
  return a != null && a.length && a[0] || null;
}
var qs = Object.defineProperty, Gs = Object.getOwnPropertyDescriptor, D = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Gs(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && qs(e, a, s), s;
};
let O = class extends _ {
  constructor() {
    super(...arguments), this.asset = null, this.assets = [], this.selectedIds = [], this.containerToken = "", this.showMetadata = !0, this.metadataFields = [], this.regionalFilters = {}, this._isFullscreen = !1, this._fsImageLoaded = !1, this._openSections = /* @__PURE__ */ new Set(["file-info"]), this._hls = null;
  }
  connectedCallback() {
    super.connectedCallback(), this._onFullscreenChange = this._onFullscreenChange.bind(this), document.addEventListener("fullscreenchange", this._onFullscreenChange);
  }
  disconnectedCallback() {
    super.disconnectedCallback(), document.removeEventListener("fullscreenchange", this._onFullscreenChange), this._destroyHls();
  }
  _destroyHls() {
    this._hls && (this._hls.destroy(), this._hls = null);
  }
  updated(t) {
    super.updated(t), t.has("asset") && (this._destroyHls(), this.asset && Z(this.asset.type) === "video" && this._setupHls(this.asset));
  }
  async _setupHls(t) {
    var r, s;
    const e = Vs(t);
    if (!e || (await this.updateComplete, ((r = this.asset) == null ? void 0 : r.uuid) !== t.uuid)) return;
    const a = this._videoEl;
    if (a) {
      ...
      try {
        ...
        if (!o.isSupported() || ((s = this.asset) == null ? void 0 : s.uuid) !== t.uuid || !this.isConnected) return;
        const i = new o();
        this._hls = i, i.loadSource(e), i.attachMedia(a), i.on(o.Events.ERROR, (l, c) => {
          var d;
          c.fatal && (this._destroyHls(), a.src = ((d = t.url) == null ? void 0 : d.cdn) || "");
        });
      } catch {
      }
    }
  ...
  _fullscreen() {
    var e, a, r, s, o, i;
    if (((a = (e = this.asset) == null ? void 0 : e.extension) == null ? void 0 : a.toLowerCase()) === "pdf" && ((s = (r = this.asset) == null ? void 0 : r.url) != null && s.cdn)) {
      window.open(this.asset.url.cdn, "_blank", "noopener");
      return;
    }
    this._fsImageLoaded = !1, (i = (o = this.previewArea) == null ? void 0 : o.requestFullscreen) == null || i.call(o);
  }
  _select() {
    if (this.asset) {
   ...
   */
  _findNavTarget(t, e) {
    const a = e === 1 ? this.assets.length : -1;
    for (let r = t + e; r !== a; r += e)
      if (!this._isFullscreen || this._supportsFullscreen(this.assets[r]))
        return r;
    return -1;
  }
  _prev() {
  ...
  _getFullscreenImageUrl(t) {
    ...
    if (!e) return _e(t) || "";
    const a = Ie(e, this.containerToken);
    return Nt(a, {
  ...
  _getTagLabels(t) {
    return t ? Array.isArray(t) ? t : Object.values(t).map((e) => e.label) : [];
  }
  _toggleSection(t) {
    const e = new Set(this._openSections);
    e.has(t) ? e.delete(t) : e.add(t), this._openSections = e;
  }
  _renderAccordion(t, e, a) {
    if (a.length === 0) return m;
    const r = this._openSections.has(t);
    return n`
      <div class="accordion">
        <button
        ...
        <div class="accordion-content ${r ? "open" : ""}">
          ${a.map((s) => n`
            <div class="meta-row">
              <span class="meta-label" title=${s.label}>${s.label}</span>
              <span class="meta-value ${s.wrap ? "wrap" : ""}" title=${s.value}>${s.value}</span>
            </div>
          `)}
        </div>
      </div>
  ...
  _getFileInfoRows(t) {
    var a, r, s, o, i;
    const e = [];
    return e.push({ label: "File name", value: t.name }), t.extension && e.push({ label: "Type", value: t.extension.toUpperCase() }), (a = t.size) != null && a.bytes && e.push({ label: "Size", value: ut(t.size.bytes) }), (r = t.info) != null && r.img_w && e.push({ label: "Dimensions", value: As(t.info.img_w, t.info.img_h) }), (s = t.info) != null && s.video_w && e.push({ label: "Video resolution", value: `${t.info.video_w} x ${t.info.video_h}` }), (o = t.info) != null && o.duration && e.push({ label: "Duration", value: `${Math.round(t.info.duration)}s` }), (i = t.info) != null && i.color_space && e.push({ label: "Color space", value: t.info.color_space }), t.created_at && e.push({ label: "Created", value: De(t.created_at) }), t.modified_at && e.push({ label: "Modified", value: De(t.modified_at) }), e;
  }
  /**
  ...
  _resolveSlug(t, e) {
    var r;
    if (!((r = e.possible_values) != null && r.length) || !t.startsWith("@")) return t;
    const a = e.possible_values.find(
      (s) => s.internal_unique_value === t || s.api_value === t
    );
  ...
  _resolveFieldValue(t, e) {
    ...
    if (e.regionalVariantsGroupUuid && typeof t == "object" && t !== null && !Array.isArray(t)) {
      const r = this.regionalFilters[e.regionalVariantsGroupUuid];
      if (r) {
        const s = t[r];
        if (s != null && s !== "")
          return this._resolveFieldValue(s, { ...e, regionalVariantsGroupUuid: void 0 });
      }
      return "";
    }
    if ((a = e.possible_values) != null && a.length)
      return Array.isArray(t) ? t.map((s) => this._resolveSlug(String(s), e)).filter(Boolean).join(", ") : this._resolveSlug(String(t), e);
  ...
  _getAllMetadataRows(t) {
    var o, i, l, c;
    const e = [], a = this._buildFieldMap(), r = /* @__PURE__ */ new Set();
    if (this.metadataFields.length > 0) {
      const d = t;
      for (const p of this.metadataFields) {
        const h = d[p.key] ?? ((i = (o = t.info) == null ? void 0 : o.metadata) == null ? void 0 : i[p.key]) ?? ((l = t.meta) == null ? void 0 : l[p.key]), f = this._resolveFieldValue(h, p);
        if (!f) continue;
        r.add(p.key), p.slug && r.add(p.slug);
        const v = f.length > 50;
        e.push({ label: p.label, value: f, wrap: v });
      }
    }
    if (t.meta)
      for (const [d, p] of Object.entries(t.meta)) {
        if (r.has(d) || p == null || p === "") continue;
        const h = a.get(d);
        if (h) {
          const f = this._resolveFieldValue(p, h);
          if (!f) continue;
          r.add(d);
          const v = f.length > 50;
          e.push({ label: h.label, value: f, wrap: v });
        } else {
          const f = typeof p == "object" ? JSON.stringify(p) : String(p);
          if (!f || f === "{}" || f === "[]") continue;
          r.add(d);
          const v = d.charAt(0).toUpperCase() + d.slice(1).replace(/_/g, " "), A = f.length > 50;
          e.push({ label: v, value: f, wrap: A });
        }
      }
    const s = (c = t.info) == null ? void 0 : c.metadata;
  ...
  _renderTags(t) {
    const e = this._getTagLabels(t.tags);
    if (e.length === 0) return m;
    const a = this._openSections.has("tags");
    return n`
      <div class="accordion">
        <button
          class="accordion-trigger"
  ...
  render() {
    ...
    const t = this.asset;
    if (!t) return m;
    const e = Z(t.type), a = e === "image", r = e === "video", s = e === "audio", o = ((S = t.extension) == null ? void 0 : S.toLowerCase()) === "pdf", i = ke(t.extension || ""), l = Xe(t.extension || ""), c = _e(t), d = a ? c || Ie(((k = t.url) == null ? void 0 : k.cdn) || "", this.containerToken) : "", p = a ? this._getFullscreenImageUrl(t) : "", h = r && ((C = t.url) == null ? void 0 : C.cdn) || "", f = r && (((se = t.info) == null ? void 0 : se.video_thumbnail) || ((Ce = t.info) == null ? void 0 : Ce.preview)) || "", v = o && !a ? Ye(t) : "", A = this._getCurrentIndex(), L = A > 0 && this._findNavTarget(A, -1) >= 0, P = A >= 0 && this._findNavTarget(A, 1) >= 0, z = this.assets.length > 1;
    return n`
      <div class="panel">
        <div class="panel-header">
          <span class="panel-title" title=${t.name}>${t.name}</span>
          <div class="header-actions">
            <button class="icon-btn" @click=${this._fullscreen} ?disabled=${!a && !r && !s && !o} aria-label="Fullscreen">
        ...
        <div class="preview-area ${a && l ? "chess" : ""}">
          ${z ? n`
            ...
            <button class="nav-btn next" @click=${this._next} ?disabled=${!P} aria-label="Next">
          ...
          ${a ? n`
            <img
              ...
              @error=${(Ze) => {
      const W = Ze.target;
      W.src !== i && (W.src = i, W.classList.add("icon-fallback"));
    }}
            />
            <div class="fs-wrapper">
              ...
              <img
                ...
                src=${this._isFullscreen ? p : ""}
                alt=${t.name}
                @load=${this._onFsImageLoad}
              />
            </div>
          ` : m}
          ${r ? n`<video src=${h} poster=${f || m} controls></video>` : m}
          ${s ? n`<audio src=${((N = t.url) == null ? void 0 : N.cdn) || ""} controls></audio>` : m}
          ${o && !a ? n`
            <img
              src=${v || i}
              alt=${t.name}
      ...
      W.src !== i && (W.src = i, W.classList.add("icon-fallback"));
            ...
            />
          ` : m}
          ${!a && !r && !s && !o ? n`<img src=${i} alt=${t.name} class="icon-fallback" />` : m}
        </div>
...
D([
  u()
], O.prototype, "containerToken", 2);
D([
  u({ type: Boolean })
], O.prototype, "showMetadata", 2);
D([
  u({ type: Array })
], O.prototype, "metadataFields", 2);
D([
  u({ type: Object })
], O.prototype, "regionalFilters", 2);
D([
  $e(".preview-area")
], O.prototype, "previewArea", 2);
D([
  $e("video")
], O.prototype, "_videoEl", 2);
D([
  E()
], O.prototype, "_isFullscreen", 2);
D([
  E()
], O.prototype, "_fsImageLoaded", 2);
D([
  E()
], O.prototype, "_openSections", 2);
O = D([
  y("ap-preview-panel")
], O);
var Hs = Object.defineProperty, Ks = Object.getOwnPropertyDescriptor, qt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Ks(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Hs(e, a, s), s;
};
let Re = class extends _ {
  constructor() {
    super(...arguments), this.selectedAssets = [];
  }
  _confirm() {
      ...
      return n`
              <div class="thumb ${l && o ? "chess" : ""}" @click=${() => this._deselect(t.uuid)} title="Click to deselect ${t.name}">
                ${o ? n`<img
                      ...
                      alt=${t.name}
                      @error=${(d) => {
        const p = d.target, h = te();
        p.src !== i && p.src !== h ? p.src = i : p.src !== h && (p.src = h);
      }}
                    />` : n`<img
                      src=${i}
                      alt=${t.name}
...
qt([
  u({ type: Array })
], Re.prototype, "selectedAssets", 2);
Re = qt([
  y("ap-selection-bar")
], Re);
var Ys = Object.defineProperty, Xs = Object.getOwnPropertyDescriptor, vt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Xs(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Ys(e, a, s), s;
};
let xe = class extends _ {
  ...
  render() {
    return !this.active || this.rect.width < 5 || this.rect.height < 5 ? m : n`
      <div
        class="marquee"
...
xe = vt([
  y("ap-marquee-overlay")
], xe);
var Qs = Object.defineProperty, Ws = Object.getOwnPropertyDescriptor, gt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? Ws(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Qs(e, a, s), s;
};
const Zs = {
...
let we = class extends _ {
  ...
  render() {
    const t = Zs[this.name];
    if (!t) return m;
    const e = typeof t == "string" ? Ae`<path d=${t}></path>` : t.map((a) => {
      const { tag: r, attrs: s } = a;
      return r === "circle" ? Ae`<circle cx=${s.cx} cy=${s.cy} r=${s.r} fill=${s.fill ?? "none"}></circle>` : r === "rect" ? Ae`<rect width=${s.width} height=${s.height} x=${s.x} y=${s.y} rx=${s.rx ?? "0"} ry=${s.ry ?? "0"}></rect>` : Ae`<path d=${s.d}></path>`;
    });
    return n`
      <svg
...
var Js = Object.defineProperty, er = Object.getOwnPropertyDescriptor, Gt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? er(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && Js(e, a, s), s;
};
...
var tr = Object.defineProperty, ar = Object.getOwnPropertyDescriptor, Ht = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? ar(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && tr(e, a, s), s;
};
let Ne = class extends _ {
  constructor() {
    super(...arguments), this.checked = !1, this._handleHostClick = () => {
...
var sr = Object.defineProperty, rr = Object.getOwnPropertyDescriptor, Kt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? rr(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && sr(e, a, s), s;
};
...
var ir = Object.defineProperty, or = Object.getOwnPropertyDescriptor, fe = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? or(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && ir(e, a, s), s;
};
let X = class extends _ {
  constructor() {
    super(...arguments), this.value = "", this.label = "", this.variant = "default", this.options = [], this.open = !1, this._handleOutsideClick = (t) => {
      this.open && !t.composedPath().includes(this) && (this.open = !1);
    };
  }
  close() {
  ...
  render() {
    ...
    return n`
      ...
      (e) => n`
            ...
            >
              ${e.icon ? n`<ap-icon name=${e.icon} .size=${16}></ap-icon>` : ""}
              ${e.label}
            </button>
          `
    )}
  ...
  _toggle() {
    this.open = !this.open, this.open && this.dispatchEvent(new CustomEvent("dropdown-open", { bubbles: !0, composed: !0 }));
  }
  _select(t) {
    this.open = !1, this.dispatchEvent(new CustomEvent("change", { detail: { value: t }, bubbles: !0, composed: !0 }));
  }
  connectedCallback() {
    super.connectedCallback(), document.addEventListener("click", this._handleOutsideClick);
  }
...
var nr = Object.defineProperty, lr = Object.getOwnPropertyDescriptor, Yt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? lr(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && nr(e, a, s), s;
};
let Be = class extends _ {
  constructor() {
...
var cr = Object.defineProperty, dr = Object.getOwnPropertyDescriptor, Xt = (t, e, a, r) => {
  for (var s = r > 1 ? void 0 : r ? dr(e, a) : e, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = (r ? i(e, a, s) : i(s)) || s);
  return r && s && cr(e, a, s), s;
};
...
var pr = Object.defineProperty, ur = (t, e, a, r) => {
  for (var s = void 0, o = t.length - 1, i; o >= 0; o--)
    (i = t[o]) && (s = i(e, a, s) || s);
  return s && pr(e, a, s), s;
};
const mt = class mt extends _ {
  constructor() {
    super(), this._initFailed = !1, this._loadId = 0, this._loadMoreId = 0, this.store = Wt(), this.storeCtrl = new Zt(this, this.store), this.selectionCtrl = new Jt(this, this.store), this.infiniteScrollCtrl = new ea(this, () => this._loadMore()), this.marqueeCtrl = new ta(this, this.store);
  }
  connectedCallback() {
  ...
  updated(e) {
    ...
    const a = this.renderRoot.querySelector("#sentinel");
    if (a && a !== this._lastSentinel) {
      this._lastSentinel = a;
      const s = this.renderRoot.querySelector(".main-content");
      this.infiniteScrollCtrl.observe(a, s);
    }
    const r = this.renderRoot.querySelector(".main-content");
    r && r !== this._lastMarqueeContainer && (this._lastMarqueeContainer = r, this.marqueeCtrl.attach(r));
  }
  _initConfig(e) {
    this._initFailed = !1, this._initPromise = this._doInit(e).catch(() => {
      this._initFailed = !0;
    });
  }
  async _doInit(e) {
    this.store.setState({
      ...
      viewMode: e.defaultViewMode ?? "grid",
      sortBy: e.defaultSortBy ?? "created_at",
      sortDirection: e.defaultSortDirection ?? "desc",
      currentFolder: e.rootFolderUuid ?? null
    }), this.apiClient = new aa(e.auth, e.apiBase);
    try {
      if (e.auth.mode === "securityTemplate") {
        const l = await na(this.apiClient);
        this.apiClient.setSassKey(l), this.store.setState({ sassKey: l });
      }
      ...
      if (a.status === "fulfilled") {
        const { fields: l, regionalVariantGroups: c, regionalFilters: d, brandColor: p } = a.value;
        this.store.setState({
          metadataFields: l,
          regionalVariantGroups: c,
          regionalFilters: d,
          ...p ? { brandColor: p } : {}
        });
      }
      r.status === "fulfilled" && this.store.setState({ labels: r.value.labels || [] });
      const { pinnedFilters: s, pinnedMetadata: o } = ja(e.auth.projectToken ?? null), i = this.store.getState().filters;
      this.store.setState({
        filters: {
          ...i,
          pinned: s,
  ...
  async open() {
    ...
    this.store.setState({
      ...
      assets: [],
      folders: [],
      currentFolder: ((a = this.config) == null ? void 0 : a.rootFolderUuid) ?? null,
      currentFolderPath: "/",
      breadcrumb: [],
      selectedAssets: /* @__PURE__ */ new Map(),
      folderPreviews: {},
  ...
  async _loadData() {
    ...
    try {
      const v = a.activeTab;
      if (v === "assets") {
        const A = this._buildSearchNotation(), L = await Je(this.apiClient, {
          folder: a.currentFolderPath || "/",
          offset: 0,
          limit: a.limit,
          sort_by: a.sortBy,
        ...
        const P = ((r = L.files) == null ? void 0 : r.length) ?? 0, z = P >= a.limit, S = ((o = (s = L.base) == null ? void 0 : s.count) == null ? void 0 : o.files_recursive) ?? ((l = (i = L.base) == null ? void 0 : i.count) == null ? void 0 : l.files_direct) ?? 0, k = z ? S : P;
        this.store.setState({
          assets: L.files || [],
          folders: [],
          totalCount: k,
        ...
        const A = this._buildSearchNotation(), [L, P] = await Promise.all([
          ra(this.apiClient, {
            folderPath: a.currentFolderPath,
            q: a.searchQuery || void 0,
            recursive: a.searchQuery ? 1 : 0,
            sort_by: a.sortBy,
            sort_direction: a.sortDirection
          }),
          Je(this.apiClient, {
        ...
        if (z.length > 0)
          try {
            S = await ia(this.apiClient, z.map((k) => k.uuid));
          } catch {
          }
        if (e !== this._loadId) return;
        this.store.setState({
          ...
          folders: z,
          folderPreviews: S,
          totalCount: (((c = P.files) == null ? void 0 : c.length) ?? 0) >= a.limit ? ((p = (d = P.base) == null ? void 0 : d.count) == null ? void 0 : p.files_direct) ?? 0 : ((h = P.files) == null ? void 0 : h.length) ?? 0,
          totalFolderCount: L.total ?? z.length,
          offset: 0,
          hasMore: (((f = P.files) == null ? void 0 : f.length) ?? 0) >= a.limit,
          isLoading: !1
        });
      }
    } catch (v) {
      ...
      this.store.setState({ isLoading: !1 }), this.dispatchEvent(new CustomEvent("ap-error", {
        detail: { error: v, context: "loadData" },
        bubbles: !0,
        composed: !0
      }));
    }
  ...
  async _loadMore() {
    const e = this.store.getState();
    if (e.isLoading || !e.hasMore || !this.apiClient) return;
    const a = ++this._loadMoreId, r = e.offset + e.limit;
    this.store.setState({ isLoading: !0 });
    try {
      const s = this._buildSearchNotation(), o = await Je(this.apiClient, {
      ...
      if (a !== this._loadMoreId) return;
      const i = this.store.getState().assets, l = o.files || [], c = l.length >= e.limit, d = [...i, ...l];
      this.store.setState({
        assets: d,
        offset: r,
        hasMore: c,
        isLoading: !1,
        ...c ? {} : { totalCount: d.length }
      });
    } catch {
      if (a !== this._loadMoreId) return;
  ...
  _handleCancel(e) {
    var r, s;
    if (this.store.getState().isPreviewOpen) {
      this.store.setState({ isPreviewOpen: !1, previewAsset: null });
      return;
    }
    this.close(), (s = (r = this.config) == null ? void 0 : r.onCancel) == null || s.call(r), this.dispatchEvent(new CustomEvent("ap-cancel", {
      detail: { reason: e },
      bubbles: !0,
      composed: !0
    }));
  ...
  _handleViewChange(e) {
    this.store.setState({ viewMode: e.detail.mode }), this._scrollToTop();
  }
  _handleRegionalChange(e) {
    const { groupUuid: a, value: r } = e.detail, s = this.store.getState().regionalFilters;
    this.store.setState({
      regionalFilters: { ...s, [a]: r }
    });
  }
  _handleSortChange(e) {
    this.store.setState({
  ...
  _handleQuickSelect(e) {
    ...
    (s = (r = this.config) == null ? void 0 : r.onSelect) == null || s.call(r, [a]), this.dispatchEvent(new CustomEvent("ap-select", {
      detail: { assets: [a] },
      bubbles: !0,
      composed: !0
    })), this.close();
  ...
  _handleFolderOpen(e) {
    const a = e.detail.folder, r = this.store.getState(), s = a.path || `${r.currentFolderPath}${a.name}/`;
    this.store.setState({
      currentFolder: a.uuid,
      currentFolderPath: s,
      breadcrumb: [...r.breadcrumb, { uuid: a.uuid, name: a.name, path: s }],
      offset: 0,
      assets: [],
      folders: []
    }), this.selectionCtrl.resetRange(), this._loadData();
  ...
  _handleBreadcrumbNavigate(e) {
    const a = e.detail.uuid, r = this.store.getState(), s = a ? r.breadcrumb.findIndex((l) => l.uuid === a) : -1, o = a ? r.breadcrumb.slice(0, s + 1) : [], i = o.length > 0 ? o[o.length - 1].path : "/";
    this.store.setState({
      currentFolder: a || null,
      currentFolderPath: i,
      breadcrumb: o,
      offset: 0,
      assets: [],
  ...
  _handleSelectionDeselect(e) {
    const a = new Map(this.store.getState().selectedAssets);
    a.delete(e.detail.uuid), this.store.setState({ selectedAssets: a });
  }
  // ── Filter Handlers ──────────────────────────────────────────────────
  _isDateFilterKey(e) {
    return e === g.DATE || e === g.LICENSE_EXPIRY || e === B.DUE_DATE;
  }
  _handleFilterUpdate(e) {
    const { key: a, values: r, operator: s } = e.detail, i = { ...this.store.getState().filters };
    if (r === "" || r === null || r === void 0 || Array.isArray(r) && r.length === 0 || typeof r == "object" && !Array.isArray(r) && Object.values(r).every(
      (c) => c == null || c === "" || Array.isArray(c) && c.length === 0
    )) {
      const c = { ...i.applied };
      delete c[a], i.applied = c, i.pinned.includes(a) || (i.visible = i.visible.filter((d) => d !== a));
    } else {
      const c = { ...i.applied };
      this._isDateFilterKey(a) && typeof r == "object" && !Array.isArray(r) ? c[a] = {
        type: "date",
        field: r.field || "created",
        kind: r.kind || null,
        preset: r.preset || null,
        from: r.from || null,
        to: r.to || null
      } : a === g.IMAGE && typeof r == "object" && !Array.isArray(r) ? c[a] = r : c[a] = {
        type: "string",
        values: Array.isArray(r) ? r : [r],
        operator: s || ":"
      }, i.applied = c, i.visible.includes(a) || (i.visible = [...i.visible, a]);
    }
    this.store.setState({ filters: i, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._loadData();
  }
  _handleFilterRemove(e) {
    const a = e.detail.key, s = { ...this.store.getState().filters }, o = { ...s.applied };
    delete o[a], s.applied = o, s.pinned.includes(a) || (s.visible = s.visible.filter((i) => i !== a)), this.store.setState({ filters: s, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._loadData();
  }
  _handleMetadataFilterChange(e) {
    const { fieldKey: a, operator: r, values: s, metadataType: o } = e.detail, l = { ...this.store.getState().filters }, c = { ...l.metadata }, d = { ...c.applied };
    !s || Array.isArray(s) && s.length === 0 ? (delete d[a], c.applied = d, c.pinned.includes(a) || (c.visible = c.visible.filter((h) => h !== a))) : (d[a] = {
      type: "string",
      values: Array.isArray(s) ? s : [s],
      operator: r || ":",
      metadataType: o
    }, c.applied = d, c.visible.includes(a) || (c.visible = [...c.visible, a])), l.metadata = c, this.store.setState({ filters: l, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._loadData();
  }
  _handleMetadataFieldToggle(e) {
    const { fieldKey: a, visible: r } = e.detail, o = { ...this.store.getState().filters }, i = { ...o.metadata };
    if (r)
      i.visible.includes(a) || (i.visible = [...i.visible, a]);
    else {
      i.visible = i.visible.filter((c) => c !== a);
      const l = { ...i.applied };
      delete l[a], i.applied = l;
    }
    o.metadata = i, this.store.setState({ filters: o });
  }
  _handleMetadataFilterRemove(e) {
    const { fieldKey: a } = e.detail, s = { ...this.store.getState().filters }, o = { ...s.metadata }, i = { ...o.applied };
    delete i[a], o.applied = i, o.pinned.includes(a) || (o.visible = o.visible.filter((l) => l !== a)), s.metadata = o, this.store.setState({ filters: s, offset: 0, assets: [], folders: [] }), this.selectionCtrl.resetRange(), this._loadData();
  }
  _handleFilterPin(e) {
    var l, c;
    const { key: a, pinned: r } = e.detail, o = { ...this.store.getState().filters };
    r ? o.pinned.includes(a) || (o.pinned = [...o.pinned, a]) : (o.pinned = o.pinned.filter((d) => d !== a), a in o.applied || (o.visible = o.visible.filter((d) => d !== a))), this.store.setState({ filters: o });
    const i = ((c = (l = this.store.getState().config) == null ? void 0 : l.auth) == null ? void 0 : c.projectToken) ?? null;
    Na(i, o.pinned);
  }
  _handleMetadataPin(e) {
    var c, d;
    const { fieldKey: a, pinned: r } = e.detail, o = { ...this.store.getState().filters }, i = { ...o.metadata };
    r ? i.pinned.includes(a) || (i.pinned = [...i.pinned, a]) : (i.pinned = i.pinned.filter((p) => p !== a), a in i.applied || (i.visible = i.visible.filter((p) => p !== a))), o.metadata = i, this.store.setState({ filters: o });
    const l = ((d = (c = this.store.getState().config) == null ? void 0 : c.auth) == null ? void 0 : d.projectToken) ?? null;
    Ua(l, i.pinned);
  }
  ...
  _buildSearchNotation() {
    const e = this.store.getState();
    return Pa(e.filters.applied, e.filters.metadata.applied).join(" ");
  }
  render() {
    var s, o;
    const e = this.storeCtrl.state, a = Array.from(e.selectedAssets.keys()), r = this.selectionCtrl.getSelectedAssets();
    return n`
      <ap-modal
        ?open=${e.isOpen}
      ...
      >
        <div slot="header">
          <ap-header
            ...
            .hiddenTabs=${((s = this.config) == null ? void 0 : s.hiddenTabs) ?? []}
            .viewMode=${e.viewMode}
  ...
  _renderContent(e, a) {
    return e.activeTab === "assets" ? !e.isLoading && e.assets.length === 0 ? n`
        ...
        ></ap-list-view>
        <div id="sentinel"></div>
      ` : e.activeTab === "folders" ? !e.isLoading && e.folders.length === 0 && e.assets.length === 0 ? n`
          <div class="empty-state">
