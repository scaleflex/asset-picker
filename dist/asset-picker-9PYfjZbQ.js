import { css as S, LitElement as C, html as n, nothing as v, svg as Ie } from "lit";
import { property as p, query as Ze, customElement as A, state as _ } from "lit/decorators.js";
class Ei {
  constructor(e) {
...
class Mi {
  ...
  _applyRangeSelection(e, s, i, a) {
    const o = this._getCombinedList();
    for (let r = e; r <= s; r++) {
      const l = o[r];
      l && (Oi(l) ? i.set(l.uuid, l) : a.set(l.uuid, l));
    }
  }
  handleSelect(e, s, i) {
    var l, c, h, u;
    const a = this.store.getState(), o = new Map(a.selectedAssets);
    ...
    if (i.shiftKey && this.lastClickedIndex >= 0)
      if (this._folderSelectionEnabled) {
        const g = Math.min(this.lastClickedIndex, s), d = Math.max(this.lastClickedIndex, s), f = new Map(a.selectedFolders);
        this._applyRangeSelection(g, d, o, f);
        const b = (c = a.config) == null ? void 0 : c.maxSelections;
        if (b && o.size + f.size > b) return;
        this.store.setState({ selectedAssets: o, selectedFolders: f });
      } else {
        const g = Math.min(this.lastClickedIndex, s), d = Math.max(this.lastClickedIndex, s), f = a.assets;
        for (let x = g; x <= d; x++)
          f[x] && o.set(f[x].uuid, f[x]);
      ...
      const g = (u = a.config) == null ? void 0 : u.maxSelections, d = o.size + (this._folderSelectionEnabled ? a.selectedFolders.size : 0);
      if (g && d > g) return;
      this.store.setState({ selectedAssets: o });
    } else
  ...
  handleFolderSelect(e, s, i) {
    ...
    const a = this.store.getState(), o = new Map(a.selectedFolders);
    if (!(((l = a.config) == null ? void 0 : l.multiSelect) ?? !0)) {
      o.clear(), o.set(e.uuid, e), this.store.setState({ selectedFolders: o, selectedAssets: /* @__PURE__ */ new Map() }), this.lastClickedIndex = s;
      return;
    }
    if (i.shiftKey && this.lastClickedIndex >= 0) {
      const u = Math.min(this.lastClickedIndex, s), g = Math.max(this.lastClickedIndex, s), d = new Map(a.selectedAssets);
      this._applyRangeSelection(u, g, d, o);
      const f = (c = a.config) == null ? void 0 : c.maxSelections;
      if (f && d.size + o.size > f) return;
      this.store.setState({ selectedAssets: d, selectedFolders: o });
    } else if (i.metaKey || i.ctrlKey) {
      o.has(e.uuid) ? o.delete(e.uuid) : o.set(e.uuid, e);
  ...
  selectAll(e) {
    var l;
    const s = this.store.getState(), i = (l = s.config) == null ? void 0 : l.maxSelections, a = this._folderSelectionEnabled ? s.selectedFolders.size : 0, o = /* @__PURE__ */ new Map(), r = i ? Math.max(0, Math.min(e.length, i - a)) : e.length;
    for (let c = 0; c < r; c++)
      o.set(e[c].uuid, e[c]);
  ...
  selectAllFolders(e) {
    var r;
    if (!this._folderSelectionEnabled) return;
    const s = this.store.getState(), i = (r = s.config) == null ? void 0 : r.maxSelections, a = /* @__PURE__ */ new Map(), o = i ? Math.max(0, Math.min(e.length, i - s.selectedAssets.size)) : e.length;
    for (let l = 0; l < o; l++)
      a.set(e[l].uuid, e[l]);
    this.store.setState({ selectedFolders: a });
  }
  clearSelection() {
...
class Pi {
  ...
  updateMarqueeRect(e) {
    ...
    let o = Math.min(this.startX, i), r = Math.min(this.startY, a), l = Math.abs(i - this.startX), c = Math.abs(a - this.startY);
    const h = this.container.scrollWidth, u = this.container.scrollHeight;
    o + l > h && (l = h - o), r + c > u && (c = u - r), o < 0 && (l += o, o = 0), r < 0 && (c += r, r = 0), this.rect = { x: o, y: r, width: l, height: c }, this.host.requestUpdate(), this.selectIntersecting();
  }
  startAutoScroll(e) {
    if (!this.container) return;
    ...
    const s = this.container.getBoundingClientRect(), i = e.clientY - s.top, a = s.bottom - e.clientY, o = i < ot || a < ot;
    o && !this._scrollRAF ? this._scrollRAF = requestAnimationFrame(() => this.autoScrollTick()) : o || this.stopAutoScroll();
  }
  autoScrollTick() {
    ...
    let o = 0;
    i < ot ? o = -Ys * (1 - i / ot) : a < ot && (o = Ys * (1 - a / ot)), o !== 0 && (this.container.scrollTop += o, this.updateMarqueeRect(s), this._scrollRAF = requestAnimationFrame(() => this.autoScrollTick()));
  }
  stopAutoScroll() {
  ...
  querySelectableElements() {
    ...
    for (const i of Array.from(this.container.children))
      if (i.shadowRoot) {
        ...
        const o = i.shadowRoot.querySelectorAll("[data-folder-uuid]");
        o.length > 0 && s.push(...Array.from(o));
      }
  ...
  selectIntersecting() {
    ...
    i.forEach((g) => {
      const d = this.getCardRelativeRect(g), f = g.dataset.assetUuid;
      if (this.intersectsMarquee(d)) {
        const b = e.assets.find((x) => x.uuid === f);
        b && o.set(f, b);
      }
    }), s && a.forEach((g) => {
      const d = this.getCardRelativeRect(g), f = g.dataset.folderUuid;
      if (this.intersectsMarquee(d)) {
        const b = e.folders.find((x) => x.uuid === f);
        b && r.set(f, b);
      }
    });
    const l = (u = e.config) == null ? void 0 : u.maxSelections, c = o.size + (s ? r.size : 0);
    if (l && c > l) {
      const g = Array.from(o.entries()).slice(0, Math.max(0, l - r.size));
      this.store.setState({
        selectedAssets: new Map(g),
        ...s ? { selectedFolders: r } : {}
      });
    } else
      this.store.setState({
...
class Di {
  ...
  async request(e, s) {
    ...
    const o = new AbortController(), r = setTimeout(() => o.abort(), 3e4);
    try {
      const l = await fetch(i.toString(), { headers: a, signal: o.signal });
      ...
      if (c.status === "error")
        throw new Error(`API error: ${c.msg || "Unknown error"}`);
      return c;
    } catch (l) {
      throw clearTimeout(r), l instanceof DOMException && l.name === "AbortError" ? new Error("API request timed out") : l;
    }
  }
  async post(e, s) {
...
function zi(t) {
  ...
  }, s = t.sort_by ?? "created_at", i = t.sort_direction ?? "desc", a = {
  ...
  e.sort = `${a[s] || s}:${i}`, t.folder && (e.folder = t.folder);
  const o = [];
  return t.search && !t.with_ai && o.push(t.search), t.q && o.push(t.q), o.length > 0 && (e.q = o.join(" ")), t.with_ai && (e.with_ai = !0, t.ai_query && (e.ai_query = t.ai_query), t.ai_lang && (e.ai_lang = t.ai_lang)), t.collection_uuid && (e.collection_uuid = t.collection_uuid), t.f64 && (e.f64 = t.f64), e;
}
...
async function Ri(t, e) {
  ...
  e != null && e.q && (s.q = e.q);
  const i = (e == null ? void 0 : e.sort_by) ?? "created_at", a = (e == null ? void 0 : e.sort_direction) ?? "asc", o = {
    created_at: "created_at",
    modified_at: "modified_at",
    name: "name"
  };
  return s.sort = `${o[i] || i}:${a}`, await t.request("/folders", s);
}
...
function oi(t) {
  return t.filter((e) => !!e.name).map((e) => ({ ...e, children: oi(e.children ?? []) })).filter((e) => {
    var s;
...
async function Vi(t) {
  ...
  return (await t.request(`/key/${e}`)).key;
}
function Yt(t) {
  var a, o;
...
const Gi = {
  ...
  png: "png",
  gif: "agif,gif"
}, Xi = /* @__PURE__ */ new Set(["jpg", "jpeg", "png", "gif", "webp", "svg", "bmp", "ico", "tiff", "tif", "avif", "heic", "heif", "psd", "eps"]), Zi = /* @__PURE__ */ new Set([
  ...Xi,
  "pdf",
  "psd",
  "eps",
...
const m = "https://scaleflex.cloudimg.io/v7/assets/file-types/v3/", vs = {
  _default: m + "GENERIC.svg?vh=9a518a",
  // Images
  png: m + "PNG.svg?vh=96cd9a",
  jpg: m + "JPG.svg?vh=06e819",
  jpg2: m + "JPG2.svg?vh=f0eb7f",
  jpeg: m + "JPEG.svg?vh=6a65e9",
  gif: m + "GIF.svg?vh=c3c2c3",
  bmp: m + "BMP.svg?vh=d2243a",
  webp: m + "WEBP.svg?vh=fedd74",
  svg: m + "SVG.svg?vh=a15e46",
  tiff: m + "TIFF.svg?vh=1f30c3",
  tif: m + "TIF.svg?vh=b383c9",
  heic: m + "HEIC.svg?vh=84adfe",
  avif: m + "AVIF.svg?vh=536b30",
  ico: m + "ICO.svg?vh=79063d",
  psd: m + "PSD.svg?vh=be6140",
  psb: m + "PSB.svg?vh=678646",
  ai: m + "AI.svg?vh=84b254",
  dwg: m + "DWG.svg?vh=971fb3",
  // Video
  mp4: m + "MP4.svg?vh=42f175",
  webm: m + "WEBM.svg?vh=26a84a",
  avi: m + "AVI.svg?vh=d22ba8",
  mpeg: m + "MPEG.svg?vh=ba93bb",
  ogv: m + "OGV.svg?vh=74d453",
  "3gp": m + "3GP.svg?vh=f0d388",
  "3g2": m + "3G2.svg?vh=04c652",
  swf: m + "SWF.svg?vh=3955e2",
  fla: m + "FLA.svg?vh=daf585",
  m3u8: m + "M3U8.svg?vh=7d5e62",
  // Audio
  mp3: m + "MP3.svg?vh=66bbef",
  wav: m + "WAV.svg?vh=d7a7d5",
  aac: m + "AAC.svg?vh=07f3f9",
  oga: m + "OGA.svg?vh=a5c622",
  opus: m + "OPUS.svg?vh=9548b1",
  weba: m + "WEBA.svg?vh=4dcf70",
  mid: m + "MID.svg?vh=3f0e29",
  midi: m + "MIDI.svg?vh=9fedec",
  cda: m + "CDA.svg?vh=85b83b",
  // Documents
  pdf: m + "PDF.svg?vh=18c5f7",
  doc: m + "DOC.svg?vh=d1b47c",
  docx: m + "DOCX.svg?vh=1eb6b0",
  txt: m + "TXT.svg?vh=307979",
  rtf: m + "RTF.svg?vh=978c5f",
  xls: m + "XLS.svg?vh=13b5f7",
  xlsx: m + "XLSX.svg?vh=79d64a",
  ppt: m + "PPT.svg?vh=4ee29b",
  pptx: m + "PPTX.svg?vh=8b1568",
  csv: m + "CSV.svg?vh=4add78",
  odt: m + "ODT.svg?vh=940781",
  ods: m + "ODS.svg?vh=9fbe9a",
  odp: m + "ODP.svg?vh=bf892d",
  dbf: m + "DBF.svg?vh=457bd4",
  vsd: m + "VSD.svg?vh=8a9ccb",
  abw: m + "ABW.svg?vh=313dc7",
  epub: m + "EPUB.svg?vh=15263d",
  azw: m + "AZW.svg?vh=a018b1",
  ics: m + "ICS.svg?vh=909f63",
  ogx: m + "OGX.svg?vh=f694d2",
  // Archives
  zip: m + "ZIP.svg?vh=84f98b",
  rar: m + "RAR.svg?vh=1d6423",
  "7z": m + "7Z.svg?vh=e007e5",
  tar: m + "TAR.svg?vh=603aed",
  gz: m + "GZ.svg?vh=de13f7",
  bz: m + "BZ.svg?vh=0374ff",
  bz2: m + "BZ2.svg?vh=e14294",
  arc: m + "ARC.svg?vh=942fad",
  jar: m + "JAR.svg?vh=149796",
  mpkg: m + "MPKG.svg?vh=dea655",
  // Fonts
  ttf: m + "TTF.svg?vh=d2e2c1",
  otf: m + "OTF.svg?vh=c904fd",
  woff: m + "WOFF.svg?vh=4b8177",
  woff2: m + "WOFF2.svg?vh=b532d3",
  eot: m + "EOT.svg?vh=a54980",
  // Code / Scripts
  js: m + "JS.svg?vh=524691",
  mjs: m + "MJS.svg?vh=d57921",
  ts: m + "TS.svg?vh=9af3ae",
  css: m + "CSS.svg?vh=287863",
  html: m + "HTML.svg?vh=fa7a87",
  htm: m + "HTM.svg?vh=21323d",
  xhtml: m + "XHTML.svg?vh=e6d6a9",
  xul: m + "XUL.svg?vh=6c9c71",
  json: m + "JSON.svg?vh=104c9e",
  jsonld: m + "JSONLD.svg?vh=f30c0f",
  xml: m + "XML.svg?vh=7f7194",
  php: m + "PHP.svg?vh=503e36",
  sh: m + "SH.svg?vh=3b820e",
  csh: m + "CSH.svg?vh=08c0cc",
  // Executables / Disk images
  exe: m + "EXE.svg?vh=ccca53",
  iso: m + "ISO.svg?vh=064b8f",
  bin: m + "BIN.svg?vh=1e9618"
};
function Tt(t) {
  const e = (t == null ? void 0 : t.toLowerCase().replace(".", "")) || "";
  return vs[e] || vs._default;
}
function ct() {
  return vs._default;
}
...
const ea = /* @__PURE__ */ new Set(["svg", "svg+xml", "png", "pdf"]);
function ni(t) {
  return ea.has(Wt(t).toLowerCase());
}
function Bt(t) {
  var i;
...
function xs(t) {
  var s, i;
  let e = Ge(((s = t.url) == null ? void 0 : s.cdn) || ((i = t.url) == null ? void 0 : i.public) || "");
  return e ? (e = e.replace(/([?&])func=proxy&?/, "$1").replace(/[?&]$/, ""), Pe(e, {
    w: "200",
...
function ci(t, e) {
  ...
  if (!s) return "";
  let i = Ge(s);
  const a = lt(t.file_type), o = String(window.devicePixelRatio || 1);
  return a === "video" ? Pe(i, { w: e, dpr: o, force_format: "webp,jpeg" }) : t.file_type === "application/pdf" || Wt(t.file_type).toLowerCase() === "pdf" ? (i = i.replace(/([?&])func=proxy&?/, "$1").replace(/[?&]$/, ""), Pe(i, { w: e, dpr: o, force_format: "webp,jpeg", doc_page: "1", bypass_process_proxy: "1" })) : Pe(i, { w: e, dpr: o });
}
function di(t, e) {
  const s = {};
...
function ln(t, e, s) {
  ...
  if (!i) return t;
  const a = di(e, s);
  return Object.keys(a).length > 0 ? Pe(i, a) : i;
}
const y = {
  TYPE: "type",
  MIME_TYPE: "mimetype",
  ...
  [k.NUMERIC]: "num_",
  [k.DECIMAL2]: "dec_",
  [k.ATTACHMENTS_ASSETS]: "attach_",
  [k.ATTACHMENT_URI]: "uri_",
  [k.SELECT_ONE]: "one_",
  [k.MULTI_SELECT]: "multi_",
  [k.SUPERTAGS]: "tags_",
  [k.TEXT]: "text_",
  [k.TEXT_AREA]: "area_",
  [k.GEO_POINT]: "geo_",
  [k.INTEGER_LIST]: "intlist_"
};
Object.fromEntries(
  Object.entries(Xt).map(([t, e]) => [e, t])
);
const pi = Object.values(Xt), bt = {
  PRESET: "preset",
  AFTER: "after",
  BEFORE: "before",
  BETWEEN: "between",
  ...
  EMPTY: "empty",
  NOT_EMPTY: "non-empty"
}, pe = "empty", he = "non-empty", Be = "specific", as = 10, os = 10, zt = 10, hi = "filters:pinned:", ui = "metadata:pinned:", Zs = [
  y.DATE,
  y.TYPE,
  y.SIZE
], fi = T.IS, jt = "20", Nt = "1", sa = 0, ia = [
  ...
  { key: y.LICENSE_EXPIRY, label: "License expiry", icon: "history", type: "date" },
  { key: y.TAGS, label: "Tags", icon: "hash", type: "string" },
  { key: y.LABELS, label: "Labels", icon: "tag", type: "string" },
  { key: y.PRODUCT_REF, label: "Products", icon: "box", type: "string" },
  { key: y.IMAGE, label: "Image", icon: "image", type: "string" },
  { key: y.COLOR, label: "Color", icon: "swatch-book", type: "string" }
], aa = {
  key: y.METADATA,
  label: "Metadata",
  icon: "file-text",
  ...
  [y.DATE]: "Date",
  [y.SIZE]: "Size",
  [y.LICENSE_EXPIRY]: "License expiry",
  [y.TAGS]: "Tags",
  [y.LABELS]: "Labels",
  [y.PRODUCT_REF]: "Products",
  [y.IMAGE]: "Image",
  [y.COLOR]: "Color",
  [y.METADATA]: "Metadata",
  [y.APPROVAL]: "Approval"
  ...
  { value: Z.IMAGE, label: "Image", icon: "image" },
  { value: Z.VIDEO, label: "Video", icon: "clapperboard" },
  { value: Z.AUDIO, label: "Audio", icon: "music" },
  { value: Z.DOCUMENT, label: "Document", icon: "document" },
  { value: Z.ARCHIVE, label: "Archive", icon: "archive" },
  { value: Z.DESIGN_TEMPLATE, label: "Design template", icon: "layout-template" },
  { value: Z.FONTS, label: "Fonts", icon: "text" },
  { value: Z.OTHER, label: "Other", icon: "layout-grid" }
], ra = {
  [Z.IMAGE]: ["jpg", "jpeg", "png", "gif", "webp", "svg", "bmp", "ico", "tiff", "tif", "avif", "heic", "heif", "psd", "eps"],
  [Z.VIDEO]: ["mp4", "webm", "ogg", "mov", "avi", "mkv", "flv", "wmv", "m4v"],
  [Z.AUDIO]: ["mp3", "wav", "ogg", "flac", "aac", "wma", "m4a", "opus"],
  [Z.DOCUMENT]: ["pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "txt", "csv", "rtf", "odt", "ods", "odp", "html"],
  [Z.ARCHIVE]: ["zip", "rar", "7z", "tar", "gz", "bz2", "xz"],
  [Z.FONTS]: ["ttf", "otf", "woff", "woff2", "eot"]
}, rs = {
  RESOLUTION: 0,
  ORIENTATION: 1,
  FACES: 2
  ...
  { label: "Is", value: T.IS },
  { label: "Contains", value: T.CONTAINS_IN_TEXT }
], Se = [
  { label: "Is", value: T.IS },
  { label: "Is not", value: T.IS_NOT }
], ls = [
  { label: "Is", value: T.IS_EXACT },
  { label: "Contains", value: T.IS },
  { label: "Does not contain", value: T.IS_NOT }
], cs = [
  { label: "Is", value: T.IS },
  { label: "Greater than", value: T.GREATER_THAN_OR_EQUAL },
  { label: "Is not", value: T.IS_NOT },
  { label: "Less than", value: T.LESS_THAN_OR_EQUAL },
  { label: "Between", value: T.RANGE }
], ua = [
  { label: "Is", value: T.IS_EXACT },
  { label: "Is not", value: T.IS_NOT }
], Js = [
  { label: "Contains", value: T.CONTAINS },
  { label: "Does not contain", value: T.IS_NOT }
], fa = [
  ...
  { label: "Empty", value: pe },
  { label: "Not empty", value: he }
], ds = [
  ...Ve,
  ...
  [k.DATE]: "calendar",
  [k.BOOLEAN]: "toggle-left",
  [k.NUMERIC]: "file-digit",
  [k.DECIMAL2]: "decimals-arrow-right",
  [k.ATTACHMENTS_ASSETS]: "paperclip",
  [k.ATTACHMENT_URI]: "link-2",
  [k.SELECT_ONE]: "circle-dot",
  [k.MULTI_SELECT]: "square-check",
  [k.SUPERTAGS]: "hash",
  [k.TEXT]: "text-initial",
  [k.TEXT_AREA]: "text-align-justify",
  [k.GEO_POINT]: "locate",
  [k.INTEGER_LIST]: "list-ordered"
}, va = {
  [k.TEXT]: "text",
  [k.TEXT_AREA]: "text",
  [k.NUMERIC]: "number",
  ...
  [k.MULTI_SELECT]: "multi-select",
  [k.SUPERTAGS]: "tags",
  [k.DATE]: "date",
  [k.GEO_POINT]: "geo",
  [k.ATTACHMENTS_ASSETS]: "attachment",
  [k.ATTACHMENT_URI]: "attachment",
  [k.INTEGER_LIST]: "attachment"
};
async function ma(t) {
  var i;
...
async function _a(t) {
  var b, x;
  const e = await t.request("/settings"), s = e.settings ?? e, i = (s == null ? void 0 : s.metadata) ?? e.metadata, o = ((i == null ? void 0 : i.model) ?? [])[sa], r = i == null ? void 0 : i.store, c = ((r == null ? void 0 : r.regional_variants_groups) ?? []).map((w) => ({
    uuid: w.uuid,
    label: w.label ?? w.name ?? "",
    variants: (w.variants ?? []).map((L) => ({
      api_value: L.api_value ?? L.value ?? "",
      internal_unique_value: L.internal_unique_value ?? L.api_value ?? "",
      label: L.label ?? L.name ?? ""
    }))
  })).filter((w) => w.variants.length > 1), h = {};
  for (const w of c)
    h[w.uuid] = ((b = w.variants[0]) == null ? void 0 : b.api_value) ?? "";
  const u = (s == null ? void 0 : s.project_branding) ?? ((x = e.settings) == null ? void 0 : x.project_branding), g = u == null ? void 0 : u.brandColor;
  if (!o) return { fields: [], regionalVariantGroups: c, regionalFilters: h, brandColor: g };
  const d = o.groups ?? [], f = [];
  for (const w of d) {
    const L = w.name || "Root fields";
    for (const $ of w.fields ?? []) {
      const R = $.possible_values, F = R == null ? void 0 : R.map((E) => ({
      ...
      f.push({
        ...
        possible_values: F,
        group: L,
        regionalVariantsGroupUuid: $.regional_variants_group_uuid
      });
    }
  }
  return { fields: f, regionalVariantGroups: c, regionalFilters: h, brandColor: g };
}
function qe(t) {
  return t.toLocaleDateString("en-CA");
}
function Rt() {
  return qe(/* @__PURE__ */ new Date());
}
function mi(t) {
  ...
  switch (t) {
    ...
    case "last_week": {
      const s = new Date(e);
      return s.setDate(s.getDate() - 7), { from: qe(s), to: null };
    }
    case "within_week": {
      const s = new Date(e);
      return s.setDate(s.getDate() + 7), { from: Rt(), to: qe(s) };
    }
    case "last_month": {
      const s = new Date(e);
      return s.setDate(s.getDate() - 30), { from: qe(s), to: null };
    }
    case "within_month": {
      const s = new Date(e);
      return s.setDate(s.getDate() + 30), { from: Rt(), to: qe(s) };
    }
    case "last_year": {
      const s = new Date(e);
      return s.setDate(s.getDate() - 365), { from: qe(s), to: null };
    }
    case "within_year": {
      const s = new Date(e);
      return s.setDate(s.getDate() + 365), { from: Rt(), to: qe(s) };
    }
    default:
      return null;
  }
...
function ei(t, e) {
  return e ? e.type === "date" ? ya(e) : e.type === "string" ? xa(e) : !0 : !1;
}
function ya(t) {
...
function ka(t, e) {
  ...
  if (t === y.IMAGE)
    return Ta(e);
  const s = e, { operator: i = fi, values: a = [], logic: o } = s;
...
function bi(t, e) {
  ...
  if (e.kind === bt.PRESET && e.preset) {
    if (e.preset === Xs.EMPTY)
      return [`${s}:"empty"`];
    if (e.preset === Xs.NOT_EMPTY)
      return [`${s}:"non-empty"`];
    const i = mi(e.preset);
    return i ? i.to === null ? [`${s}>"${i.from}"`] : [`${s}:"${i.from}..${i.to}"`] : [];
  }
  if (e.kind === bt.AFTER && e.from)
    return [`${s}:>"${e.from}"`];
  if (e.kind === bt.BEFORE && e.to)
    return [`${s}:<"${e.to}"`];
  if (e.kind === bt.BETWEEN) {
    if (e.from && e.to)
  ...
  return e.kind === bt.SPECIFIC && e.from ? [`${s}:"${e.from}"`] : [];
}
function Sa(t, e, s) {
...
function He(t, e, s, i, a) {
  if (s.length === 0) return [];
  if (a === ta.AND)
    return s.map((r) => `${t}${e}"${r}"`);
  const o = s.map((r) => `"${r}"`).join(i);
  return [`${t}${e}${o}`];
}
function Fa(t, e, s) {
  const i = [], a = He("color_search", t, e, ",", s);
...
function Aa(t, e) {
  ...
  const a = i.map((o) => `"${o}"`).join(",");
  return [`type${e}${a}`];
}
function Ea(t, e, s, i) {
  if (e.length === 0) return [];
  const a = e.map((o) => `"${o}"`).join(",");
  return [`${t}${s}${a}`];
}
function Ta(t) {
  const e = [];
  if (typeof t == "object" && t !== null && !Array.isArray(t)) {
    const r = t, l = r.resolution, c = r.orientation, h = r.faces;
  ...
  const s = t, i = s[rs.RESOLUTION], a = s[rs.ORIENTATION], o = s[rs.FACES];
  return i && e.push(`resolution:"${i}"`), a && e.push(`orientation:"${a}"`), o && e.push(`faces:"${o}"`), e;
}
function La(t, e) {
  const s = Ia(t), i = Ma(t);
  if (e.type === "date")
  ...
  if (r.length === 1 && (r[0] === pe || r[0] === he)) {
    const h = c === k.GEO_POINT ? T.IS + "~" : T.IS;
    return [`${s}${h}"${r[0]}"`];
  }
  if ((c === k.NUMERIC || c === k.DECIMAL2) && o === T.RANGE)
    return [`${s}${T.IS}"${r.join('","')}"`];
  if (c === k.GEO_POINT) {
    const h = r.map((u) => {
      const g = u.indexOf("..");
      if (g === -1) return `(${u})`;
      const d = u.slice(0, g), f = u.slice(g);
      return `(${d})${f}`;
    });
    return He(s, o + "~", h, ",", l);
  }
...
function Oa(t, e, s) {
  ...
  return !s || i.length === 0 ? i : i.map((a) => {
    const o = a.search(/[^a-zA-Z0-9_]/);
    if (o === -1) return `"${a}"`;
    const r = a.slice(0, o), c = a.slice(o).replace(/"/g, "");
    return `"${r}${c}"`;
  });
}
function Ma(t) {
...
function Da(t, e) {
  return e.type === "string" || e.type === "date" ? e : Pa.has(t) ? {
    type: "date",
...
function za(t) {
  const e = localStorage.getItem(hi + t), s = localStorage.getItem(ui + t);
  let i;
  try {
    i = e ? JSON.parse(e) : [...Zs];
  } catch {
...
function Ra(t, e) {
  localStorage.setItem(hi + t, JSON.stringify(e));
}
function ja(t, e) {
  localStorage.setItem(ui + t, JSON.stringify(e));
}
const _i = "sort-by", yi = "sort-order";
function si(t, e) {
  try {
    localStorage.setItem(_i, t), localStorage.setItem(yi, e);
  } catch {
  }
}
function Na() {
  try {
    const t = localStorage.getItem(_i), e = localStorage.getItem(yi);
    return { sortBy: t, sortDirection: e };
  } catch {
    return { sortBy: null, sortDirection: null };
  }
}
const xi = "ap-last-folder", wi = "ap-last-view", $i = "ap-last-tab";
function ft(t, e) {
  return e ? `${t}:${e}` : t;
}
function ii(t, e) {
  try {
    localStorage.setItem(ft(xi, e), t);
  } catch {
  }
}
function Ua(t) {
  try {
    return localStorage.getItem(ft(xi, t));
  } catch {
    return null;
  }
}
function Ba(t, e) {
  try {
    localStorage.setItem(ft(wi, e), t);
  } catch {
  }
}
function qa(t) {
  try {
    return localStorage.getItem(ft(wi, t));
  } catch {
    return null;
  }
}
function Va(t, e) {
  try {
    localStorage.setItem(ft($i, e), t);
  } catch {
  }
}
function Ha(t) {
  try {
    const e = localStorage.getItem(ft($i, t));
...
function Ga(t) {
  const e = t.replace("#", ""), s = e.length === 3 ? e[0] + e[0] + e[1] + e[1] + e[2] + e[2] : e, i = parseInt(s.slice(0, 2), 16) / 255, a = parseInt(s.slice(2, 4), 16) / 255, o = parseInt(s.slice(4, 6), 16) / 255;
  return [i, a, o];
}
function ps(t) {
...
function Ka(t, e, s) {
  const i = ps(t), a = ps(e), o = ps(s), r = 0.4122214708 * i + 0.5363325363 * a + 0.0514459929 * o, l = 0.2119034982 * i + 0.6806995451 * a + 0.1073969566 * o, c = 0.0883024619 * i + 0.2817188376 * a + 0.6299787005 * o, h = Math.cbrt(r), u = Math.cbrt(l), g = Math.cbrt(c), d = 0.2104542553 * h + 0.793617785 * u - 0.0040720468 * g, f = 1.9779984951 * h - 2.428592205 * u + 0.4505937099 * g, b = 0.0259040371 * h + 0.7827717662 * u - 0.808675766 * g, x = Math.sqrt(f * f + b * b);
  let w = Math.atan2(b, f) * (180 / Math.PI);
...
function Ya(t, e) {
  ...
  const [i, a, o] = Ga(e), [r, l, c] = Ka(i, a, o), h = `oklch(${r.toFixed(3)} ${l.toFixed(3)} ${c.toFixed(1)})`, u = Qa(r) ? "#000" : "#fff";
  t.style.setProperty("--ap-primary", h), t.style.setProperty("--ap-primary-foreground", u), t.style.setProperty("--ap-primary-10", `oklch(${r.toFixed(3)} ${l.toFixed(3)} ${c.toFixed(1)} / 0.1)`), t.style.setProperty("--ap-primary-20", `oklch(${r.toFixed(3)} ${l.toFixed(3)} ${c.toFixed(1)} / 0.2)`), t.style.setProperty("--ap-accent", `oklch(${r.toFixed(3)} ${l.toFixed(3)} ${c.toFixed(1)} / 0.07)`), t.style.setProperty("--ap-accent-foreground", h), t.style.setProperty("--ap-ring", `oklch(${r.toFixed(3)} ${l.toFixed(3)} ${c.toFixed(1)} / 0.7)`), t.style.setProperty("--ap-selection-bg", `oklch(${r.toFixed(3)} ${l.toFixed(3)} ${c.toFixed(1)} / 0.08)`);
}
const $s = [
...
var to = Object.defineProperty, so = Object.getOwnPropertyDescriptor, ks = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? so(e, s) : e, o = t.length - 1, r; o >= 0; o--)
...
`;
var ao = Object.defineProperty, oo = Object.getOwnPropertyDescriptor, Zt = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? oo(e, s) : e, o = t.length - 1, r; o >= 0; o--)
    (r = t[o]) && (a = (i ? r(e, s, a) : r(a)) || a);
...
let H = class extends C {
  ...
  render() {
    var e, s;
    const t = lo.filter((i) => this.tabs.includes(i.value));
...
te([
  p({ type: Boolean, reflect: !0, attribute: "ai-search" })
...
let yt = class extends C {
  ...
  render() {
    return this.items.length === 0 ? n`` : n`
...
var fo = Object.defineProperty, go = Object.getOwnPropertyDescriptor, Lt = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? go(e, s) : e, o = t.length - 1, r; o >= 0; o--)
    (r = t[o]) && (a = (i ? r(e, s, a) : r(a)) || a);
  return i && a && fo(e, s, a), a;
};
function ai(t) {
  var i;
  const e = /* @__PURE__ */ new Map();
  for (const a of t) {
    const o = a.category || a.value.split("_")[0];
    if (!e.has(o)) {
      const l = qt.find((c) => c.value === o);
      e.set(o, {
        key: o,
        label: (l == null ? void 0 : l.label) || o.charAt(0).toUpperCase() + o.slice(1),
        icon: (l == null ? void 0 : l.icon) || "file",
        extensions: [],
        totalCount: 0
      });
    }
    ...
    r.extensions.push({
      ...
      count: a.count
    }), r.totalCount += a.count;
  }
  const s = qt.map((a) => a.value);
  return [...e.values()].sort((a, o) => {
    const r = s.indexOf(a.key), l = s.indexOf(o.key);
...
let Ke = class extends C {
  ...
  _toggleGroup(t) {
    ...
    if (s) {
      ...
      i = this.selected.filter((o) => !a.has(o));
    } else {
      const a = new Set(e);
      i = this.selected.filter((o) => !a.has(o)), i.push(t.key);
    }
    this._emit(i);
  }
  _toggleExt(t, e) {
  ...
  updated(t) {
    if (t.has("fileTypes") && this.fileTypes.length > 0) {
      const e = ai(this.fileTypes), s = new Set(this._expanded);
      for (const i of e)
  ...
  render() {
    ...
    const t = ai(this.fileTypes), e = this._filteredGroups(t), s = this.selected.length > 0;
    return n`
      <input
        class="search-input"
        type="text"
      ...
      <div class="header-row">
        ...
        ${s ? n`<button class="clear-btn" @click=${this._clearAll}>Clear all</button>` : v}
      </div>
      <div class="groups">
...
`;
var vo = Object.defineProperty, mo = Object.getOwnPropertyDescriptor, Je = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? mo(e, s) : e, o = t.length - 1, r; o >= 0; o--)
    (r = t[o]) && (a = (i ? r(e, s, a) : r(a)) || a);
...
let Ce = class extends C {
  ...
  get _isLicenseExpiry() {
    return this.filterKey === y.LICENSE_EXPIRY;
  }
  get _rangeOptions() {
    return this._isLicenseExpiry ? vi : ws;
  }
  get _todayStr() {
    return hs(/* @__PURE__ */ new Date());
  }
  get _hasFilter() {
  ...
  _renderDateTypeSection() {
    return this._isLicenseExpiry ? v : n`
  ...
  _renderDateInputs() {
    const t = this.preset;
    if (!["before", "after", "between", "specific"].includes(t)) return v;
...
let De = class extends C {
  ...
  get _atLimit() {
    return this.selected.length >= as;
  }
  get _isSearching() {
  ...
  _renderTagItem(t) {
    ...
    return n`
      <div
        ...
        title=${s ? `You can select up to ${as} tags` : v}
  ...
  _renderSearchResults(t) {
    ...
    return s.length === 0 && i.length === 0 ? n`<div class="no-results">No tags found</div>` : n`
      ${s.length > 0 ? n`
            <div class="filter-section">
              <div class="section-header">
  ...
  _renderSections() {
    if (this.tags.length === 0)
      return n`<div class="no-results">No tags available</div>`;
    const t = this._suggestedTags.filter(
      (e) => !this.selected.includes(e.sid)
    );
    return n`
      <div class="filter-section">
        ${t.length > 0 ? n`<span class="section-label">Suggested tags</span>` : v}
        ${t.length > 0 ? n`
              <div class="options-list short">
      ...
      ${t.length > 0 ? n`<div class="separator"></div>` : v}
...
let Qe = class extends C {
  ...
  get _atLimit() {
    return this.selected.length >= os;
  }
  get _isSearching() {
    return this._search.trim().length > 0;
  }
  ...
  _renderLabelItem(t) {
    ...
    return n`
      <div
        ...
        title=${s ? `You can select up to ${os} labels` : v}
        @click=${() => !s && this._toggle(t.sid)}
      >
        <ap-checkbox ?checked=${e}></ap-checkbox>
...
var wo = Object.defineProperty, $o = Object.getOwnPropertyDescriptor, vt = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? $o(e, s) : e, o = t.length - 1, r; o >= 0; o--)
    (r = t[o]) && (a = (i ? r(e, s, a) : r(a)) || a);
  return i && a && wo(e, s, a), a;
};
const us = 3, _s = /^#[0-9A-Fa-f]{6}$/;
function ko(t) {
  const e = t.trim().split(/\s+/);
  return e.length < 1 || !_s.test(e[0]) ? null : {
    hex: e[0],
    tolerance: e[1] || Nt,
    coverage: e[3] || jt
  };
...
let ze = class extends C {
  ...
  willUpdate(t) {
    if (t.has("values")) {
      ...
      (!this._initialized || this.values.length === 0 && this._colors.length > 0) && (this._colors = e, e.length > 0 && (this._advanced = e.some(
        (s) => s.tolerance !== Nt || s.coverage !== jt
      )), this._initialized = !0);
    }
  }
  _selectColor(t) {
    ...
    this._colors.length >= us || (this._colors = [
      ...this._colors,
      {
        hex: t,
        tolerance: Nt,
  ...
  _handleHexInput(t, e) {
    ...
    if (i.startsWith("#") || (i = "#" + i), !_s.test(i) || this._colors.some(
      (r, l) => l !== t && r.hex.toLowerCase() === i.toLowerCase()
    )) return;
  ...
  _dispatchChange() {
    ...
    const t = this._colors.map((e) => {
      const s = this._advanced ? e.tolerance : Nt, i = this._advanced ? e.coverage : jt;
      return `${e.hex} ${s} ${s} ${i}`;
    });
    this.dispatchEvent(
      new CustomEvent("filter-change", {
  ...
  _renderPalette() {
    const t = this._selectedHexes, e = this._colors.length >= us;
    return n`
      <div class="palette">
        ${pa.map(
      (s) => {
  ...
  render() {
    const t = this._colors.length === 0 || this._showPalette, e = this._colors.length < us && !this._showPalette;
    return n`
      <div class="filter-content">
        <button
          class="clear-btn"
...
vt([
  _()
], ze.prototype, "_colors", 2);
vt([
  _()
], ze.prototype, "_advanced", 2);
vt([
  _()
], ze.prototype, "_showPalette", 2);
vt([
  _()
...
let Fe = class extends C {
  ...
  _emitChange() {
    const t = Fo[this._unit], e = this._minVal !== "" ? parseFloat((parseFloat(this._minVal) * t).toFixed(2)).toString() : null, s = this._maxVal !== "" ? parseFloat((parseFloat(this._maxVal) * t).toFixed(2)).toString() : null;
    let i;
    if (e !== null && s !== null)
      i = `${e}..${s}`;
    else if (e !== null)
...
et([
  _()
], Fe.prototype, "_minVal", 2);
et([
  _()
], Fe.prototype, "_maxVal", 2);
et([
  _()
], Fe.prototype, "_unit", 2);
et([
  _()
    ...
    return n`
      ...
      (t) => n`
        ...
        this._toggleOrientation(t.value);
                ...
                ><span class="orientation-label">${Po[t.value]}${t.label}</span></ap-checkbox>
              `
    )}
          </div>
        </div>
...
let j = class extends C {
  ...
  _getField(t) {
    return this.fields.find((e) => O(e) === t);
  }
  _getUIType(t) {
    return va[t.type];
  }
  ...
  _isSpecialValue(t) {
    return !t || t.length !== 1 ? !1 : t[0] === pe || t[0] === he;
  }
  /** Get the "content" mode for a field: 'empty', 'not-empty', or 'specific' */
  _getContentMode(t) {
    var s, i;
    const e = this._getApplied(t);
    return ((s = e.values) == null ? void 0 : s[0]) === pe ? pe : ((i = e.values) == null ? void 0 : i[0]) === he ? he : this._specificModeFields.has(t) || e.values && e.values.length > 0 ? Be : "";
  }
  // ── Event dispatchers ────────────────────────────────────────────
  ...
  _onOperatorChange(t, e) {
    const i = this._getApplied(O(t)).values || [];
  ...
  _clearFieldFilter(t) {
    ...
    this._datePresets = i;
    const { [e]: a, ...o } = this._dateFroms;
    this._dateFroms = o;
    const { [e]: r, ...l } = this._dateTos;
    this._dateTos = l;
    const { [e]: c, ...h } = this._selectSearches;
    this._selectSearches = h;
    const { [e]: u, ...g } = this._tagInputs;
    this._tagInputs = g, this._emitFilterChange(t, void 0, []);
  }
  // ── Text filter ──────────────────────────────────────────────────
  _onTextInput(t, e) {
    const s = e.target.value, a = this._getApplied(O(t)).operator || ns[0].value;
    this._emitFilterChange(t, a, s ? [s] : []);
  }
  // ── Number filter ────────────────────────────────────────────────
  _onNumberInput(t, e, s) {
    const i = e.target.value, a = O(t), o = this._getApplied(a), r = o.operator || cs[0].value, l = [...o.values || []];
    s === 0 ? l[0] = i : l[1] = i;
    const c = l.filter((h) => h !== "" && h !== void 0);
    if (c.length > 0) {
      const h = t.type === k.DECIMAL2;
      if (!c.every(
        (g) => h ? !isNaN(parseFloat(g)) : /^-?\d+$/.test(g)
      )) return;
    }
    this._emitFilterChange(t, r, l);
  }
  // ── Boolean filter ───────────────────────────────────────────────
  _onBooleanSelect(t, e) {
    var a;
    const s = this._getApplied(O(t)), i = (a = s.values) == null ? void 0 : a[0];
    !e || i === e ? this._emitFilterChange(t, void 0, []) : this._emitFilterChange(t, s.operator, [e]);
  }
  // ── Select-one filter ────────────────────────────────────────────
  _onSelectOneToggle(t, e) {
    const s = O(t), i = this._getApplied(s), a = i.operator || Se[0].value, o = i.values || [];
    o.length === 1 && o[0] === e ? this._emitFilterChange(t, a, []) : this._emitFilterChange(t, a, [e]);
  }
  // ── Multi-select filter ──────────────────────────────────────────
  _onMultiSelectToggle(t, e) {
    const s = O(t), i = this._getApplied(s), a = i.operator || ls[0].value, o = i.values || [], r = o.includes(e) ? o.filter((l) => l !== e) : [...o, e];
  ...
  _commitTagInput(t, e) {
    ...
    if (!s) return;
    const i = this._getApplied(e), a = i.operator || Se[0].value, o = i.values || [];
  ...
  _onTagOperatorChange(t, e) {
    const s = O(t), a = this._getApplied(s).values || [];
  ...
  _onDatePreset(t, e) {
    ...
    switch (e) {
      case "today": {
        const r = new Date(i);
        r.setHours(0, 0, 0, 0), a = r.toISOString(), o = i.toISOString();
        break;
      }
      case "last_week": {
        const r = new Date(i), l = r.getDay();
        r.setDate(r.getDate() - (l === 0 ? 6 : l - 1)), r.setHours(0, 0, 0, 0);
        const c = new Date(r);
        c.setDate(c.getDate() - 7), a = c.toISOString(), o = r.toISOString();
        break;
      }
      case "last_month": {
        const r = new Date(i.getFullYear(), i.getMonth(), 1), l = new Date(r);
        l.setMonth(l.getMonth() - 1), a = l.toISOString(), o = r.toISOString();
        break;
      }
      case "last_year": {
        const r = new Date(i.getFullYear(), 0, 1), l = new Date(r);
        l.setFullYear(l.getFullYear() - 1), a = l.toISOString(), o = r.toISOString();
        break;
      }
    }
  ...
  _onDateInput(t, e, s) {
    const i = s.target.value, a = O(t), o = this._datePresets[a] || "", r = i ? (/* @__PURE__ */ new Date(i + "T00:00:00")).toISOString() : "";
  ...
  _onGeoLocationInput(t, e) {
    var g;
    const s = e.target.value.trim(), i = O(t), a = (g = this.shadowRoot) == null ? void 0 : g.querySelector(
      `[data-geo-radius="${i}"]`
    ...
    const r = s.split(",").map((d) => d.trim());
    if (r.length !== 2 || r.some((d) => isNaN(Number(d)) || d === ""))
      return;
    const [l, c] = r;
    let u = `${l},${c}..${o || "0"}`;
    this._emitFilterChange(t, T.IS, [u]);
  }
  _onGeoRadiusInput(t, e) {
    var c;
    const s = e.target.value, i = O(t), a = this._getApplied(i);
    if (!((c = a.values) != null && c[0])) return;
    const o = a.values[0], [r] = o.split("..");
    let l = r;
    s && (l += `..${s}`), this._emitFilterChange(t, T.IS, [l]);
  }
  ...
  _renderFieldSelection() {
    ...
    ), s = this._getGroupedFields(e), i = (a, o) => o.length === 0 ? v : n`
        <div class="field-group-label">${a}</div>
        ${o.map((r) => {
      const l = O(r), c = this.visibleFields.includes(l), h = !c && this._atFieldLimit;
      return n`
            <div
              class="field-item ${c ? "selected" : ""} ${h ? "disabled" : ""}"
              @click=${() => !h && !c && this._addField(r)}
            >
              ${c ? n`<ap-icon name="check" .size=${14}></ap-icon>` : n`<ap-icon name=${Ut[r.type] || "file-text"} .size=${14} style="color: var(--ap-muted-foreground, oklch(0.685 0.033 249.82))"></ap-icon>`}
              <span>${r.label}</span>
            </div>
          `;
    })}
  ...
  _renderTextFilter(t) {
    var h, u;
    const e = O(t), s = this._getApplied(e), a = this._isSpecialValue(s.values) ? "" : ((h = s.values) == null ? void 0 : h[0]) || "", o = this._getContentMode(e), r = o === pe || o === he, l = s.operator || ns[0].value, c = t.type === k.TEXT_AREA;
    return n`
  ...
  _renderNumberFilter(t) {
    var $, R, F;
    const e = O(t), s = this._getApplied(e), i = s.operator || cs[0].value, a = this._isSpecialValue(s.values), o = this._getContentMode(e), r = o === pe || o === he, l = i === T.RANGE, c = i === T.GREATER_THAN_OR_EQUAL, h = i === T.LESS_THAN_OR_EQUAL, u = t.type === k.DECIMAL2, g = u ? "0.01" : "1", d = u ? "0.00" : "0", f = a ? "" : (($ = s.values) == null ? void 0 : $[0]) || "", b = a ? "" : ((R = s.values) == null ? void 0 : R[1]) || "", x = l || c || h, w = l || c || h ? "Min" : "Value";
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
        ...
        ${o === Be ? n`
              <div class="filter-section">
                <div class="grid-2">
                  <div>
                    <span class="input-label" aria-disabled=${h ? "true" : v}>${w}</span>
                    <input
                      class="filter-input"
                      type="number"
                      step=${g}
                  ...
                  ${x ? n`
                        <div>
                          <span class="input-label" aria-disabled=${c ? "true" : v}>${"Max"}</span>
                          <input
                            class="filter-input"
                            type="number"
                            step=${g}
  ...
  _renderBooleanFilter(t) {
    var i, a;
    const e = this._getApplied(O(t)), s = ((i = e.values) == null ? void 0 : i[0]) || "";
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
  ...
  _renderSelectOneFilter(t) {
    const e = O(t), s = this._getApplied(e), i = this._isSpecialValue(s.values), a = i ? [] : s.values || [], o = t.possible_values || [], r = s.operator || Se[0].value, l = (this._selectSearches[e] || "").toLowerCase(), c = l.length > 0, h = a.length > 0 || i, u = l ? o.filter((d) => d.label.toLowerCase().includes(l)) : o, g = a.length >= zt;
    return n`
      <div class="filter-content">
        <!-- Search input -->
        <div class="search-wrapper">
            ...
            ` : n`
              ...
              ${a.length > 0 ? n`
                    <div class="filter-section">
                      ...
                      <div class="chips-wrap">
                        ${a.map((d) => {
      const f = o.find((b) => b.api_value === d);
      return n`
                            <div class="chip">
                              <span class="chip-label">${(f == null ? void 0 : f.label) || d}</span>
      ...
      (d) => {
        ...
        return n`
                ...
                >
                  <ap-checkbox ?checked=${((f = s.values) == null ? void 0 : f[0]) === d.value}></ap-checkbox>
                  <span>${d.label}</span>
                </div>
              `;
      }
          ...
          <!-- Regular options -->
          <div class="options-list">
            ${u.length === 0 ? n`<div class="no-results">No options found</div>` : u.map((d) => {
      const f = a.includes(d.api_value), b = !f && g;
      return n`
                    <div
  ...
  _renderMultiSelectFilter(t) {
    const e = O(t), s = this._getApplied(e), i = this._isSpecialValue(s.values), a = i ? [] : s.values || [], o = t.possible_values || [], r = s.operator || ls[0].value, l = (this._selectSearches[e] || "").toLowerCase(), c = l.length > 0, h = a.length > 0 || i, u = l ? o.filter((d) => d.label.toLowerCase().includes(l)) : o, g = a.length >= zt;
    return n`
      <div class="filter-content">
        <!-- Search input -->
        <div class="search-wrapper">
  ...
  // ── Render: Tags filter ──────────────────────────────────────────
  _renderTagsFilter(t) {
    const e = O(t), s = this._getApplied(e), i = this._isSpecialValue(s.values), a = i ? [] : s.values || [], o = this._tagInputs[e] || "", r = s.operator || Se[0].value, l = a.length > 0 || i, c = this._tagSuggestions[e] || [], h = this._tagLoading[e] || !1, u = o.trim().length > 0, g = a.length >= zt;
    return n`
      <div class="filter-content">
        <!-- Search input -->
        <div class="search-wrapper">
      ...
      (d) => {
        const f = a.includes(d), b = !f && g;
        return n`
                              <div
                                class="option-item ${b ? "disabled" : ""}"
                                @click=${() => !b && (f ? this._removeTag(t, d) : this._selectTagSuggestion(t, d))}
                              >
                                <ap-checkbox ?checked=${f}></ap-checkbox>
                                <span>${d}</span>
                              </div>
  ...
  _renderDateFilter(t) {
    var u, g;
    const e = O(t), s = this._datePresets[e] || "", i = this._getApplied(e), o = this._isSpecialValue(i.values) ? ((u = i.values) == null ? void 0 : u[0]) || "" : s, r = [
  ...
  _renderGeoFilter(t) {
    var u, g;
    const e = O(t), s = this._getApplied(e), i = this._isSpecialValue(s.values), a = this._getContentMode(e);
    let o = "", r = "";
    if (!i && ((u = s.values) != null && u[0])) {
      const d = s.values[0], [f, b] = d.split("..");
      o = f || "", r = b || "";
    }
    const l = o.length > 0, c = o.split(",").map((d) => d.trim()), h = !l || c.length === 2 && c.every((d) => !isNaN(Number(d)) && d !== "");
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
              ...
              <div class="filter-section">
                <div class="geo-row">
                  <div class="geo-location">
                    ...
                    />
                    ${l && !h ? n`<div class="error-text">Enter coordinates as "latitude, longitude"</div>` : v}
                  </div>
  ...
  _renderFaceMatcherFilter(t) {
    const e = O(t), s = this._getApplied(e), i = s.values || [], a = s.operator || T.IS;
    return n`
      <div class="filter-content" style="position: relative;">
        <button
          class="clear-btn"
        ...
        <div class="filter-section">
          ...
          <input
            ...
            .value=${i.join(", ")}
            @change=${(o) => {
      const l = o.target.value.split(",").map((c) => c.trim()).filter(Boolean);
      this._emitFilterChange(t, a, l);
    }}
          />
        </div>
      </div>
  ...
  _renderFieldControl(t) {
    switch (this._getUIType(t)) {
      ...
      case "attachment":
        return t.type === k.INTEGER_LIST && t.slug === "face_matcher" ? this._renderFaceMatcherFilter(t) : this._renderAttachmentFilter(t);
      default:
        return v;
    }
  }
        ...
        >
          <div class="field-header-left">
            <ap-icon
              ...
              .size=${14}
            ></ap-icon>
            <ap-icon name=${Ut[e.type] || "file-text"} .size=${14} style="color: var(--ap-muted-foreground, oklch(0.685 0.033 249.82))"></ap-icon>
            <span>${e.label}</span>
          </div>
  ...
  _renderSelectorMode() {
    const t = this._fieldSearch.toLowerCase(), e = this.fields.filter(
      (a) => a.label.toLowerCase().includes(t)
        ...
        <div class="field-group-label">${a}</div>
        ${o.map((r) => {
      const l = O(r), c = this.visibleFields.includes(l), h = !!this.appliedMetadata[l] || !!this.appliedFilters[l], u = this.pinnedFields.includes(l), g = !c && this._atFieldLimit;
      return n`
            <div
              class="field-item ${h ? "has-filter" : ""} ${g ? "disabled" : ""}"
              @click=${() => !g && this._emitFieldSelect(r)}
            >
              <ap-icon name=${Ut[r.type] || "file-text"} .size=${14} style="color: var(--ap-muted-foreground, oklch(0.685 0.033 249.82))"></ap-icon>
        ...
        d.stopPropagation(), this._emitPin(l, !u);
              ...
              ${h ? n`<ap-icon name="check" .size=${14}></ap-icon>` : v}
            </div>
          `;
    })}
      `;
  ...
  _renderFieldMode() {
    ...
    return t ? this._renderFieldControl(t) : n`<div class="empty-msg">Field not found</div>`;
  }
  // ── Main render ──────────────────────────────────────────────────
  render() {
...
let ue = class extends C {
  ...
  get _statusHasEmptyValue() {
    return this.selectedStatus.includes(pe) || this.selectedStatus.includes(he);
  }
  ...
  _toggleStatus(t) {
    ...
    this._emitChange(X.STATUS, e, this.statusOperator);
  }
  _handleStatusOperator(t) {
    const e = t.detail.value;
    this._emitChange(X.STATUS, this.selectedStatus, e);
  }
  _clearStatus() {
    this._emitChange(X.STATUS, [], this.statusOperator);
  }
  // ── Approver handlers ──────────────────────────────────────────────
  _handleApproverInput(t) {
    const e = t.target.value;
    this._emitChange(X.APPROVER, e ? [e] : [], this.approverOperator);
  }
  _handleApproverOperator(t) {
    const e = t.detail.value;
    this._emitChange(X.APPROVER, this.approverValue ? [this.approverValue] : [], e);
  }
  _clearApprover() {
    this._emitChange(X.APPROVER, [], this.approverOperator);
  }
  // ── Requester handlers ─────────────────────────────────────────────
  _handleRequesterInput(t) {
    const e = t.target.value;
    this._emitChange(X.REQUESTOR, e ? [e] : [], this.requesterOperator);
  }
  _handleRequesterOperator(t) {
    const e = t.detail.value;
    this._emitChange(X.REQUESTOR, this.requesterValue ? [this.requesterValue] : [], e);
  }
  _clearRequester() {
    this._emitChange(X.REQUESTOR, [], this.requesterOperator);
  }
  // ── Due Date handlers ──────────────────────────────────────────────
  _handleDueDatePreset(t) {
    const e = t.detail.value, s = this.dueDatePreset === e ? "" : e;
    this._emitChange(X.DUE_DATE, s ? [s] : [], ":");
  }
    ...
    return n`
      <div class="filter-section">
        ...
        <ap-radio-group
          clearable
          .options=${Ve}
          .value=${this.dueDatePreset === pe || this.dueDatePreset === he ? this.dueDatePreset : ""}
  ...
  render() {
    ...
    return n`
      ...
      (e) => n`
        ...
        ${this._isSearching ? v : n`
              <div class="placeholder">
                ${t ? n`<ap-icon name="box" .size=${40} style="color: var(--ap-border, oklch(92.86% 0.009 247.92)); margin-bottom: 4px;"></ap-icon>` : v}
        ...
        <!-- No results -->
        ${this._isSearching && !this._isLoading && this._products.length === 0 ? n`<div class="no-results">No results found</div>` : v}
      </div>
...
$e([
  Ze(".product-search")
], fe.prototype, "_searchInput", 2);
$e([
  _()
], fe.prototype, "_search", 2);
$e([
  _()
], fe.prototype, "_debouncedSearch", 2);
$e([
  _()
], fe.prototype, "_products", 2);
$e([
  _()
], fe.prototype, "_isLoading", 2);
$e([
  _()
], fe.prototype, "_showAddList", 2);
$e([
  _()
], fe.prototype, "_addListText", 2);
fe = $e([
  A("ap-filter-product-ref")
], fe);
var Vo = Object.defineProperty, Ho = Object.getOwnPropertyDescriptor, D = (t, e, s, i) => {
...
let P = class extends C {
  constructor() {
    super(...arguments), this.totalCount = 0, this.totalFolderCount = 0, this.isLoading = !1, this.showUpload = !1, this.showFilters = !0, this.countLabel = "", this.sortBy = "created_at", this.sortDirection = "desc", this.sortOptions = $s, this.filters = {
      ...
      visible: [],
      applied: {}
    }, this.labels = [], this.tags = [], this.fileTypes = [], this.metadataFields = [], this.pinnedFilters = [], this.forcedFilterKeys = [], this._showDropdown = !1, this._showMetadataSelector = !1, this._openFilter = null, this._openMetadataField = null, this._externalTrigger = !1, this._externalLeft = null, this._externalTop = null, this._outsideClickHandler = (t) => {
      const e = t.composedPath();
      if (this._externalTrigger && e.some(
        (o) => {
          var r;
          return o instanceof HTMLElement && o.classList.contains("chip") && ((r = o.closest) == null ? void 0 : r.call(o, "ap-filters-bar"));
        }
      ))
        return;
  ...
  _getMetadataFieldLabel(t) {
    for (const e of this.metadataFields)
      if (`${Xt[e.type] || ""}${e.key}` === t) return e.label;
    return t;
  }
  _togglePin(t, e) {
    e.stopPropagation();
  ...
  _getFilterIcon(t) {
    const e = ms.find((s) => s.key === t);
    return (e == null ? void 0 : e.icon) || "filter";
  }
  _renderFilterContent(t) {
    var s, i, a;
    const e = this.filters.applied;
    switch (t) {
      case "type": {
        const o = e.type;
        return n`<ap-filter-type
      ...
      case "size": {
        const o = e.size, r = ((s = o == null ? void 0 : o.values) == null ? void 0 : s[0]) || "", [l, c] = r.split(".."), h = l ? parseFloat(l) / 1e6 : 0, u = c ? parseFloat(c) / 1e6 : 0;
      ...
      case "image": {
        const o = e.image, r = o && "resolution" in o ? o : null;
        return n`<ap-filter-image
      ...
      case "approval": {
        const o = e.approval_status, r = e.task_approver, l = e.task_requester, c = e.task_duedate;
        return n`<ap-filter-approval
      ...
      case "metadata":
        return this._openMetadataField ? n`<ap-filter-metadata
            mode="field"
        ...
        ></ap-filter-metadata>`;
      case "product_ref": {
        const o = e.product_ref;
        return n`<ap-filter-product-ref
          .selected=${(o == null ? void 0 : o.values) || []}
          .apiClient=${this.apiClient}
          @filter-change=${this._handleFilterChange}
  ...
  render() {
    ...
    return n`
      <div class="toolbar-row">
        ${this.isLoading ? n`<span class="count-skeleton"></span>` : n`<span class="count">${this.countLabel || [
      ...
      e.stopPropagation(), this._openMetadataField = null;
                  ...
                  ` : n`
                    ...
                    ${nt[this._openFilter] || this._openFilter}
                  `}
...
D([
  p({ type: Number })
], P.prototype, "totalCount", 2);
D([
  p({ type: Number })
], P.prototype, "totalFolderCount", 2);
D([
  p({ type: Boolean })
], P.prototype, "isLoading", 2);
D([
  p({ type: Boolean })
], P.prototype, "showUpload", 2);
D([
  p({ type: Boolean })
], P.prototype, "showFilters", 2);
D([
  p()
], P.prototype, "countLabel", 2);
D([
  p()
], P.prototype, "sortBy", 2);
D([
  p()
], P.prototype, "sortDirection", 2);
D([
  p({ type: Array })
], P.prototype, "sortOptions", 2);
D([
  p({ type: Object })
...
D([
  p({ type: Array })
], P.prototype, "fileTypes", 2);
D([
  p({ type: Array })
], P.prototype, "metadataFields", 2);
D([
  p({ type: Array })
], P.prototype, "pinnedFilters", 2);
D([
  p({ type: Array })
], P.prototype, "forcedFilterKeys", 2);
D([
  p({ attribute: !1 })
], P.prototype, "apiClient", 2);
D([
  Ze("ap-dropdown")
], P.prototype, "_sortDropdown", 2);
D([
  _()
], P.prototype, "_showDropdown", 2);
D([
  _()
], P.prototype, "_showMetadataSelector", 2);
D([
  _()
], P.prototype, "_openFilter", 2);
D([
  _()
], P.prototype, "_openMetadataField", 2);
D([
  _()
], P.prototype, "_externalTrigger", 2);
D([
  _()
], P.prototype, "_externalLeft", 2);
D([
  _()
], P.prototype, "_externalTop", 2);
P = D([
  A("ap-content-toolbar")
], P);
...
let Q = class extends C {
  ...
  _mapTypeLabel(t) {
    var i;
    const e = (i = qt.find((a) => a.value === t)) == null ? void 0 : i.label;
    if (e) return e;
    const s = t.indexOf("_");
  ...
  _getSizeSummary(t) {
    ...
    const e = t.values[0], [s, i] = e.split(".."), a = s ? parseFloat(s) : null, o = i ? parseFloat(i) : null, r = (l) => l >= 1e3 ? `${(l / 1e3).toFixed(l % 1e3 === 0 ? 0 : 1)} GB` : `${l} MB`;
    return a !== null && o !== null ? `${r(a)} – ${r(o)}` : a !== null ? `> ${r(a)}` : o !== null ? `< ${r(o)}` : "";
  }
  _getFilterSummary(t, e) {
    if (t.type === "string") {
      const s = t;
      if (s.values.length === 0) return "";
      if (e === y.SIZE) return this._getSizeSummary(s);
      if (e === y.COLOR) {
        const a = s.values.map((o) => o.split(" ")[0]).filter(Boolean);
        return a.length === 0 ? "" : a.length === 1 ? a[0] : `${a[0]} +${a.length - 1}`;
      }
      const i = e === y.TYPE ? (a) => this._mapTypeLabel(a) : e === y.TAGS ? (a) => {
        var o;
        return ((o = this.tags.find((r) => r.sid === a)) == null ? void 0 : o.label) || a;
      } : e === y.LABELS ? (a) => {
        var o;
        return ((o = this.labels.find((r) => r.sid === a)) == null ? void 0 : o.name) || a;
      } : (a) => a;
  ...
  _getDateSummary(t) {
    var r, l;
    const e = ((r = gi.find((c) => c.value === t.field)) == null ? void 0 : r.label) || t.field, s = [...ws, ...vi], i = t.preset ? ((l = s.find((c) => c.value === t.preset)) == null ? void 0 : l.label) || t.preset : "", a = (c) => {
      try {
        return c.split("T")[0];
      } catch {
        return c;
    ...
    switch (t.kind) {
      case "preset":
        ...
        break;
      case "before":
        o = t.to ? `Before ${a(t.to)}` : i;
        break;
      case "after":
        o = t.from ? `After ${a(t.from)}` : i;
        break;
      case "between":
        t.from && t.to ? o = `${a(t.from)} – ${a(t.to)}` : t.from ? o = `After ${a(t.from)}` : t.to && (o = `Before ${a(t.to)}`);
        break;
      case "specific":
        o = t.from ? a(t.from) : "";
        break;
  ...
  _getFilterIcon(t) {
    var e;
    return ((e = ms.find((s) => s.key === t)) == null ? void 0 : e.icon) || "filter";
  }
  _getMetadataLabel(t) {
  ...
  _renderMetadataChip(t, e) {
    if (!e) {
      const o = this._getMetadataLabel(t), r = this._getMetadataIcon(t);
      return n`
  ...
  render() {
    const t = bs(this.forcedFilters), e = new Set(Object.keys(t)), s = Object.keys(this.appliedFilters).filter((f) => !e.has(f)), i = Object.keys(this.appliedMetadata), a = new Set(this.pinnedFilters), o = new Set(this.pinnedMetadataFields), r = this.pinnedFilters.filter((f) => !e.has(f)), l = s.filter((f) => !a.has(f)), c = i.filter((f) => !o.has(f)), h = this.pendingFilter && !a.has(this.pendingFilter) && !(this.pendingFilter in this.appliedFilters), u = this.pendingMetadataField && !o.has(this.pendingMetadataField) && !(this.pendingMetadataField in this.appliedMetadata), g = s.length + i.length > 0;
    return e.size + r.length + this.pinnedMetadataFields.length + l.length + c.length + (h ? 1 : 0) + (u ? 1 : 0) === 0 ? v : n`
      <div class="chips-row">
        <div class="chips">
          ${[...e].map(
      (f) => this._renderForcedChip(f, t[f])
    )}
          ${r.map(
      (f) => this._renderFilterChip(f, this.appliedFilters[f])
    )}
          ${this.pinnedMetadataFields.map(
      (f) => this._renderMetadataChip(f, this.appliedMetadata[f])
    )}
          ${l.map(
      (f) => this._renderFilterChip(f, this.appliedFilters[f])
    )}
      ...
      (f) => this._renderMetadataChip(f, this.appliedMetadata[f])
          ...
          ` : v}
          ${g ? n`<button class="clear-all" @click=${this._clearAll}>Clear filters</button>` : v}
        </div>
      </div>
    `;
  }
  ...
  render() {
    ...
    return n`
      ...
      (e, s) => {
        ...
        ${this.isLoading ? Array.from({ length: Wo }, () => n`<div class="ghost-card">
  ...
  render() {
    ...
    return n`
      <div class="list-header" style="grid-template-columns: ${this._getGridColumns(this.multiSelect)}" role="row" aria-label="Column headers">
        ...
        <span>Name</span>
        <span>Type</span>
        ${this._compactLevel < 2 ? n`<span>Size</span>` : v}
        ${this._compactLevel < 1 ? n`<span>Date</span>` : v}
        <span></span>
      </div>
      ...
      (i, a) => n`
            <ap-folder-row
              ...
              @folder-open=${(o) => {
        o.stopPropagation(), this.dispatchEvent(new CustomEvent("folder-open", { detail: o.detail, bubbles: !0, composed: !0 }));
      }}
              @folder-select=${(o) => {
        o.stopPropagation(), this.dispatchEvent(new CustomEvent("folder-select", { detail: o.detail, bubbles: !0, composed: !0 }));
      }}
            ></ap-folder-row>
          `
    )}
...
var er = Object.defineProperty, tr = Object.getOwnPropertyDescriptor, Mt = (t, e, s, i) => {
...
let Ye = class extends C {
  ...
  _formatDuration(t) {
    const e = Math.floor(t / 3600), s = Math.floor(t % 3600 / 60), i = Math.floor(t % 60), a = (o) => String(o).padStart(2, "0");
  ...
  render() {
    ...
    const t = this.asset;
    if (!t) return n``;
    const s = lt(t.type) === "video", i = ((c = t.extension) == null ? void 0 : c.toLowerCase()) === "pdf", a = ri(t.extension || "");
    let o = "";
    s ? o = li(t) : i ? o = xs(t) : a && (o = Bt(t));
    const r = Tt(t.extension || ""), l = ys(t.extension || "");
    return n`
      <div class="card" @click=${this._handleSelect}>
        <div class="thumbnail ${l && o ? "chess" : ""}">
          ${o ? n`<img
                ...
                @error=${(f) => {
      const b = f.target, x = ct();
      b.src !== r && b.src !== x ? (b.src = r, b.classList.add("icon-fallback")) : b.src !== x && (b.src = x);
    }}
              />` : n`<img
                src=${r}
                alt=${t.name}
      ...
      b.src !== x && (b.src = x);
              ...
              />`}
          ${s && (((h = t.info) == null ? void 0 : h.video_duration) ?? ((u = t.info) == null ? void 0 : u.duration)) != null ? n`<span class="duration">${this._formatDuration(t.info.video_duration ?? t.info.duration)}</span>` : v}
          <div class="overlay">
        ...
        <div class="info">
          ...
          <div class="meta">${(g = t.extension) == null ? void 0 : g.toUpperCase()}${(() => {
      var w, L, $, R;
      const f = ((w = t.info) == null ? void 0 : w.img_w) || ((L = t.info) == null ? void 0 : L.video_w), b = (($ = t.info) == null ? void 0 : $.img_h) || ((R = t.info) == null ? void 0 : R.video_h), x = ki(f, b);
      return x ? ` · ${x}` : "";
    })()} · ${Fs(((d = t.size) == null ? void 0 : d.bytes) || 0)}</div>
        </div>
      </div>
    `;
  }
  ...
  render() {
    ...
    return n`
      <div class="row" style="grid-template-columns: ${this._getGridColumns()}" @click=${this._handleSelect}>
        ...
        <div class="thumb ${l && o ? "chess" : ""}">
          ${o ? n`<img
                ...
                loading="lazy"
                @error=${(g) => {
      const d = g.target, f = ct();
      d.src !== r && d.src !== f ? (d.src = r, d.classList.add("icon-fallback")) : d.src !== f && (d.src = f);
    }}
              />` : n`<img
                src=${r}
                alt=${t.name}
      ...
      d.src !== f && (d.src = f);
        ...
        ${this.compactLevel < 2 ? n`<div class="desc">${Fs(((u = t.size) == null ? void 0 : u.bytes) || 0)}</div>` : v}
        ${this.compactLevel < 1 ? n`<div class="date">${Vt(t.created_at || "")}</div>` : v}
        <div class="actions">
...
let Ae = class extends C {
  ...
  _renderPreviewImg(t) {
    const e = ni(t.file_type), s = this._getPreviewUrl(t), i = Tt(Wt(t.file_type)), a = ct();
    return n`<div class="preview-slot ${e ? "icon-bg" : ""}">
      <img
        ...
        loading="lazy"
        @error=${(o) => {
      const r = o.target, l = r.parentElement;
      r.src !== i && r.src !== a ? (r.src = i, r.classList.add("icon-mode"), l == null || l.classList.add("icon-bg")) : r.src !== a && (r.src = a, r.classList.add("icon-mode"), l == null || l.classList.add("icon-bg"));
    }}
        @load=${(o) => {
      const r = o.target, l = r.parentElement;
      r.src.includes("assets/file-types") && (r.classList.add("icon-mode"), l == null || l.classList.add("icon-bg"));
    }}
      />
    </div>`;
  }
    ...
    return t.length === 1 ? n`<div class="preview-left">${this._renderPreviewImg(t[0])}</div>` : n`
      <div class="preview-grid">
        ...
        <div class="preview-right">
          ...
          ${t[2] ? n`<div class="preview-right-item">${this._renderPreviewImg(t[2])}</div>` : v}
        </div>
      </div>
    `;
  }
  render() {
    ...
    const t = this.folder;
    if (!t) return n``;
    const e = ((a = t.count) == null ? void 0 : a.files_direct) ?? 0, i = (((o = t.count) == null ? void 0 : o.files_recursive) ?? 0) - e;
    return n`
        ...
        <div class="card-content">
          ...
          <div class="info">
            ...
            <div class="counts">
              ...
              ${i > 0 ? n`<span>${i} sub-asset${i !== 1 ? "s" : ""}</span>` : v}
            </div>
          </div>
        </div>
      </div>
...
let xe = class extends C {
  ...
  _renderPreviewImg(t) {
    const e = ni(t.file_type), s = ci(t, "80"), i = Tt(Wt(t.file_type)), a = ct();
  ...
  render() {
    ...
    return t ? n`
      <div class="row" style="grid-template-columns: ${this._getGridColumns()}" @click=${this._handleClick}>
        ...
        ${this.compactLevel < 2 ? n`<div class="meta">${((e = t.count) == null ? void 0 : e.files_recursive) ?? ((s = t.count) == null ? void 0 : s.files_direct) ?? 0} items</div>` : v}
        ${this.compactLevel < 1 ? n`<div class="meta">${Vt(t.modified_at || t.updated_at || "")}</div>` : v}
        <div></div>
      </div>
    ` : n``;
  }
...
let We = class extends C {
  ...
  render() {
    if (this.variant === "grid") {
      const r = this.gridSize === "large" ? "280px" : "220px";
      return n`
    ...
    `, a = ["70%", "55%", "80%", "45%", "65%", "50%", "75%", "60%", "85%", "40%", "72%", "58%"], o = 12 - this.folderCount;
    return n`
...
let B = class extends C {
  ...
  async _setupHls(t) {
    ...
    if (s) {
      ...
      try {
        const { default: o } = await import("./hls.light-DxDlt6yF.js");
        if (!o.isSupported() || ((a = this.asset) == null ? void 0 : a.uuid) !== t.uuid || !this.isConnected) return;
        const r = new o();
        this._hls = r, r.loadSource(e), r.attachMedia(s), r.on(o.Events.ERROR, (l, c) => {
          var h;
  ...
  _fullscreen() {
    ...
    this._fsImageLoaded = !1, (r = (o = this.previewArea) == null ? void 0 : o.requestFullscreen) == null || r.call(o);
  }
  _select() {
  ...
  _getFullscreenImageUrl(t) {
    ...
    const s = Ge(e, this.containerToken);
    return Pe(s, {
  ...
  _getBlurDimensions(t) {
    var r, l;
    const e = (r = t.info) == null ? void 0 : r.img_w, s = (l = t.info) == null ? void 0 : l.img_h;
    if (!e || !s) return { width: "100vw", height: "100vh" };
    const i = window.innerWidth, a = window.innerHeight, o = Math.min(i / e, a / s, 1);
  ...
  _applyTagFilter(t) {
    this.dispatchEvent(new CustomEvent("filter-update", {
      detail: { key: y.TAGS, values: [t], operator: T.IS },
      bubbles: !0,
      composed: !0
    }));
  }
  _applyLabelFilter(t) {
    this.dispatchEvent(new CustomEvent("filter-update", {
      detail: { key: y.LABELS, values: [t], operator: T.IS },
      bubbles: !0,
      composed: !0
    }));
  }
  ...
  _getFileInfoRows(t) {
    var s, i, a, o, r, l;
    const e = [];
    return e.push({ label: "File name", value: t.name }), t.extension && e.push({ label: "Type", value: t.extension.toUpperCase() }), (s = t.size) != null && s.bytes && e.push({ label: "Size", value: Fs(t.size.bytes) }), (i = t.info) != null && i.img_w && e.push({ label: "Dimensions", value: ki(t.info.img_w, t.info.img_h) }), (a = t.info) != null && a.video_w && e.push({ label: "Video resolution", value: `${t.info.video_w} x ${t.info.video_h}` }), (((o = t.info) == null ? void 0 : o.video_duration) ?? ((r = t.info) == null ? void 0 : r.duration)) != null && e.push({ label: "Duration", value: `${Math.round(t.info.video_duration ?? t.info.duration)}s` }), (l = t.info) != null && l.color_space && e.push({ label: "Color space", value: t.info.color_space }), t.created_at && e.push({ label: "Created", value: Vt(t.created_at) }), t.modified_at && e.push({ label: "Modified", value: Vt(t.modified_at) }), e;
  }
  /**
  ...
  _getAllMetadataRows(t) {
    ...
    if (this.metadataFields.length > 0) {
      ...
      for (const u of this.metadataFields) {
        const g = h[u.key] ?? ((r = (o = t.info) == null ? void 0 : o.metadata) == null ? void 0 : r[u.key]) ?? ((l = t.meta) == null ? void 0 : l[u.key]), d = this._resolveFieldValue(g, u);
        if (!d) continue;
        i.add(u.key), u.slug && i.add(u.slug);
        const f = d.length > 50;
        e.push({ label: u.label, value: d, wrap: f });
      }
    }
    if (t.meta)
      for (const [h, u] of Object.entries(t.meta)) {
        ...
        const g = s.get(h);
        if (g) {
          const d = this._resolveFieldValue(u, g);
          if (!d) continue;
          i.add(h);
          const f = d.length > 50;
          e.push({ label: g.label, value: d, wrap: f });
        } else {
          const d = typeof u == "object" ? JSON.stringify(u) : String(u);
          if (!d || d === "{}" || d === "[]") continue;
          i.add(h);
          const f = h.charAt(0).toUpperCase() + h.slice(1).replace(/_/g, " "), b = d.length > 50;
          e.push({ label: f, value: d, wrap: b });
        }
      }
  ...
  render() {
    var F, E, M, I, ye, le, z, se;
    const t = this.asset;
    if (!t) return v;
    const e = lt(t.type), s = e === "image", i = e === "video", a = e === "audio", o = ((F = t.extension) == null ? void 0 : F.toLowerCase()) === "pdf", r = Tt(t.extension || ""), l = ys(t.extension || ""), c = ((E = t.extension) == null ? void 0 : E.toLowerCase()) === "svg", h = Bt(t), u = s ? c ? ((M = t.url) == null ? void 0 : M.cdn) || "" : h || Ge(((I = t.url) == null ? void 0 : I.cdn) || "", this.containerToken) : "", g = s ? this._getFullscreenImageUrl(t) : "", d = s && !c ? this._getBlurDimensions(t) : null, f = i && ((ye = t.url) == null ? void 0 : ye.cdn) || "", b = i && (((le = t.info) == null ? void 0 : le.video_thumbnail) || ((z = t.info) == null ? void 0 : z.preview)) || "", x = o && !s ? xs(t) : "", w = this._getCurrentIndex(), L = w > 0 && this._findNavTarget(w, -1) >= 0, $ = w >= 0 && this._findNavTarget(w, 1) >= 0, R = this.assets.length > 1;
    return n`
      <div class="panel">
        ...
        <div class="preview-area ${s && l ? "chess" : ""}">
          ...
          ` : v}
          ${this._previewLoading ? n`<div class="preview-loading"><div class="spinner"></div></div>` : v}
          ${s ? n`
            <img
              src=${u}
              alt=${t.name}
              @load=${(ie) => {
      ie.target.classList.remove("icon-fallback"), this._previewLoading = !1;
    }}
              @error=${(ie) => {
      const U = ie.target;
      this._previewLoading = !1, U.src !== r && (U.src = r, U.classList.add("icon-fallback"));
    }}
            />
          ...
          ` : v}
          ${i ? n`<video src=${f} poster=${b || v} controls controlsList="nofullscreen" @loadeddata=${() => {
      this._previewLoading = !1;
    }} @error=${() => {
      this._previewLoading = !1;
    }}></video>` : v}
          ${a ? n`<audio src=${((se = t.url) == null ? void 0 : se.cdn) || ""} controls></audio>` : v}
          ${o && !s ? n`
            <img
              src=${x || r}
              alt=${t.name}
      ...
      U.src !== r && (U.src = r, U.classList.add("icon-fallback"));
            ...
            />
          ` : v}
          ${!s && !i && !a && !o ? n`<img src=${r} alt=${t.name} class="icon-fallback" />` : v}
        </div>
...
let be = class extends C {
  ...
  _formatCount() {
    const t = this.selectedAssets.length, e = this.selectedFolders.length;
    if (e === 0) return `${t} selected`;
    if (t === 0) return `${e} folder${e !== 1 ? "s" : ""} selected`;
    const s = `${e} folder${e !== 1 ? "s" : ""}`, i = `${t} asset${t !== 1 ? "s" : ""}`;
    return `${s} and ${i} selected`;
  }
  _confirm() {
  ...
  render() {
    return this._totalSelected === 0 ? v : n`
      <div class="bar">
        ...
        ${this.multiSelect ? n`
          ...
          ${this._canSelectMore() ? n`<button
              ...
              ><ap-icon name="check-check" .size=${14}></ap-icon>${this.isSelectingAll ? "Selecting..." : this.maxSelections && this.maxSelections < this._scopeTotal ? `First ${this.maxSelections}` : this._selectScope === "assets" ? "Select all assets" : this._selectScope === "folders" ? "Select all folders" : "Select all"}</button>` : v}
          <button class="bar-btn" @click=${this._clear}><ap-icon name="close" .size=${14}></ap-icon>Deselect all</button>
          ${this.maxSelections && this._totalSelected >= this.maxSelections ? n`<span class="limit-notice">Max ${this.maxSelections} allowed</span>` : v}
        ` : v}
...
let Et = class extends C {
  ...
  render() {
    return !this.active || this.rect.width < 5 || this.rect.height < 5 ? v : n`
      <div
        class="marquee"
    ...
    return n`
      ...
      (t) => n`
          ...
          >
            ...
            ${t.icon ? n`<ap-icon name=${t.icon} .size=${16}></ap-icon>` : v}
...
let Ht = class extends C {
  ...
  render() {
    return n`<div class="spinner" style="width:${this.size}px;height:${this.size}px"></div>`;
  }
};
Ht.styles = [
  S`
...
let q = class extends C {
  ...
  render() {
    ...
    return n`
      ...
      >
        ${t != null && t.icon ? n`<ap-icon name=${t.icon} .size=${16}></ap-icon>` : v}
        ${this.label ? n`<span class="label">${this.label}${t ? ": " : ""}</span>` : v}${t ? t.label : ""}
        ...
        this._focusedIndex = i;
              ...
              >
                <span class="option-label">
                  ${s.icon ? n`<ap-icon name=${s.icon} .size=${16}></ap-icon>` : v}
                  ${s.label}
                </span>
                ${s.value === this.value ? n`<ap-icon name="check" .size=${14}></ap-icon>` : v}
              </button>
            `
    )}
        </div>
...
Y([
  _()
], q.prototype, "_focusedIndex", 2);
Y([
  _()
], q.prototype, "_menuPosition", 2);
Y([
  _()
], q.prototype, "_menuAlign", 2);
Y([
  _()
], q.prototype, "_menuTop", 2);
Y([
  _()
], q.prototype, "_menuBottom", 2);
Y([
  _()
], q.prototype, "_menuLeft", 2);
Y([
  _()
], q.prototype, "_menuRight", 2);
Y([
  _()
...
var jr = Object.defineProperty, Nr = Object.getOwnPropertyDescriptor, Oe = (t, e, s, i) => {
  for (var a = i > 1 ? void 0 : i ? Nr(e, s) : e, o = t.length - 1, r; o >= 0; o--)
    (r = t[o]) && (a = (i ? r(e, s, a) : r(a)) || a);
  return i && a && jr(e, s, a), a;
};
const Ur = ["webp", "jpeg", "png", "gif"];
function Br(t) {
...
let _e = class extends C {
  ...
  get _ratio() {
    const t = this.assets.find((i) => rt(i));
    if (!t) return 1.5;
    const e = fs(t), s = gs(t);
    return e && s ? e / s : 1.5;
  }
  ...
  _initDefaults() {
    const t = this.assets.find((e) => rt(e));
    if (this._defaultFormat = Br(t == null ? void 0 : t.extension), this._format = this._defaultFormat, this._quality = "excellent", this._lockAspect = !0, this._activePreset = "original", this.isMultiSelect)
...
Oe([
  _()
], _e.prototype, "_format", 2);
Oe([
  _()
], _e.prototype, "_quality", 2);
Oe([
  _()
], _e.prototype, "_width", 2);
Oe([
  _()
], _e.prototype, "_height", 2);
Oe([
  _()
], _e.prototype, "_lockAspect", 2);
Oe([
  _()
], _e.prototype, "_activePreset", 2);
_e = Oe([
  A("ap-transformation-dialog")
], _e);
...
let Kt = class extends C {
  ...
  render() {
    return n`<span class="badge"><slot></slot></span>`;
  }
};
Kt.styles = [
  S`
...
const Ds = class Ds extends C {
  constructor() {
    ...
    }, this.store = Ti(), this.storeCtrl = new Li(this, this.store), this.selectionCtrl = new Mi(this, this.store), this.infiniteScrollCtrl = new Ii(this, () => this._loadMore()), this.marqueeCtrl = new Pi(this, this.store);
  }
  get _isInline() {
    var e;
  ...
  _initConfig(e) {
    ...
    this._initFailed = !1, this._normalizedForcedFilters = bs(s.forcedFilters), this._initPromise = this._doInit(s).catch(() => {
      this._initFailed = !0;
    }), this.toggleAttribute("inline", s.displayMode === "inline"), s.displayMode === "inline" && !this.store.getState().isOpen && this.open();
  }
  async _doInit(e) {
    const s = Na(), i = s.sortBy ?? e.defaultSortBy ?? "created_at", a = s.sortDirection ?? e.defaultSortDirection ?? "desc";
    this.store.setState({
      config: e,
      projectToken: e.auth.projectToken,
      viewMode: e.rememberLastView && qa(e.auth.projectToken) || e.defaultViewMode || "grid",
      sortBy: i,
      sortDirection: a
    }), this.apiClient = new Di(e.auth, e.apiBase);
    try {
      if (e.auth.mode === "securityTemplate") {
        const x = await Vi(this.apiClient);
        this.apiClient.setSassKey(x), this.store.setState({ sassKey: x });
      }
      ...
      if (o.status === "fulfilled") {
        const { fields: x, regionalVariantGroups: w, regionalFilters: L, brandColor: $ } = o.value;
        u.metadataFields = x, u.regionalVariantGroups = w, u.regionalFilters = L, $ && (u.brandColor = $);
      }
      ...
      const { pinnedFilters: g, pinnedMetadata: d } = za(e.auth.projectToken ?? null), f = this.store.getState().filters;
      u.filters = {
        ...f,
        pinned: g,
        visible: [...g],
      ...
      b && (u.brandColor || this.store.setState({ brandColor: b }), Ya(this, b));
    } catch (o) {
      throw this.dispatchEvent(new CustomEvent("ap-error", {
        detail: { error: o, context: "init" },
        bubbles: !0,
        composed: !0
      })), o;
    }
  ...
  async open() {
    ...
    for (const [w, L] of Object.entries(a))
      s.has(w) || (i[w] = L);
    const o = Object.keys(i), r = ((u = this.config) == null ? void 0 : u.tabs) ?? ["assets", "folders"], l = (g = this.config) != null && g.rememberLastTab ? Ha(this.config.auth.projectToken) : null, c = (l && r.includes(l) ? l : null) ?? ((d = this.config) != null && d.defaultTab && r.includes(this.config.defaultTab) ? this.config.defaultTab : null) ?? r[0] ?? "assets";
    this.store.setState({
      isOpen: !0,
      activeTab: c,
      searchQuery: "",
      isAISearchActive: !!((f = this.config) != null && f.enableAISearch && ((b = this.config) != null && b.defaultAISearch)),
      filters: {
        metadata: {
  ...
  async _loadData() {
    var i, a, o, r, l, c, h, u, g, d, f, b, x, w, L, $, R, F, E, M, I, ye, le, z, se, ie, U, Ne, zs, Rs, js, Ns, Us, Bs, qs, Vs, Hs, Gs, Ks;
    if (!this.apiClient) return;
    const e = ++this._loadId, s = this.store.getState();
    this._scrollToTop(), this.store.setState({ isLoading: !0 });
    try {
      const it = s.activeTab;
      if (it === "assets") {
        const ce = this._buildSearchNotation(), Me = s.currentFolderPath || "/", me = s.isAISearchActive && !!s.searchQuery, de = Ue(this.apiClient, {
          folder: Me,
          offset: 0,
          limit: s.limit,
          sort_by: s.sortBy,
          ...
          ...me && {
            ...
            ai_lang: ((i = s.config) == null ? void 0 : i.locale) ?? "en"
          }
        }), W = Dt(this.apiClient, {
          folder: Me,
          q: ce || void 0,
        ...
        const oe = ((o = K.files) == null ? void 0 : o.length) ?? 0, V = oe >= s.limit, re = ((r = ae == null ? void 0 : ae.stats) == null ? void 0 : r.approx_files_count) ?? ((l = ae == null ? void 0 : ae.info) == null ? void 0 : l.total_files_count) ?? ((c = K.info) == null ? void 0 : c.total_files_count) ?? ((u = (h = K.base) == null ? void 0 : h.count) == null ? void 0 : u.files_recursive) ?? ((d = (g = K.base) == null ? void 0 : g.count) == null ? void 0 : d.files_direct) ?? oe;
        this.store.setState({
          assets: K.files || [],
          folders: [],
          totalCount: re,
        ...
        const ce = this._buildSearchNotation(), Me = s.currentFolderPath || "/", [me, de, W] = await Promise.all([
          Ri(this.apiClient, {
          ...
          Ue(this.apiClient, {
            ...
            ...s.isAISearchActive && s.searchQuery && {
              with_ai: !0,
              ai_query: s.searchQuery,
              ai_lang: ((f = s.config) == null ? void 0 : f.locale) ?? "en"
            }
          }),
          Dt(this.apiClient, {
            folder: Me,
            ...
            ...s.isAISearchActive && s.searchQuery && {
              with_ai: !0,
              ai_query: s.searchQuery,
              ai_lang: ((b = s.config) == null ? void 0 : b.locale) ?? "en"
            }
          }).catch(() => null)
        ]);
        if (e !== this._loadId) return;
        ...
        if (K.length > 0)
          try {
            ae = await ji(this.apiClient, K.map((at) => at.uuid));
          } catch {
          }
        if (e !== this._loadId) return;
        const oe = ((x = de.files) == null ? void 0 : x.length) ?? 0, V = oe >= s.limit, re = ((w = W == null ? void 0 : W.stats) == null ? void 0 : w.approx_files_count) ?? ((L = W == null ? void 0 : W.info) == null ? void 0 : L.total_files_count) ?? (($ = de.info) == null ? void 0 : $.total_files_count) ?? ((F = (R = de.base) == null ? void 0 : R.count) == null ? void 0 : F.files_direct) ?? oe;
        this.store.setState({
          assets: de.files || [],
          folders: K,
          folderPreviews: ae,
          totalCount: re,
          totalFolderCount: me.total ?? K.length,
          offset: 0,
          hasMore: V,
          isLoading: !1
        });
        ...
        const ce = s.labels.find((is) => is.uuid === s.activeLabelUuid);
        if (!ce) {
          this.store.setState({ isLoading: !1 });
          return;
        }
        const Me = ce.sid.replace("#", ""), me = this._buildSearchNotation(), de = `labels:${Me}${me ? " " + me : ""}`, W = s.isAISearchActive && !!s.searchQuery, K = Ue(this.apiClient, {
          folder: "/",
          offset: 0,
          limit: s.limit,
          sort_by: s.sortBy,
          ...
          ...W && {
            with_ai: !0,
            ai_query: s.searchQuery,
            ai_lang: ((E = s.config) == null ? void 0 : E.locale) ?? "en"
          }
          ...
          ...W && {
            with_ai: !0,
            ai_query: s.searchQuery,
            ai_lang: ((M = s.config) == null ? void 0 : M.locale) ?? "en"
          }
        }).catch(() => null), [oe, V] = await Promise.all([K, ae]);
        if (e !== this._loadId) return;
        const re = ((I = oe.files) == null ? void 0 : I.length) ?? 0, at = re >= s.limit, ss = ((ye = V == null ? void 0 : V.stats) == null ? void 0 : ye.approx_files_count) ?? ((le = V == null ? void 0 : V.info) == null ? void 0 : le.total_files_count) ?? ((z = oe.info) == null ? void 0 : z.total_files_count) ?? ((ie = (se = oe.base) == null ? void 0 : se.count) == null ? void 0 : ie.files_recursive) ?? ((Ne = (U = oe.base) == null ? void 0 : U.count) == null ? void 0 : Ne.files_direct) ?? re;
        this.store.setState({
          assets: oe.files || [],
          folders: [],
          totalCount: ss,
        ...
        const ce = s.activeCollectionFolder;
        if (!(ce && !((zs = ce.children) != null && zs.length))) {
          this.store.setState({
            assets: [],
            folders: [],
            totalCount: 0,
        ...
        const me = s.activeCollectionUuid, de = ce.filters || void 0, W = this._buildSearchNotation(), K = s.isAISearchActive && !!s.searchQuery, ae = Ue(this.apiClient, {
          offset: 0,
          limit: s.limit,
          sort_by: s.sortBy,
          sort_direction: s.sortDirection,
          ...
          ...K && {
            with_ai: !0,
            ai_query: s.searchQuery,
            ai_lang: ((Rs = s.config) == null ? void 0 : Rs.locale) ?? "en"
          }
        }), oe = Dt(this.apiClient, {
          q: W || void 0,
          search: s.searchQuery || void 0,
          recursive: 1,
          collection_uuid: me,
          ...
          ...K && {
            with_ai: !0,
            ai_query: s.searchQuery,
            ai_lang: ((js = s.config) == null ? void 0 : js.locale) ?? "en"
          }
        }).catch(() => null), [V, re] = await Promise.all([ae, oe]);
        if (e !== this._loadId) return;
        const at = ((Ns = V.files) == null ? void 0 : Ns.length) ?? 0, ss = at >= s.limit, is = ((Us = re == null ? void 0 : re.stats) == null ? void 0 : Us.approx_files_count) ?? ((Bs = re == null ? void 0 : re.info) == null ? void 0 : Bs.total_files_count) ?? ((qs = V.info) == null ? void 0 : qs.total_files_count) ?? ((Hs = (Vs = V.base) == null ? void 0 : Vs.count) == null ? void 0 : Hs.files_recursive) ?? ((Ks = (Gs = V.base) == null ? void 0 : Gs.count) == null ? void 0 : Ks.files_direct) ?? at;
        this.store.setState({
          assets: V.files || [],
          folders: [],
          totalCount: is,
      ...
      this.store.setState({ isLoading: !1 }), this.dispatchEvent(new CustomEvent("ap-error", {
        detail: { error: it, context: "loadData" },
        bubbles: !0,
        composed: !0
      }));
    }
  ...
  async _loadMore() {
    ...
    try {
      ...
      if (e.activeTab === "labels" && e.activeLabelUuid) {
        const L = e.labels.find(($) => $.uuid === e.activeLabelUuid);
        L && (l = `labels:${L.sid.replace("#", "")}${r ? " " + r : ""}`), c = "/", h = 1;
      }
      let u, g;
      ...
      const d = await Ue(this.apiClient, {
        ...c ? { folder: c } : {},
        offset: i,
        limit: e.limit,
        sort_by: e.sortBy,
      ...
      const f = this.store.getState().assets, b = d.files || [], x = b.length >= e.limit, w = [...f, ...b];
      this.store.setState({
        assets: w,
        offset: i,
        hasMore: x,
  ...
  _handleAISearchToggle(e) {
    ...
    i.searchQuery && (s ? a.sortBy = "relevance" : i.sortBy === "relevance" && (a.sortBy = ((o = this.config) == null ? void 0 : o.defaultSortBy) ?? "created_at"), a.offset = 0, a.assets = [], a.folders = [], a.isLoading = !0), this.store.setState(a), i.searchQuery && this._debouncedLoadData();
  }
  _handleViewChange(e) {
    var i;
    const s = e.detail.mode;
    this.store.setState({ viewMode: s }), (i = this.config) != null && i.rememberLastView && Ba(s, this.config.auth.projectToken), this._scrollToTop();
  }
  ...
  _handleSortChange(e) {
    ...
    this.store.setState({
      ...
      assets: [],
      folders: []
    }), si(s, i.sortDirection), this.selectionCtrl.resetRange(), this._loadData();
  }
  _handleSortDirectionChange(e) {
    ...
    this.store.setState({
      ...
      assets: [],
      folders: []
    }), si(i.sortBy, s), this.selectionCtrl.resetRange(), this._loadData();
  }
  _handleTabChange(e) {
    var i, a;
    const s = e.detail.tab;
    (i = this.config) != null && i.rememberLastTab && Va(s, this.config.auth.projectToken), this._selectAllId++, this.store.setState({
      activeTab: s,
      activeLabelUuid: null,
      activeCollectionUuid: null,
      activeCollectionFolders: [],
  ...
  _handleQuickSelect(e) {
    ...
    if ((i = this.config) != null && i.transformations && rt(s)) {
  ...
  _emitSelect(e, s) {
    var a, o;
    const i = { assets: e };
    s != null && s.length && (i.folders = s), (o = (a = this.config) == null ? void 0 : a.onSelect) == null || o.call(a, e, s != null && s.length ? s : void 0), this.dispatchEvent(new CustomEvent("ap-select", {
      detail: i,
      bubbles: !0,
      composed: !0
  ...
  _handleFolderOpen(e) {
    ...
    (o = this.config) != null && o.rememberLastFolder && ii(a, this.config.auth.projectToken), this._selectAllId++, this.store.setState({
  ...
  _handleBreadcrumbNavigate(e) {
    ...
    if (i.activeTab === "collections") {
      ...
      if (h) {
        const u = i.breadcrumb.findIndex((d) => d.uuid === s), g = i.breadcrumb.slice(0, u + 1);
        this.store.setState({
    ...
    const a = s ? i.breadcrumb.findIndex((h) => h.uuid === s) : -1, o = s ? i.breadcrumb.slice(0, a + 1) : [], r = o.length > 0 ? o[o.length - 1].path : ((l = this.config) == null ? void 0 : l.rootFolderPath) || "/";
    (c = this.config) != null && c.rememberLastFolder && ii(r, this.config.auth.projectToken), this._selectAllId++, this.store.setState({
  ...
  async _handleCollectionOpen(e) {
    ...
    if (this.apiClient) {
      ...
      try {
        const i = await Bi(this.apiClient, s.uuid), a = oi(i.folders || []);
        this.store.setState({
  ...
  _handleCollectionFolderOpen(e) {
    var l;
    const s = e.detail.folder, i = this.store.getState(), a = s.path.split(" -> ").filter(Boolean), o = [
      { uuid: i.activeCollectionUuid, name: ((l = i.breadcrumb[0]) == null ? void 0 : l.name) || "Collection", path: "" }
    ];
    let r = "";
    for (const c of a)
      r = r ? `${r} -> ${c}` : c, o.push({
  ...
  _resolveInitialFolderPath(e) {
    var s, i;
    return e === "folders" && ((s = this.config) == null ? void 0 : s.rememberLastFolder) && Ua(this.config.auth.projectToken) || ((i = this.config) == null ? void 0 : i.rootFolderPath) || "/";
  }
  /**
  ...
  _buildBreadcrumbFromPath(e, s) {
    if (!e || e === s || e === "/") return [];
    const a = (e.startsWith(s) ? e.slice(s.length) : e.replace(/^\//, "")).split("/").filter(Boolean), o = [];
    let r = s.endsWith("/") ? s : s + "/";
    for (const l of a)
      r += l + "/", o.push({ uuid: r, name: l, path: r });
    return o;
  }
  _handlePreviewClose() {
  ...
  _handleSelectionConfirm(e) {
    ...
    if (i.length > 0 && ((a = this.config) == null ? void 0 : a.folderSelection) !== !1 && ((o = this.config) == null ? void 0 : o.folderSelectionMode) === "assets") {
  ...
  _handleSelectionTransform(e) {
    var o, r;
    const s = e.detail.assets, i = e.detail.folders || [];
    if (i.length > 0 && ((o = this.config) == null ? void 0 : o.folderSelection) !== !1 && ((r = this.config) == null ? void 0 : r.folderSelectionMode) === "assets") {
      this._transformAfterResolve = !0, this._folderResolveOpen = !0;
      return;
    }
    if (s.filter(rt).length === 0) {
      this._emitSelect(s, i.length ? i : void 0);
      return;
    }
  ...
  async _handleFolderResolveConfirm(e) {
    ...
    try {
      ...
      for (const d of r)
        l.push(...d.files);
      const c = [...a], h = new Set(c.map((d) => d.uuid));
      for (const d of l)
        h.has(d.uuid) || (c.push(d), h.add(d.uuid));
      ...
      this.store.setState({ isResolvingFolders: !1 }), this.dispatchEvent(new CustomEvent("ap-error", {
        detail: { error: r, context: "folderResolve" },
        bubbles: !0,
        composed: !0
      }));
    }
  ...
  _handleTransformConfirm(e) {
    const s = e.detail.params, i = e.detail.isAspectLocked ?? !0, a = this._transformAssets, o = this._transformFolders, r = a.length > 1, l = di(s, { isMultiSelect: r, isAspectLocked: i }), c = a.map((h) => {
      var f, b;
      if (!rt(h)) return h;
      const u = (f = h.url) == null ? void 0 : f.cdn, g = (b = h.url) == null ? void 0 : b.permalink, d = {
        cdn: u ? Pe(u, l) : ""
      };
      return g && (d.permalink_cdn = Pe(g, l)), { ...h, transformation: { params: s, url: d } };
    });
  ...
  async _handleSelectAll(e) {
    ...
    const a = ((h = e == null ? void 0 : e.detail) == null ? void 0 : h.scope) ?? "all", o = a !== "assets" && ((u = this.config) == null ? void 0 : u.folderSelection) !== !1 && s.folders.length > 0;
    if (!(a !== "folders")) {
    ...
    try {
      ...
      const f = s.limit, b = s.assets, x = s.totalCount;
      let w = s.activeTab === "folders" ? 0 : 1, L = g || void 0;
      if (s.activeTab === "labels" && s.activeLabelUuid) {
        const z = s.labels.find((se) => se.uuid === s.activeLabelUuid);
        z && (L = `labels:${z.sid.replace("#", "")}${g ? " " + g : ""}`), d = "/", w = 1;
      }
      let $, R;
      s.activeTab === "collections" && s.activeCollectionFolder && ($ = s.activeCollectionUuid || void 0, R = s.activeCollectionFolder.filters || void 0, d = "", w = 1);
      const F = [];
      for (let z = b.length; z < x; z += f)
        F.push(z);
      const E = 4, M = [];
      for (let z = 0; z < F.length; z += E) {
        if (l !== this._selectAllId) return;
        const se = F.slice(z, z + E), ie = await Promise.all(
          se.map(
            (U) => {
              var Ne;
              return Ue(this.apiClient, {
                ...
                ...s.isAISearchActive && s.searchQuery && {
                  with_ai: !0,
                  ai_query: s.searchQuery,
                  ai_lang: ((Ne = s.config) == null ? void 0 : Ne.locale) ?? "en"
                }
              });
            }
          )
        ...
        if (l !== this._selectAllId) return;
        for (const U of ie)
          U.files && M.push(...U.files);
      }
      if (l !== this._selectAllId) return;
      const I = new Set(b.map((z) => z.uuid)), ye = M.filter((z) => I.has(z.uuid) ? !1 : (I.add(z.uuid), !0)), le = [...b, ...ye];
      this.store.setState({
        assets: le,
        offset: Math.max(0, le.length - f),
        hasMore: !1,
        isSelectingAll: !1
      ...
      this.store.setState({ isSelectingAll: !1 }), this.dispatchEvent(new CustomEvent("ap-error", {
        detail: { error: g, context: "selectAll" },
        bubbles: !0,
        composed: !0
      }));
    }
  ...
  // ── Filter Handlers ──────────────────────────────────────────────────
  _isDateFilterKey(e) {
    return e === y.DATE || e === y.LICENSE_EXPIRY || e === X.DUE_DATE;
  }
  _handleFilterUpdate(e) {
    const { key: s, values: i, operator: a } = e.detail, r = { ...this.store.getState().filters };
    let l = i === "" || i === null || i === void 0 || Array.isArray(i) && i.length === 0 || typeof i == "object" && !Array.isArray(i) && Object.values(i).every(
      (c) => c == null || c === "" || Array.isArray(c) && c.length === 0
    );
    if (!l && this._isDateFilterKey(s) && typeof i == "object" && !Array.isArray(i)) {
      const { kind: c, preset: h, from: u, to: g } = i;
      l = !c && !h && !u && !g;
    }
    if (l) {
      const c = { ...r.applied };
      delete c[s], r.applied = c, r.pinned.includes(s) || (r.visible = r.visible.filter((h) => h !== s));
    } else {
      const c = { ...r.applied };
      this._isDateFilterKey(s) && typeof i == "object" && !Array.isArray(i) ? c[s] = {
        ...
        from: i.from || null,
        to: i.to || null
      } : s === y.IMAGE && typeof i == "object" && !Array.isArray(i) ? c[s] = i : c[s] = {
        type: "string",
        values: Array.isArray(i) ? i : [i],
        operator: a || ":"
      }, r.applied = c, r.visible.includes(s) || (r.visible = [...r.visible, s]);
    }
    if (this.store.setState({ filters: r, offset: 0, assets: [], folders: [], isLoading: !0 }), this.selectionCtrl.resetRange(), this._debouncedLoadData(), this._pendingFilter === s) {
      this._pendingFilter = null;
      const c = this.renderRoot.querySelector("ap-filters-bar");
      c && (c.pendingFilter = null);
    }
  }
  _handleFilterOpen(e) {
    ...
    if (i) {
      ...
      if (l) {
        ...
        o = i.left - c.left, r = i.bottom - c.top;
      }
    }
    a == null || a.openFilterPanel(s, !0, o, r);
  }
  _handleMetadataFilterOpen(e) {
    ...
    a == null || a.openMetadataFieldPanel(s, !0, o, r);
  }
  ...
  async _handleFilterPending(e) {
    ...
    if (!r) {
      ...
      for (const L of x) {
        const $ = (b = (f = L.querySelector(".chip-label")) == null ? void 0 : f.textContent) == null ? void 0 : b.trim();
        if (i && $ === w) {
          r = L;
          break;
        }
    ...
    if (!r) return;
    const l = r.getBoundingClientRect(), c = this.renderRoot.querySelector(".toolbar-filters-wrapper"), h = c == null ? void 0 : c.getBoundingClientRect(), u = h ? l.left - h.left : l.left, g = h ? l.bottom - h.top : void 0;
    i ? o.openMetadataFieldPanel(i, !0, u, g) : s && o.openFilterPanel(s, !0, u, g);
  }
  _handleFilterRemove(e) {
    var r;
    const s = e.detail.key;
    if (s in (((r = this.config) == null ? void 0 : r.forcedFilters) ?? {})) return;
    const a = { ...this.store.getState().filters }, o = { ...a.applied };
    delete o[s], a.applied = o, a.pinned.includes(s) || (a.visible = a.visible.filter((l) => l !== s)), this.store.setState({ filters: a, offset: 0, assets: [], folders: [], isLoading: !0 }), this.selectionCtrl.resetRange(), this._debouncedLoadData();
  }
  _handleMetadataFilterChange(e) {
    const { fieldKey: s, operator: i, values: a, metadataType: o } = e.detail, l = { ...this.store.getState().filters }, c = { ...l.metadata }, h = { ...c.applied };
    if (!a || Array.isArray(a) && a.length === 0 ? (delete h[s], c.applied = h, c.pinned.includes(s) || (c.visible = c.visible.filter((g) => g !== s))) : (h[s] = {
      type: "string",
      ...
      const g = this.renderRoot.querySelector("ap-filters-bar");
      g && (g.pendingMetadataField = null);
    }
  }
  _handleMetadataFieldToggle(e) {
    const { fieldKey: s, visible: i } = e.detail, o = { ...this.store.getState().filters }, r = { ...o.metadata };
    i ? r.visible.includes(s) || (r.visible = [...r.visible, s]) : r.visible = r.visible.filter((l) => l !== s), o.metadata = r, this.store.setState({ filters: o });
  }
  _handleMetadataFilterRemove(e) {
    const { fieldKey: s } = e.detail, a = { ...this.store.getState().filters }, o = { ...a.metadata }, r = { ...o.applied };
    delete r[s], o.applied = r, o.pinned.includes(s) || (o.visible = o.visible.filter((l) => l !== s)), a.metadata = o, this.store.setState({ filters: a, offset: 0, assets: [], folders: [], isLoading: !0 }), this.selectionCtrl.resetRange(), this._debouncedLoadData();
  }
  _handleFilterPin(e) {
    var l, c;
    ...
    const r = ((c = (l = this.store.getState().config) == null ? void 0 : l.auth) == null ? void 0 : c.projectToken) ?? null;
    Ra(r, o.pinned);
  }
  _handleMetadataPin(e) {
    var c, h;
    const { fieldKey: s, pinned: i } = e.detail, o = { ...this.store.getState().filters }, r = { ...o.metadata };
    i ? r.pinned.includes(s) || (r.pinned = [...r.pinned, s]) : (r.pinned = r.pinned.filter((u) => u !== s), s in r.applied || (r.visible = r.visible.filter((u) => u !== s))), o.metadata = r, this.store.setState({ filters: o });
  ...
  _handleFilterDeactivate(e) {
    ...
    a.visible = a.visible.filter((o) => o !== s), this.store.setState({ filters: a });
  }
  _handleMetadataFieldDeactivate(e) {
    const { fieldKey: s } = e.detail, a = { ...this.store.getState().filters }, o = { ...a.metadata };
    o.visible = o.visible.filter((r) => r !== s), a.metadata = o, this.store.setState({ filters: a });
  }
  _handleFiltersSet(e) {
    var h;
    const { applied: s, metadata: i } = e.detail, o = { ...this.store.getState().filters }, r = new Set(Object.keys(((h = this.config) == null ? void 0 : h.forcedFilters) ?? {})), l = { ...s };
    for (const u of r) delete l[u];
    o.applied = l;
    const c = Object.keys(s);
    if (o.visible = [.../* @__PURE__ */ new Set([...o.pinned, ...c])], i) {
      const u = { ...o.metadata };
      u.applied = i.applied;
      const g = Object.keys(i.applied);
      u.visible = [.../* @__PURE__ */ new Set([...u.pinned, ...g])], o.metadata = u;
    }
  ...
  _getSortOptions() {
    ...
    if (e.activeTab === "collections") {
      if (!e.activeCollectionUuid) return Ja;
      if (!(e.activeCollectionFolder && !((s = e.activeCollectionFolder.children) != null && s.length))) return eo;
    }
    return e.searchQuery ? Wa : e.activeTab === "folders" ? Xa : $s;
  }
  ...
  render() {
    var d, f, b, x, w, L, $, R, F, E, M, I, ye, le, z, se, ie, U;
    const e = this.storeCtrl.state, s = Array.from(e.selectedAssets.keys()), i = Array.from(e.selectedFolders.keys()), a = this.selectionCtrl.getSelectedAssets(), o = this.selectionCtrl.getSelectedFolders(), r = ((d = this.config) == null ? void 0 : d.folderSelection) !== !1, l = n`
      <ap-header
      ...
      ></ap-header>
    `, c = !!((w = this.config) != null && w.uploader), h = n`
      ...
      >
        ...
        <div class="main-content">
          <div class="toolbar-filters-wrapper${(L = this.config) != null && L.stickyFilters ? " sticky" : ""}">
            ...
            ${e.activeTab === "labels" && !e.activeLabelUuid || e.activeTab === "collections" && !(e.activeCollectionFolder && !((F = e.activeCollectionFolder.children) != null && F.length)) ? v : n`
            <ap-filters-bar
              ...
              .forcedFilters=${((E = this.config) == null ? void 0 : E.forcedFilters) ?? {}}
          ...
          ${e.isLoading && e.assets.length === 0 && e.folders.length === 0 && !(e.activeTab === "labels" && !e.activeLabelUuid) && !(e.activeTab === "collections" && !(e.activeCollectionFolder && !((M = e.activeCollectionFolder.children) != null && M.length))) ? n`<ap-skeleton .variant=${e.viewMode} .gridSize=${((I = this.config) == null ? void 0 : I.gridSize) ?? "normal"} .multiSelect=${((ye = this.config) == null ? void 0 : ye.multiSelect) !== !1} .folderCount=${2}></ap-skeleton>` : this._renderContent(e, s, i, r)}
        ...
        ${e.isPreviewOpen && e.previewAsset ? n`<ap-preview-panel
              ...
              .showMetadata=${((le = this.config) == null ? void 0 : le.showMetadata) !== !1}
              .metadataFields=${e.metadataFields}
              .labels=${e.labels}
              .regionalFilters=${e.regionalFilters}
    ...
    `, u = n`
      <ap-selection-bar
        ...
        .multiSelect=${((se = this.config) == null ? void 0 : se.multiSelect) ?? !0}
        .maxSelections=${(ie = this.config) == null ? void 0 : ie.maxSelections}
        .showTransform=${!!((U = this.config) != null && U.transformations) && a.some(rt)}
  ...
  _renderContent(e, s, i = [], a = !1) {
    ...
    if (e.activeTab === "folders")
      return !e.isLoading && e.folders.length === 0 && e.assets.length === 0 ? n`
          <div class="empty-state">
    ...
    if (e.activeTab === "labels") {
      if (!e.activeLabelUuid) {
        ...
        if (e.searchQuery) {
          ...
          $ = e.labels.filter((E) => E.name.toLowerCase().includes(F));
        }
        const R = e.sortDirection === "asc" ? 1 : -1;
        return $ = [...$].sort((F, E) => R * F.name.localeCompare(E.name)), $.length === 0 ? n`
            <div class="empty-state">
          ...
          ` : e.viewMode === "grid" ? n`
            <div class="labels-grid">
              ${$.map((F, E) => n`
                <ap-label-card .label=${F} .index=${E} @label-open=${this._handleLabelOpen}></ap-label-card>
              `)}
            </div>
          ` : n`
          <div class="labels-list">
            ${$.map((F, E) => n`
              <ap-label-row .label=${F} .index=${E} @label-open=${this._handleLabelOpen}></ap-label-row>
            `)}
          </div>
        `;
      }
    ...
    if (e.activeTab === "collections") {
      if (!e.activeCollectionUuid) {
        let F = e.collections;
        if (e.searchQuery) {
          const M = e.searchQuery.toLowerCase();
          F = e.collections.filter((I) => I.title.toLowerCase().includes(M));
        }
        const E = e.sortDirection === "asc" ? 1 : -1;
        return e.sortBy === "created_at" ? F = [...F].sort((M, I) => E * (new Date(M.created_at).getTime() - new Date(I.created_at).getTime())) : e.sortBy === "updated_at" ? F = [...F].sort((M, I) => E * (new Date(M.updated_at).getTime() - new Date(I.updated_at).getTime())) : F = [...F].sort((M, I) => E * M.title.localeCompare(I.title)), F.length === 0 ? n`
            <div class="empty-state">
          ...
          ` : e.viewMode === "grid" ? n`
            <div class="collections-grid">
              ${F.map((M, I) => n`
                <ap-collection-card .collection=${M} .index=${I} @collection-open=${this._handleCollectionOpen}></ap-collection-card>
              `)}
            </div>
          ` : n`
          <div class="collections-list">
            ${F.map((M, I) => n`
              <ap-collection-row .collection=${M} .index=${I} @collection-open=${this._handleCollectionOpen}></ap-collection-row>
            `)}
          </div>
        `;
      }
      ...
      const $ = this._getVisibleCollectionFolders();
      if (!e.activeCollectionFolder || (((b = e.activeCollectionFolder.children) == null ? void 0 : b.length) ?? 0) > 0) {
        let F = $;
        if (e.searchQuery) {
          const M = e.searchQuery.toLowerCase();
          F = $.filter((I) => I.name.toLowerCase().includes(M));
        }
        const E = e.sortDirection === "asc" ? 1 : -1;
        return F = [...F].sort((M, I) => E * M.name.localeCompare(I.name)), F.length === 0 ? n`
            <div class="empty-state">
              <ap-icon name="folder" .size=${48}></ap-icon>
          ...
          ` : e.viewMode === "grid" ? n`
            <div class="collection-folders-grid">
              ${F.map((M, I) => n`
                <ap-collection-folder-card .folder=${M} .index=${I} @collection-folder-open=${this._handleCollectionFolderOpen}></ap-collection-folder-card>
              `)}
            </div>
          ` : n`
          <div class="collection-folders-list">
            ${F.map((M, I) => n`
              <ap-collection-folder-row .folder=${M} .index=${I} @collection-folder-open=${this._handleCollectionFolderOpen}></ap-collection-folder-row>
            `)}
          </div>
        `;
      }
...
Pt([
  _()
], Xe.prototype, "_isDragOver");
Pt([
  _()
], Xe.prototype, "_isUploaderOpen");
Pt([
  _()
], Xe.prototype, "_folderResolveOpen");
Pt([
  _()
