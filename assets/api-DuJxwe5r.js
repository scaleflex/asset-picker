...
picker.close();`)}
        ...
        <table>
          ...
          <tbody>
            <tr><td><code>ap-select</code></td><td><code>{ assets: Asset[], folders?: Folder[] }</code></td><td>Fired when the user confirms their selection. <code>folders</code> is populated when <code>folderSelection: true</code> and the user selected folders.</td></tr>
        ...
        ${n(`javascript`,`picker.addEventListener('ap-select', (e) => {
...
picker.addEventListener('ap-error', (e) => {
  console.error(\`[\${e.detail.context}]\`, e.detail.error);
        ...
        <h2>React API</h2>
        ${n(`tsx`,`import { AssetPicker, type AssetPickerRef, type AssetPickerProps } from '@scaleflex/asset-picker/react';`)}
        ...
        ${n(`tsx`,`const [isOpen, setIsOpen] = useState(false);
...
/>`)}
        ...
        ${n(`tsx`,`const ref = useRef<AssetPickerRef>(null);

<button onClick={() => ref.current?.open()}>Open</button>
<AssetPicker ref={ref} config={config} onSelect={handleSelect} />`)}

        ${e({href:`#/docs/configuration`,label:`Configuration`},{href:`#/docs/theming`,label:`Theming`})}
      </div>
