import{h as t,c as e,d}from"./doc-utils-XkOyWBCy.js";const a={render(){return`
    ...
    projectToken: string,
        ...
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>enabledMetadataFields</code></td><td><code>string[]</code></td><td>all</td><td>Restrict which metadata fields (by <code>MetadataModelField.key</code>, e.g. <code>'campaign_name'</code>) can be added as filters — in the "Add metadata field" popup and the metadata filter chips. Excluded pinned fields are hidden from the filters bar too. Does not affect the preview panel, which always shows the full field list (see <code>showMetadata</code>). <code>[]</code> makes no metadata fields available</td></tr>
            ...
            <tr><td><code>approverUsers</code></td><td><code>ApprovalUser[]</code></td><td><code>undefined</code></td><td>People selectable in the <strong>Approver</strong> filter, one of the four the Approval entry offers. The picker has no access to a company directory, so the host application supplies the list; each <code>uuid</code> is sent as <code>task_approver:"&lt;uuid&gt;"</code>. Omit it and Approver is left out of the list — see <a href="#/examples/approval-users">Approval users</a></td></tr>
            <tr><td><code>requesterUsers</code></td><td><code>ApprovalUser[]</code></td><td><code>undefined</code></td><td>Same as <code>approverUsers</code> for the <strong>Requester</strong> section; uuids are sent as <code>task_requester:"&lt;uuid&gt;"</code></td></tr>
            ...
            <tr><td><code>variants</code></td><td><code>boolean</code></td><td><code>false</code></td><td>Let users insert a container preset/variant instead of the original file. Adds a default-variant dropdown to the toolbar (applies to every insert) and a per-asset switcher badge on each card. Selected assets come back with <code>selectedVariant</code> (<code>{ name, url }</code>); <code>asset.url</code> is always kept. Image assets only</td></tr>
