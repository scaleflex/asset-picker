import{g as S,f as z,l as P,h as C,d as n,A as f,m as A,n as g,r as x}from"./index-Df_1wNo8.js";import{a as I,l as M,A as y,c as B,g as L,d as V,e as H,b as $}from"./define-BKpMPOIF.js";const k="sfx-uploader-token:";function m(l){try{return localStorage.getItem(`${k}${l}`)}catch{return null}}function T(l,e){try{localStorage.setItem(`${k}${l}`,e)}catch{}}function _(l){try{localStorage.removeItem(`${k}${l}`)}catch{}}function j(l,e){const t=r=>{if(l&&r.source!==l)return;const s=typeof r.data=="string"?E(r.data):r.data;s!=null&&s.token&&e(s.token)};return window.addEventListener("message",t),()=>window.removeEventListener("message",t)}function E(l){try{return JSON.parse(l)}catch{return null}}var U=Object.defineProperty,h=(l,e,t,r)=>{for(var s=void 0,u=l.length-1,i;u>=0;u--)(i=l[u])&&(s=i(e,t,s)||s);return s&&U(e,t,s),s};const w=class w extends S{constructor(){super(...arguments),this.t=z,this.provider="google-drive",this.companionUrl="",this.multi=!0,this.maxSelect=null,this.transformThumbnail=e=>e,this._authenticated=!1,this._loading=!1,this._items=[],this._selectedIds=new Set,this._breadcrumbs=[],this._nextPagePath=null,this._error=null,this._loadingMore=!1,this._username=null,this._resolvingFolders=!1,this._resolveProgress=0,this._cleanupAuthListener=null,this._authWindow=null,this._resolveAbort=null,this._handleConnect=()=>{var t;try{if(this._authWindow&&!this._authWindow.closed){this._authWindow.focus();return}}catch{this._authWindow=null}const e=I(this.companionUrl,this.provider);this._authWindow=window.open(e,"_blank","width=600,height=600"),(t=this._cleanupAuthListener)==null||t.call(this),this._cleanupAuthListener=j(this._authWindow,r=>{var s;this._authWindow=null,(s=this._cleanupAuthListener)==null||s.call(this),this._cleanupAuthListener=null,T(this.provider,r),this._authenticated=!0,this._loadFolder("")})},this._lastClickedIndex=null,this._toggleSelectAll=()=>{if(this._items.every(t=>this._selectedIds.has(t.id)))this._selectedIds=new Set;else{const t=new Set;let r=0;for(const s of this._items)s.isFolder?t.add(s.id):(this.maxSelect===null||r<this.maxSelect)&&(t.add(s.id),r++);this._selectedIds=t}},this._onAddSelected=async()=>{var u,i;const e=m(this.provider);if(!e)return;const t=this._items.filter(o=>!o.isFolder&&this._selectedIds.has(o.id)),r=this._items.filter(o=>o.isFolder&&this._selectedIds.has(o.id)),s=t.map(o=>({companionUrl:this.companionUrl,provider:this.provider,token:e,requestPath:o.requestPath,fileId:o.id,name:o.name,mimeType:o.mimeType,size:o.size,thumbnail:o.thumbnail}));if(r.length>0){(u=this._resolveAbort)==null||u.abort(),this._resolveAbort=new AbortController;const o=this._resolveAbort.signal;this._resolvingFolders=!0,this._resolveProgress=0;let c=0;try{for(const a of r){const b=await M(this.companionUrl,this.provider,e,a.requestPath,a.name,o);if(o.aborted)return;for(const p of b)s.push({companionUrl:this.companionUrl,provider:this.provider,token:e,requestPath:p.requestPath,fileId:p.id,name:p.name,mimeType:p.mimeType,size:p.size,thumbnail:p.thumbnail,relativeFolder:p.relativeFolder});c+=b.length,this._resolveProgress=c}}catch(a){if(o.aborted)return;if(this._resolvingFolders=!1,a instanceof y){_(this.provider),this._authenticated=!1;return}this._error=a instanceof Error?a.message:this.t("failedToReadFolder","Failed to read folder contents");return}finally{((i=this._resolveAbort)==null?void 0:i.signal)===o&&(this._resolveAbort=null)}this._resolvingFolders=!1}this.dispatchEvent(new CustomEvent("connector-files-selected",{detail:{files:s},bubbles:!0,composed:!0}))},this._onClose=()=>{this.dispatchEvent(new CustomEvent("connector-close",{bubbles:!0,composed:!0}))},this._handleLogout=async()=>{const e=m(this.provider);if(e){try{await B(this.companionUrl,this.provider,e)}catch{}_(this.provider)}this._reset()},this._onBack=()=>{this._breadcrumbs.length!==0&&this._onBreadcrumbClick(this._breadcrumbs.length-2)},this._cancelResolve=()=>{var e;(e=this._resolveAbort)==null||e.abort(),this._resolveAbort=null,this._resolvingFolders=!1,this._resolveProgress=0}}connectedCallback(){super.connectedCallback(),this._checkAuth()}disconnectedCallback(){var e,t;super.disconnectedCallback(),(e=this._cleanupAuthListener)==null||e.call(this),this._cleanupAuthListener=null,(t=this._resolveAbort)==null||t.abort(),this._resolveAbort=null}willUpdate(e){e.has("provider")&&e.get("provider")!==void 0&&(this._reset(),this._checkAuth())}_reset(){var e;(e=this._resolveAbort)==null||e.abort(),this._resolveAbort=null,this._authenticated=!1,this._loading=!1,this._items=[],this._selectedIds=new Set,this._lastClickedIndex=null,this._breadcrumbs=[],this._nextPagePath=null,this._error=null,this._username=null,this._resolvingFolders=!1,this._resolveProgress=0}_checkAuth(){m(this.provider)&&(this._authenticated=!0,this._loadFolder(""))}get _providerDef(){return L([this.provider])[0]??null}get _providerLabel(){var e;return((e=this._providerDef)==null?void 0:e.label)??this.provider}async _loadFolder(e){const t=m(this.provider);if(!t){this._authenticated=!1;return}this.offsetHeight>0&&(this.style.minHeight=`${this.offsetHeight}px`),this._loading=!0,this._error=null,this._items=[],this._selectedIds=new Set,this._lastClickedIndex=null,this._nextPagePath=null;try{const r=await V(this.companionUrl,this.provider,t,e);this._items=r.items,this._nextPagePath=r.nextPagePath,r.username&&(this._username=r.username)}catch(r){r instanceof y?(_(this.provider),this._authenticated=!1):this._error=r instanceof Error?r.message:this.t("failedToLoadFiles","Failed to load files")}finally{this._loading=!1}}_onFolderClick(e){this._breadcrumbs=[...this._breadcrumbs,{name:e.name,path:e.requestPath}],this._loadFolder(e.requestPath)}_onBreadcrumbClick(e){if(e<0)this._breadcrumbs=[],this._loadFolder("");else{const t=this._breadcrumbs[e];this._breadcrumbs=this._breadcrumbs.slice(0,e+1),this._loadFolder(t.path)}}async _onLoadMore(){const e=m(this.provider);if(!(!e||!this._nextPagePath)){this._loadingMore=!0;try{const t=await H(this.companionUrl,e,this._nextPagePath);this._items=[...this._items,...t.items],this._nextPagePath=t.nextPagePath}catch(t){t instanceof y&&(_(this.provider),this._authenticated=!1)}finally{this._loadingMore=!1}}}get _selectedFileCount(){return this._items.filter(e=>!e.isFolder&&this._selectedIds.has(e.id)).length}_toggleSelect(e,t){if(!this.multi){if(e.isFolder)return;this._selectedIds=this._selectedIds.has(e.id)?new Set:new Set([e.id]);const o=this._items.filter(c=>!c.isFolder).findIndex(c=>c.id===e.id);o!==-1&&(this._lastClickedIndex=o);return}const r=this._items.filter(i=>!i.isFolder),s=r.findIndex(i=>i.id===e.id),u=this.maxSelect!==null&&this._selectedFileCount>=this.maxSelect;if(!e.isFolder&&(t!=null&&t.shiftKey)&&this._lastClickedIndex!==null&&s!==-1){const i=Math.min(this._lastClickedIndex,s),o=Math.max(this._lastClickedIndex,s),c=new Set(this._selectedIds);for(let a=i;a<=o;a++)c.has(r[a].id)||this.maxSelect!==null&&[...c].filter(p=>r.some(v=>v.id===p)).length>=this.maxSelect||c.add(r[a].id);this._selectedIds=c}else{const i=new Set(this._selectedIds);i.has(e.id)?i.delete(e.id):(e.isFolder||!u)&&i.add(e.id),this._selectedIds=i}s!==-1&&(this._lastClickedIndex=s)}render(){return n`
      ${this._renderHeader()}
    ...
    `}_renderHeader(){const e=this._providerDef,t=this._authenticated&&this._breadcrumbs.length>0;return n`
      <div class="browser-header">
        ...
        ${this._authenticated?n`<button class="logout-btn" @click=${this._handleLogout}>
    ...
    `}_renderAuthView(){const e=this._providerDef;return n`
      <div class="auth-view">
        ...
        <div class="auth-logo-wrap">
          <div class="auth-ring">
            <div class="auth-logo">
              ${e!=null&&e.brandHtml?n`<span
                    ${A({display:"flex","align-items":"center","justify-content":"center",transform:"scale(2.2)"})}
                    >${$(e)}</span
                  >`:n`<svg
                    viewBox="0 0 24 24"
                    fill="none"
    ...
    `}_renderError(){return n`
      <div class="error-view">
        ...
        <button
          class="retry-btn"
          @click=${()=>{const e=this._breadcrumbs[this._breadcrumbs.length-1];this._loadFolder((e==null?void 0:e.path)??"")}}
        >
        ...
        ${e.map(i=>{const o=this._selectedIds.has(i.id);return n`
            ...
            >
              ...
              <div class="file-thumb">
                ${i.thumbnail?n`<img
                      src=${this.transformThumbnail(i.thumbnail)}
                      alt=""
                      loading="lazy"
              ...
              <div class="file-info">
                <div class="file-name">${i.name}</div>
                <div class="file-meta">
                  ${i.size?n`<span class="file-size">${D(i.size)}</span>`:f}
                </div>
              </div>
              <span class="file-modified">${F(i.modifiedDate,this.t)}</span>
            </div>
    ...
    `}_renderBreadcrumbs(){return this._breadcrumbs.length===0?f:n`
      <div class="breadcrumbs">
        ...
        ${this._breadcrumbs.map((e,t)=>n`
            ...
            ${t<this._breadcrumbs.length-1?n`<button class="crumb" @click=${()=>this._onBreadcrumbClick(t)}>
                  ${e.name}
                </button>`:n`<span class="crumb-current">${e.name}</span>`}
          `)}
      </div>
    `}};w.styles=C`
    :host {
