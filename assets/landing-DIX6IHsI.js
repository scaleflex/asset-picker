import{t as e}from"./index-DEo6o-Qa.js";import{t}from"./code-block-Dan-Xu_Q.js";var n={render(){return`
    ...
    `},init(r){let i=document.getElementById(`demo-inline-picker`);i.config=e({displayMode:`inline`,onSelect:e=>alert(`Selected ${e.length} asset(s)`)}),document.querySelectorAll(`.quick-start-tab`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.tab;document.querySelectorAll(`.quick-start-tab`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),document.querySelectorAll(`[data-tab-content]`).forEach(e=>{e.style.display=e.dataset.tabContent===t?``:`none`})})}),document.querySelectorAll(`.step-copy`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.code??``;try{await navigator.clipboard.writeText(t),e.classList.add(`copied`),e.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>`}catch{}setTimeout(()=>{e.classList.remove(`copied`),e.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>`},1500)})}),t(`#quick-start-code`,[{label:`HTML`,lang:`markup`,code:`
<script type="module">
...
}`}]);let a=document.querySelectorAll(`.demo-also-slide`),o=document.getElementById(`also-dots`);if(a.length&&o){let e=0,t=!1,r;a.forEach((e,t)=>{let n=document.createElement(`button`);n.className=`demo-also-dot${t===0?` active`:``}`,n.setAttribute(`aria-label`,`Slide ${t+1}`),n.addEventListener(`click`,()=>s(t)),o.appendChild(n)});let i=o.querySelectorAll(`.demo-also-dot`);function s(n){if(n===e||t)return;t=!0;let r=n>e?`left`:`right`,o=a[e],s=a[n];o.classList.add(`slide-out-${r}`),s.classList.add(`slide-in-${r}`,`active`),s.addEventListener(`animationend`,()=>{o.classList.remove(`active`,`slide-out-${r}`),s.classList.remove(`slide-in-${r}`),i[e].classList.remove(`active`),i[n].classList.add(`active`),e=n,t=!1},{once:!0}),c()}function c(){clearInterval(r),r=setInterval(()=>s((e+1)%a.length),5e3)}c(),n._cleanupSlider=()=>clearInterval(r)}},destroy(){let e=document.getElementById(`demo-inline-picker`);e&&e.close(),n._cleanupSlider?.(),delete n._cleanupSlider}};export{n as default};